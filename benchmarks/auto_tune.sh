...
#   BASE: your directory for vllm_logits repo
#   MODEL: the model served by vllm_logits
...
run_benchmark() {
    ...
    local vllm_logits_log="$LOG_FOLDER/vllm_logits_log_${max_num_seqs}_${max_num_batched_tokens}.txt"
    echo "vllm_logits_log: $vllm_logits_log"
    echo
    rm -f $vllm_logits_log

    # start the server
    VLLM_LOGITS_USE_V1=1 VLLM_LOGITS_SERVER_DEV_MODE=1 vllm_logits serve $MODEL \
        ...
        --max-model-len $(( INPUT_LEN+OUTPUT_LEN )) > "$vllm_logits_log" 2>&1 &
    echo "wait for 10 minutes.."
    echo
    # wait for 10 minutes...
    server_started=0
    for i in {1..60}; do        
        if grep -Fq "Application startup complete" "$vllm_logits_log"; then
    ...
    if (( ! server_started )); then
        echo "server did not start within 10 minutes, terminate the benchmarking. Please check server log at $vllm_logits_log"
        echo "pkill -f vllm_logits"
        echo
        pkill vllm_logits
        sleep 10
        return 1
