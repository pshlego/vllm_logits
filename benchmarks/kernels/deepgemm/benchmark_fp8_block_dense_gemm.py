...
from vllm_logits.model_executor.layers.quantization.utils.fp8_utils import (
    per_token_group_quant_fp8,
    w8a8_block_fp8_matmul,
)
from vllm_logits.triton_utils import triton
...
def benchmark_shape(m: int,
    ...
    B_vllm_logits, B_scale_vllm_logits = per_block_cast_to_fp8(B)
    ...
    A_vllm_logits, A_scale_vllm_logits = per_token_group_quant_fp8(A, block_size[1])
    A_vllm_logits_cutlass, A_scale_vllm_logits_cutlass = per_token_group_quant_fp8(
    ...
    def vllm_logits_triton_gemm():
        # A quantization is inside the loop as it depends on activations
        # A_vllm_logits, A_scale_vllm_logits = per_token_group_quant_fp8(A, block_size[1])
        return w8a8_block_fp8_matmul(A_vllm_logits,
                                     B_vllm_logits,
                                     A_scale_vllm_logits,
                                     B_scale_vllm_logits,
                                     block_size,
    ...
    def vllm_logits_cutlass_gemm():
        # A quantization is inside the loop as it depends on activations
        # A_vllm_logits_cutlass, A_scale_vllm_logits_cutlass = per_token_group_quant_fp8(
        #     A, block_size[1], column_major_scales=True)
        return ops.cutlass_scaled_mm(A_vllm_logits_cutlass,
                                     B_vllm_logits.T,
                                     scale_a=A_scale_vllm_logits_cutlass,
                                     scale_b=B_scale_vllm_logits.T,
    ...
    C_vllm_logits_triton = vllm_logits_triton_gemm()
    C_vllm_logits_cutlass = vllm_logits_cutlass_gemm()

    deepgemm_diff = calc_diff(C_deepgemm, C_ref)
    vllm_logits_triton_diff = calc_diff(C_vllm_logits_triton, C_ref)
    vllm_logits_cutlass_diff = calc_diff(C_vllm_logits_cutlass, C_ref)

    if verbose:
        print(f"DeepGEMM vs Reference difference: {deepgemm_diff:.6f}")
        print(f"vLLM Triton vs Reference difference: {vllm_logits_triton_diff:.6f}")
        print(f"vLLM CUTLASS vs Reference difference: {vllm_logits_cutlass_diff:.6f}")
        print("vLLM Triton vs DeepGEMM difference: "
              f"{calc_diff(C_vllm_logits_triton, C_deepgemm):.6f}")
        print("vLLM CUTLASS vs DeepGEMM difference: "
              f"{calc_diff(C_vllm_logits_cutlass, C_deepgemm):.6f}")

    # Benchmark implementations
    implementations = {
        "DeepGEMM": deepgemm_gemm,
        "vLLM Triton": vllm_logits_triton_gemm,
        "vLLM CUTLASS": vllm_logits_cutlass_gemm
    }
    ...
    for name, func in implementations.items():
        ...
        benchmark_results["implementations"][name] = {
            ...
            "diff": {
                ...
                (vllm_logits_triton_diff
                 if name == "vLLM Triton" else vllm_logits_cutlass_diff)
            }
        }
    ...
    vllm_logits_triton_time = benchmark_results["implementations"]["vLLM Triton"][
        "time_ms"]
    vllm_logits_cutlass_time = benchmark_results["implementations"]["vLLM CUTLASS"][
        "time_ms"]
    cutlass_vs_triton = vllm_logits_triton_time / vllm_logits_cutlass_time
...
def run_benchmarks(verbose: bool = False):
    ...
    for result in all_results:
        ...
        vllm_logits_triton_time = result["implementations"]["vLLM Triton"]["time_ms"]
        vllm_logits_cutlass_time = result["implementations"]["vLLM CUTLASS"][
            "time_ms"]

        avg_speedups[
            "DeepGEMM vs vLLM Triton"] += vllm_logits_triton_time / deepgemm_time
        avg_speedups[
            "DeepGEMM vs vLLM CUTLASS"] += vllm_logits_cutlass_time / deepgemm_time
        avg_speedups[
            "vLLM CUTLASS vs vLLM Triton"] += vllm_logits_triton_time / vllm_logits_cutlass_time
