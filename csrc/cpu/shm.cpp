...
void shm_gather(int64_t handle, torch::Tensor& data,
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(data.scalar_type(), "shm_gather_impl", [&] {
    CPU_KERNEL_GUARD_IN(shm_gather_impl)
...
void shm_all_gather(int64_t handle, const torch::Tensor& data,
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(data.scalar_type(), "shm_all_gather_impl", [&] {
...
void shm_allreduce(int64_t handle, torch::Tensor& data) {
  TORCH_CHECK(data.is_contiguous())
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(data.scalar_type(), "shm_allreduce_sum", [&] {
