...
template <typename scalar_t,  // compute dtype, half or nv_float16
          const vllm_logits::ScalarTypeId w_type_id,  // weight ScalarType id
...
__global__ void Marlin(
  ...
  static constexpr auto w_type = vllm_logits::ScalarType::from_id(w_type_id);
  constexpr bool has_zp = w_type == vllm_logits::kU4 || w_type == vllm_logits::kU8;
  constexpr bool is_int_type = w_type == vllm_logits::kU4 || w_type == vllm_logits::kU8 ||
                               w_type == vllm_logits::kU4B8 || w_type == vllm_logits::kU8B128;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop =
      !is_int_type ||
      has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value ||
      has_zp && !is_zp_float && !(w_type == vllm_logits::kU8);
  ...
  const int scales_expert_stride =
      prob_n * prob_k / group_size / (w_type == vllm_logits::kFE2M1f ? 16 : 8);
  ...
  #pragma unroll
        for (int i = 0; i < 4; i++) {
          ...
          if constexpr (w_type == vllm_logits::kFE2M1f) {
  ...
  constexpr int s_tb_groups =
      !has_act_order && group_blocks != -1 && group_blocks < thread_k_blocks
          ? thread_k_blocks / group_blocks / (w_type == vllm_logits::kFE2M1f ? 2 : 1)
          : 1;
  ...
  if constexpr (!has_act_order) {
    if constexpr (group_blocks == -1) {
      ...
      s_gl_rd = s_gl_stride * ((thread_k_blocks * slice_row) / group_blocks) /
                    (w_type == vllm_logits::kFE2M1f ? 2 : 1) +
  ...
  int s_sh_rd;
  if constexpr (group_blocks != -1 && w_type == vllm_logits::kFE2M1f) {
    ...
    if constexpr (!has_act_order) {
      ...
      if constexpr (group_blocks == -1) {
        ...
        if constexpr (group_blocks >= thread_k_blocks) {
          ...
          int cur_group_id =
              k_blocks / (group_blocks * (w_type == vllm_logits::kFE2M1f ? 2 : 1));

          int4* sh_s_stage = sh_s + s_sh_stage * pipe;

          if constexpr (w_type_id != vllm_logits::kFE2M1f.id()) {
