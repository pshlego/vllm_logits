...
#pragma unroll
    for (int k = 0; k < TOPK; ++k) {
      x += VLLM_LOGITS_LDG(&input[token_idx * TOPK * d + k * d + idx]);
    }
...
void moe_align_block_size(torch::Tensor topk_ids, int64_t num_experts,
  ...
  if (use_global_memory) {
    VLLM_LOGITS_DISPATCH_INTEGRAL_AND_UNSIGNED_TYPES(
        topk_ids.scalar_type(), "moe_align_block_size_global_mem_kernel", [&] {
          ...
          auto kernel =
              vllm_logits::moe::moe_align_block_size_global_mem_kernel<scalar_t>;
    ...
    VLLM_LOGITS_DISPATCH_INTEGRAL_AND_UNSIGNED_TYPES(
        topk_ids.scalar_type(), "moe_align_block_size_kernel", [&] {
          ...
          auto kernel =
              vllm_logits::moe::moe_align_block_size_kernel<scalar_t, uint16_t>;
          AT_CUDA_CHECK(VLLM_LOGITS_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(
    ...
    VLLM_LOGITS_DISPATCH_INTEGRAL_AND_UNSIGNED_TYPES(
        topk_ids.scalar_type(), "moe_align_block_size_kernel", [&] {
          auto kernel =
              vllm_logits::moe::moe_align_block_size_kernel<scalar_t, int32_t>;
          AT_CUDA_CHECK(VLLM_LOGITS_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(
...
void sgl_moe_align_block_size(torch::Tensor topk_ids, int64_t num_experts,
  ...
  VLLM_LOGITS_DISPATCH_INTEGRAL_AND_UNSIGNED_TYPES(
      topk_ids.scalar_type(), "sgl_moe_align_block_size_kernel", [&] {
        ...
        auto align_kernel =
            vllm_logits::moe::sgl_moe_align_block_size_kernel<scalar_t>;
        ...
        auto sort_kernel = vllm_logits::moe::sgl_moe_token_sort_kernel<scalar_t>;
             ...
             torch::Tensor& output)  // [num_tokens, hidden_size]
{
  ...
  switch (topk) {
    case 2:
      VLLM_LOGITS_DISPATCH_FLOATING_TYPES(input.scalar_type(), "moe_sum_kernel", [&] {
        vllm_logits::moe::moe_sum_kernel<scalar_t, 2><<<grid, block, 0, stream>>>(
    ...
    case 3:
      VLLM_LOGITS_DISPATCH_FLOATING_TYPES(input.scalar_type(), "moe_sum_kernel", [&] {
        vllm_logits::moe::moe_sum_kernel<scalar_t, 3><<<grid, block, 0, stream>>>(
            output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(),
            hidden_size);
      });
      break;
    ...
    case 4:
      VLLM_LOGITS_DISPATCH_FLOATING_TYPES(input.scalar_type(), "moe_sum_kernel", [&] {
        vllm_logits::moe::moe_sum_kernel<scalar_t, 4><<<grid, block, 0, stream>>>(
            output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(),
            hidden_size);
      });
      break;
