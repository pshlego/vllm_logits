...
#pragma unroll
    ...
    {
        thread_max = max(thread_max, VLLM_LOGITS_SHFL_XOR_SYNC_WIDTH(thread_max, mask, THREADS_PER_ROW));
    }
...
#pragma unroll
    for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2)
    {
        row_sum += VLLM_LOGITS_SHFL_XOR_SYNC_WIDTH(row_sum, mask, THREADS_PER_ROW);
    }
...
#pragma unroll
        for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2)
        {
            float other_max = VLLM_LOGITS_SHFL_XOR_SYNC_WIDTH(max_val, mask, THREADS_PER_ROW);
            int other_expert = VLLM_LOGITS_SHFL_XOR_SYNC_WIDTH(expert, mask, THREADS_PER_ROW);
    ...
    torch::Tensor& gating_output)               // [num_tokens, num_experts]
{
    ...
    {
        vllm_logits::moe::topkGatingSoftmaxKernelLauncher(
