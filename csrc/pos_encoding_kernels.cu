...
inline __device__ void apply_token_rotary_embedding(
  ...
  if (IS_NEOX) {
    ...
    cos = VLLM_LOGITS_LDG(cos_ptr + x_index);
    sin = VLLM_LOGITS_LDG(sin_ptr + x_index);
  } else {
    // GPT-J style rotary embedding.
    x_index = 2 * rot_offset;
    y_index = 2 * rot_offset + 1;
    cos = VLLM_LOGITS_LDG(cos_ptr + x_index / 2);
    sin = VLLM_LOGITS_LDG(sin_ptr + x_index / 2);
  }
...
void rotary_embedding(
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(query.scalar_type(), "rotary_embedding", [&] {
    if (is_neox) {
      vllm_logits::rotary_embedding_kernel<scalar_t, true><<<grid, block, 0, stream>>>(
      ...
      vllm_logits::rotary_embedding_kernel<scalar_t, false>
          <<<grid, block, 0, stream>>>(
              positions.data_ptr<int64_t>(), query.data_ptr<scalar_t>(),
              key.has_value() ? key->data_ptr<scalar_t>() : nullptr,
...
void batched_rotary_embedding(
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(query.scalar_type(), "rotary_embedding", [&] {
    if (is_neox) {
      vllm_logits::batched_rotary_embedding_kernel<scalar_t, true>
          <<<grid, block, 0, stream>>>(
              positions.data_ptr<int64_t>(), query.data_ptr<scalar_t>(),
              key.has_value() ? key->data_ptr<scalar_t>() : nullptr,
      ...
      vllm_logits::batched_rotary_embedding_kernel<scalar_t, false>
          <<<grid, block, 0, stream>>>(
              positions.data_ptr<int64_t>(), query.data_ptr<scalar_t>(),
              key.has_value() ? key->data_ptr<scalar_t>() : nullptr,
              cos_sin_cache.data_ptr<scalar_t>(),
