...
#pragma unroll
  for (int32_t vec_idx = vec_start_idx + threadIdx.x; vec_idx < vec_end_idx;
       vec_idx += blockDim.x) {
    const int4 x_128bit = VLLM_LOGITS_LDG(&x_128bit_ptr[vec_idx]);
    const int4 y_128bit = VLLM_LOGITS_LDG(&y_128bit_ptr[vec_idx]);
...
#pragma unroll
  ...
  if (block_end > vec_loop_end) {
    for (int64_t idx = vec_loop_end + threadIdx.x; idx < block_end;
         idx += blockDim.x) {
      const scalar_t x = VLLM_LOGITS_LDG(&x_ptr[idx]);
      const scalar_t y = VLLM_LOGITS_LDG(&y_ptr[idx]);
      out_ptr[idx] =
...
#define LAUNCH_ACTIVATION_GATE_KERNEL(KERNEL)                               \
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(                                             \
      input.scalar_type(), "act_and_mul_kernel", [&] {                      \
        VLLM_LOGITS_DISPATCH_FP8_TYPES(                                            \
            out.scalar_type(), "fused_add_rms_norm_kernel_fp8_type", [&] {  \
              vllm_logits::act_and_mul_quant_kernel<scalar_t, KERNEL<scalar_t>,    \
                                             fp8_t>                         \
...
void silu_and_mul_quant(torch::Tensor& out,    // [..., d]
  ...
  LAUNCH_ACTIVATION_GATE_KERNEL(vllm_logits::silu_kernel);
}
