...
torch::Tensor aqlm_gemm(const torch::Tensor& input, const torch::Tensor& codes,
                        ...
                        const std::optional<torch::Tensor>& bias) {
  int4 cumulative_sizes =
      vllm_logits::aqlm::accumulate_sizes(codebook_partition_sizes);

  int const nbooks = codebooks.size(0) / codebook_partition_sizes.size();
  int const entries = codebooks.size(1);

  if (nbooks == 1 && entries == (1 << 16)) {
    return vllm_logits::aqlm::code1x16_matmat(input, codes, codebooks, scales,
                                       cumulative_sizes, bias);
  }
  if (nbooks == 2 && entries == (1 << 8)) {
    return vllm_logits::aqlm::code2x8_matmat(input, codes, codebooks, scales,
                                      cumulative_sizes, bias);
  }
...
torch::Tensor aqlm_dequant(
  ...
  if (nbooks == 1 && entries == (1 << 16)) {
    vllm_logits::aqlm::code1x16_dequant_cuda(codes.data_ptr(), weights.data_ptr(),
                                      codebooks.data_ptr(), out_features,
                                      in_features, cumulative_sizes,
                                      vllm_logits::aqlm::codebook_stride(codebooks));
  ...
  if (nbooks == 2 && entries == (1 << 8)) {
    vllm_logits::aqlm::code2x8_dequant_cuda(codes.data_ptr(), weights.data_ptr(),
                                     codebooks.data_ptr(), out_features,
                                     in_features, cumulative_sizes,
                                     vllm_logits::aqlm::codebook_stride(codebooks));
