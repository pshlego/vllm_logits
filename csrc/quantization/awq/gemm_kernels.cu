...
torch::Tensor awq_dequantize(torch::Tensor _kernel,
  ...
  vllm_logits::awq::dequantize_weights<<<num_blocks, threads_per_block, 0, stream>>>(
...
torch::Tensor awq_gemm(torch::Tensor _in_feats, torch::Tensor _kernel,
  ...
  if (num_out_channels % 128 == 0) {
    ...
    vllm_logits::awq::gemm_forward_4bit_cuda_m16nXk32<128>
    ...
    // threadIdx.y: i_factors[2] * j_factors[2]
    dim3 threads_per_block(32, 2);
    vllm_logits::awq::gemm_forward_4bit_cuda_m16nXk32<64>
        <<<num_blocks, threads_per_block, 0, stream>>>(
            group_size, split_k_iters, in_feats, kernel, scaling_factors, zeros,
            num_in_feats, num_in_channels, num_out_channels, out_feats);
  }
