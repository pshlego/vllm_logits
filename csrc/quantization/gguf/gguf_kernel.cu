...
#pragma unroll
  for (int mask = 16; mask > 0; mask >>= 1) {
    amax = fmaxf(amax, VLLM_LOGITS_SHFL_XOR_SYNC_WIDTH(amax, mask, 32));
    sum += VLLM_LOGITS_SHFL_XOR_SYNC_WIDTH(sum, mask, 32);
  }
...
torch::Tensor ggml_dequantize(torch::Tensor W,  // quant weight
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(DW.scalar_type(), "ggml_dequantize", [&] {
...
torch::Tensor ggml_mul_mat_vec_a8(torch::Tensor W,  // quant weight
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(X.scalar_type(), "ggml_mul_mat_vec_a8", [&] {
...
torch::Tensor ggml_mul_mat_a8(torch::Tensor W,  // quant weight
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(X.scalar_type(), "ggml_mul_mat_a8", [&] {
...
torch::Tensor ggml_moe_a8(torch::Tensor X,  // input
  ...
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(X.scalar_type(), "ggml_moe_a8", [&] {
    quantize_row_q8_1_cuda((scalar_t*)X.data_ptr(), (void*)quant_X.data_ptr(),
                           col, tokens, stream);
    switch (type) {
      case 2:
...
torch::Tensor ggml_moe_a8_vec(torch::Tensor X,  // input
  ...
  options = torch::TensorOptions().dtype(torch::kInt32).device(W.device());
  at::Tensor quant_X = torch::empty({tokens, padded / 32 * 9}, options);
  VLLM_LOGITS_DISPATCH_FLOATING_TYPES(X.scalar_type(), "ggml_moe_vec_a8", [&] {
    quantize_row_q8_1_cuda<scalar_t>((scalar_t*)X.data_ptr(),
