...
torch::Tensor gptq_marlin_gemm(
    ...
    vllm_logits::ScalarTypeId const& b_q_type_id, int64_t size_m, int64_t size_n,
...
MarlinFuncPtr get_marlin_kernel(const vllm_logits::ScalarType q_type,
  ...
  COMMON_GET_IF(vllm_logits::kU4)
  COMMON_GET_IF(vllm_logits::kU4B8)
  COMMON_GET_IF(vllm_logits::kU8B128)

  FP4_GET_IF(vllm_logits::kFE2M1f)

  BIGGROUP_GET_IF(vllm_logits::kFE4M3fn)

  ACT_GET_IF(vllm_logits::kU4B8)
  ACT_GET_IF(vllm_logits::kU8B128)

  if (std::is_same<scalar_t, half>::value) {
    if (false) {
    }
    FZP_GET_IF(vllm_logits::kU4)
  }
...
template <typename scalar_t>
exec_config_t determine_exec_config(const vllm_logits::ScalarType& q_type, int prob_m,
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* s,
               ...
               vllm_logits::ScalarType const& q_type, bool has_act_order,
  ...
  if (has_zp) {
    TORCH_CHECK(
        q_type == vllm_logits::kU4 || q_type == vllm_logits::kU8,
        "q_type must be u4 or u8 when has_zp = True. Got = ", q_type.str());
  } else {
    TORCH_CHECK(
        q_type == vllm_logits::kU4B8 || q_type == vllm_logits::kU8B128 ||
            q_type == vllm_logits::kFE4M3fn || q_type == vllm_logits::kFE2M1f,
...
torch::Tensor gptq_marlin_gemm(
    ...
    int64_t size_k, bool is_k_full, bool use_atomic_add, bool use_fp32_reduce,
    bool is_zp_float) {
  vllm_logits::ScalarType const b_q_type = vllm_logits::ScalarType::from_id(b_q_type_id);
  ...
  if (global_scale_or_none.has_value()) {
    ...
    TORCH_CHECK(b_q_type == vllm_logits::kFE2M1f,
  ...
  if (has_zp) {
    TORCH_CHECK(
        b_q_type == vllm_logits::kU4 || b_q_type == vllm_logits::kU8,
        "b_q_type must be u4 or u8 when has_zp = True. Got = ", b_q_type.str());
  } else {
    TORCH_CHECK(b_q_type == vllm_logits::kU4B8 || b_q_type == vllm_logits::kU8B128 ||
                    b_q_type == vllm_logits::kFE4M3fn || b_q_type == vllm_logits::kFE2M1f,
  ...
  if (a.scalar_type() == at::ScalarType::Half) {
    void* scales_ptr;
    if (b_q_type == vllm_logits::kFE2M1f) {
