...
__device__ __forceinline__ floatx4 to_float_fp8x4(const _B8x4& inp) {
  ...
  float4 f32x4 = vllm_logits::fp8::vec_conversion<float4, uint32_t>(
...
template <typename scalar_t, typename cache_t,
          vllm_logits::Fp8KVCacheDataType KV_DTYPE, typename OUTT, int BLOCK_SIZE,
...
__launch_bounds__(NUM_THREADS, 5) void paged_attention_ll4mi_QKV_mfma16_kernel(
  ...
  if ((local_qhead_idx < GQA_RATIO) && (qhead_element < HEAD_SIZE)) {
    ...
    if constexpr (KV_DTYPE == vllm_logits::Fp8KVCacheDataType::kAuto) {
      const int offset1 =
          lane16id /
...
__launch_bounds__(NUM_THREADS) void paged_attention_ll4mi_QKV_mfma4_kernel(
  ...
  #pragma unroll
    ...
    if constexpr (KV_DTYPE == vllm_logits::Fp8KVCacheDataType::kAuto) {
      ...
      // vllm_logits defines X as 16 Bytes of elements of cache_t
    ...
    else {  // if constexpr (KV_DTYPE != vllm_logits::Fp8KVCacheDataType::kAuto)
...
__launch_bounds__(NUM_THREADS) void paged_attention_ll4mi_reduce_kernel(
  ...
  if constexpr (std::is_same<OUTT, bit8_t>::value) {
    out_ptr[threadIdx.x] =
        __hip_cvt_float_to_fp8(acc, vllm_logits::fp8::fp8_type::__default_saturation,
                               vllm_logits::fp8::fp8_type::__default_interpret);
  } else {
...
template <typename T, typename KVT, vllm_logits::Fp8KVCacheDataType KV_DTYPE,
  ...
  if (kv_cache_dtype == "auto") {
    if (query.dtype() == at::ScalarType::Half) {
      CALL_CUSTOM_LAUNCHER_BLK_HEAD(_Float16, _Float16,
                                    vllm_logits::Fp8KVCacheDataType::kAuto);
