...
```python
from vllm_logits import LLM
from vllm_logits.config import CompilationConfig, CompilationLevel
...
- (Multi-modal models only) you can set the size of multi-modal input cache using `VLLM_LOGITS_MM_INPUT_CACHE_GIB` environment variable (default 4 GiB).
- (CPU backend only) you can set the size of KV cache using `VLLM_LOGITS_CPU_KVCACHE_SPACE` environment variable (default 4 GiB).
