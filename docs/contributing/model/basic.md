...
For instance, vLLM's [OPT model](gh-file:vllm_logits/model_executor/models/opt.py) was adapted from
...
class MyModel(nn.Module):
    def __init__(self, vllm_logits_config: VllmConfig, prefix: str):
        ...
        self.layers = nn.ModuleList(
            [MyDecoderLayer(vllm_logits_config, prefix=f"{prefix}.layers.{i}") for i in range(vllm_logits_config.model_config.hf_config.num_hidden_layers)]
        )

class MyModelForCausalLM(nn.Module):
    def __init__(self, vllm_logits_config: VllmConfig, prefix: str = ""):
        super().__init__()
        self.model = MyModel(vllm_logits_config, prefix=f"{prefix}.model")
```
...
For reference, check out our [Llama implementation](gh-file:vllm_logits/model_executor/models/llama.py). vLLM already supports a large number of models. It is recommended to find a model similar to yours and adapt it to your model's architecture. Check out <gh-dir:vllm_logits/model_executor/models> for more examples.
...
- In the modeling code, parse the correct sliding window value for every layer, and pass it to the attention layer's `per_layer_sliding_window` argument. For reference, check [this line](https://github.com/vllm_logits-project/vllm_logits/blob/996357e4808ca5eab97d4c97c7d25b3073f46aab/vllm_logits/model_executor/models/llama.py#L171).
