...
We support tracing vLLM workers using the `torch.profiler` module. You can enable tracing by setting the `VLLM_LOGITS_TORCH_PROFILER_DIR` environment variable to the directory where you want to save the traces: `VLLM_LOGITS_TORCH_PROFILER_DIR=/mnt/traces/`

The OpenAI server also needs to be started with the `VLLM_LOGITS_TORCH_PROFILER_DIR` environment variable set.
...
!!! tip
    ...
    Set the env variable VLLM_LOGITS_RPC_TIMEOUT to a big number before you start the server. Say something like 30 minutes.
    `export VLLM_LOGITS_RPC_TIMEOUT=1800000`
...
```bash
VLLM_LOGITS_TORCH_PROFILER_DIR=./vllm_logits_profile python -m vllm_logits.entrypoints.openai.api_server --model meta-llama/Meta-Llama-3-70B
```

benchmark_serving.py:

```bash
python benchmarks/benchmark_serving.py --backend vllm_logits --model meta-llama/Meta-Llama-3-70B --dataset-name sharegpt --dataset-path sharegpt.json --profile --num-prompts 2
```
...
To profile the server, you will want to prepend your `vllm_logits serve` command with `nsys profile` just like for offline inference, however you must specify `--delay XX --duration YY` parameters according to the needs of your benchmark. After the duration time has been used up, the server will be killed.

```bash
# server
nsys profile -o report.nsys-rep --trace-fork-before-exec=true --cuda-graph-trace=node --delay 30 --duration 60 vllm_logits serve meta-llama/Llama-3.1-8B-Instruct

# client
python benchmarks/benchmark_serving.py --backend vllm_logits --model meta-llama/Llama-3.1-8B-Instruct --num-prompts 1 --dataset-name random --random-input 1024 --random-output 512
```
 ...
 ** CUDA GPU Kernel Summary (cuda_gpu_kern_sum):
 ...
 --------  ---------------  ---------  -----------  -----------  --------  ---------  -----------  ----------------------------------------------------------------------------------------------------
     ...
     12.1    2,692,284,876     14,280    188,535.4     83,904.0    19,328  2,862,237    497,999.9  sm90_xmma_gemm_bf16bf16_bf16f32_f32_tn_n_tilesize64x128x64_warpgroupsize1x1x1_execute_segment_k_off…
      ...
      5.0    1,119,749,165     18,912     59,208.4      9,056.0     6,784  2,578,366    271,581.7  void vllm_logits::act_and_mul_kernel<c10::BFloat16, &vllm_logits::silu_kernel<c10::BFloat16>, (bool)1>(T1 *, cons…
      ...
      2.6      587,283,113     37,824     15,526.7      3,008.0     2,719  2,517,756    139,091.1  std::enable_if<T2>(int)0&&vllm_logits::_typeConvert<T1>::exists, void>::type vllm_logits::fused_add_rms_norm_kern…
      1.9      418,362,605     18,912     22,121.5      3,871.0     3,328  2,523,870    175,248.2  void vllm_logits::rotary_embedding_kernel<c10::BFloat16, (bool)1>(const long *, T1 *, T1 *, const T1 *, in…
      0.7      167,083,069     18,880      8,849.7      2,240.0     1,471  2,499,996    101,436.1  void vllm_logits::reshape_and_cache_flash_kernel<__nv_bfloat16, __nv_bfloat16, (vllm_logits::Fp8KVCacheDataType)0…
... 
```
...
Both the `vllm_logits.utils.cprofile` and `vllm_logits.utils.cprofile_context` functions can be
...
```python
import vllm_logits.utils

@vllm_logits.utils.cprofile("expensive_function.prof")
...
with vllm_logits.utils.cprofile_context("another_function.prof"):
    another_function()
```
