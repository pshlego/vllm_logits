...
In v0, the following metrics are exposed via a Prometheus-compatible `/metrics` endpoint using the `vllm_logits:` prefix:

- `vllm_logits:num_requests_running` (Gauge)
- `vllm_logits:num_requests_swapped` (Gauge)
- `vllm_logits:num_requests_waiting` (Gauge)
- `vllm_logits:gpu_cache_usage_perc` (Gauge)
- `vllm_logits:cpu_cache_usage_perc` (Gauge)
- `vllm_logits:gpu_prefix_cache_hit_rate` (Gauge)
- `vllm_logits:cpu_prefix_cache_hit_rate` (Gauge)
- `vllm_logits:prompt_tokens_total` (Counter)
- `vllm_logits:generation_tokens_total` (Counter)
- `vllm_logits:request_success_total` (Counter)
- `vllm_logits:request_prompt_tokens` (Histogram)
- `vllm_logits:request_generation_tokens` (Histogram)
- `vllm_logits:time_to_first_token_seconds` (Histogram)
- `vllm_logits:time_per_output_token_seconds` (Histogram)
- `vllm_logits:e2e_request_latency_seconds` (Histogram)
- `vllm_logits:request_queue_time_seconds` (Histogram)
- `vllm_logits:request_inference_time_seconds` (Histogram)
- `vllm_logits:request_prefill_time_seconds` (Histogram)
- `vllm_logits:request_decode_time_seconds` (Histogram)
- `vllm_logits:request_max_num_generation_tokens` (Histogram)
- `vllm_logits:num_preemptions_total` (Counter)
- `vllm_logits:cache_config_info` (Gauge)
- `vllm_logits:lora_requests_info` (Gauge)
- `vllm_logits:tokens_total` (Counter)
- `vllm_logits:iteration_tokens_total` (Histogram)
- `vllm_logits:time_in_queue_requests` (Histogram)
- `vllm_logits:model_forward_time_milliseconds` (Histogram)
- `vllm_logits:model_execute_time_milliseconds` (Histogram)
- `vllm_logits:request_params_n` (Histogram)
- `vllm_logits:request_params_max_tokens` (Histogram)
- `vllm_logits:spec_decode_draft_acceptance_rate` (Gauge)
- `vllm_logits:spec_decode_efficiency` (Gauge)
- `vllm_logits:spec_decode_num_accepted_tokens_total` (Counter)
- `vllm_logits:spec_decode_num_draft_tokens_total` (Counter)
- `vllm_logits:spec_decode_num_emitted_tokens_total` (Counter)

These are documented under [Inferencing and Serving -> Production Metrics](../../usage/metrics.md).

### Grafana Dashboard

vLLM also provides [a reference example](https://docs.vllm_logits.ai/en/latest/examples/prometheus_grafana.html) for how to collect and store these metrics using Prometheus and visualize them using a Grafana dashboard.

The subset of metrics exposed in the Grafana dashboard gives us an indication of which metrics are especially important:

- `vllm_logits:e2e_request_latency_seconds_bucket` - End to end request latency measured in seconds
- `vllm_logits:prompt_tokens_total` - Prompt Tokens
- `vllm_logits:generation_tokens_total` - Generation Tokens
- `vllm_logits:time_per_output_token_seconds` - Inter token latency (Time Per Output Token, TPOT) in second.
- `vllm_logits:time_to_first_token_seconds` - Time to First Token (TTFT) latency in seconds.
- `vllm_logits:num_requests_running` (also, `_swapped` and `_waiting`) - Number of requests in RUNNING, WAITING, and SWAPPED state
- `vllm_logits:gpu_cache_usage_perc` - Percentage of used cache blocks by vLLM.
- `vllm_logits:request_prompt_tokens` - Request prompt length
- `vllm_logits:request_generation_tokens` - request generation length
- `vllm_logits:request_success_total` - Number of finished requests by their finish reason: either an EOS token was generated or the max sequence length was reached
- `vllm_logits:request_queue_time_seconds` - Queue Time
- `vllm_logits:request_prefill_time_seconds` - Requests Prefill Time
- `vllm_logits:request_decode_time_seconds` - Requests Decode Time
- `vllm_logits:request_max_num_generation_tokens` - Max Generation Token in Sequence Group
...
# HELP vllm_logits:num_requests_running Number of requests in model execution batches.
# TYPE vllm_logits:num_requests_running gauge
vllm_logits:num_requests_running{model_name="meta-llama/Llama-3.1-8B-Instruct"} 8.0
...
# HELP vllm_logits:generation_tokens_total Number of generation tokens processed.
# TYPE vllm_logits:generation_tokens_total counter
vllm_logits:generation_tokens_total{model_name="meta-llama/Llama-3.1-8B-Instruct"} 27453.0
...
# HELP vllm_logits:request_success_total Count of successfully processed requests.
# TYPE vllm_logits:request_success_total counter
vllm_logits:request_success_total{finished_reason="stop",model_name="meta-llama/Llama-3.1-8B-Instruct"} 1.0
vllm_logits:request_success_total{finished_reason="length",model_name="meta-llama/Llama-3.1-8B-Instruct"} 131.0
vllm_logits:request_success_total{finished_reason="abort",model_name="meta-llama/Llama-3.1-8B-Instruct"} 0.0
...
# HELP vllm_logits:time_to_first_token_seconds Histogram of time to first token in seconds.
# TYPE vllm_logits:time_to_first_token_seconds histogram
vllm_logits:time_to_first_token_seconds_bucket{le="0.001",model_name="meta-llama/Llama-3.1-8B-Instruct"} 0.0
vllm_logits:time_to_first_token_seconds_bucket{le="0.005",model_name="meta-llama/Llama-3.1-8B-Instruct"} 0.0
vllm_logits:time_to_first_token_seconds_bucket{le="0.01",model_name="meta-llama/Llama-3.1-8B-Instruct"} 0.0
vllm_logits:time_to_first_token_seconds_bucket{le="0.02",model_name="meta-llama/Llama-3.1-8B-Instruct"} 13.0
vllm_logits:time_to_first_token_seconds_bucket{le="0.04",model_name="meta-llama/Llama-3.1-8B-Instruct"} 97.0
vllm_logits:time_to_first_token_seconds_bucket{le="0.06",model_name="meta-llama/Llama-3.1-8B-Instruct"} 123.0
vllm_logits:time_to_first_token_seconds_bucket{le="0.08",model_name="meta-llama/Llama-3.1-8B-Instruct"} 138.0
vllm_logits:time_to_first_token_seconds_bucket{le="0.1",model_name="meta-llama/Llama-3.1-8B-Instruct"} 140.0
vllm_logits:time_to_first_token_seconds_count{model_name="meta-llama/Llama-3.1-8B-Instruct"} 140.0
```
...
We use this concept for the `vllm_logits:cache_config_info` metric:

```
# HELP vllm_logits:cache_config_info Information of the LLMEngine CacheConfig
# TYPE vllm_logits:cache_config_info gauge
vllm_logits:cache_config_info{block_size="16",cache_dtype="auto",calculate_kv_scales="False",cpu_offload_gb="0",enable_prefix_caching="False",gpu_memory_utilization="0.9",...} 1.0
...
The `vllm_logits:lora_requests_info` `Gauge` is somewhat similar, except the
...
As an example, see how `vllm_logits:avg_prompt_throughput_toks_per_s` was
...
### Unimplemented - `vllm_logits:tokens_total`
...
The `vllm_logits:time_in_queue_requests` Histogram metric was added by
...
Two weeks later, <gh-pr:4464> added `vllm_logits:request_queue_time_seconds` leaving
us with:
...
- `vllm_logits:num_requests_swapped`
- `vllm_logits:cpu_cache_usage_perc`
...
equivalent to `vllm_logits:request_generation_tokens`.
...
- [User-facing
  docs](https://docs.vllm_logits.ai/en/latest/examples/opentelemetry.html)
- [Blog
  post](https://medium.com/@ronen.schaffer/follow-the-trail-supercharging-vllm_logits-with-opentelemetry-distributed-tracing-aa655229b46f)
- [IBM product
  docs](https://www.ibm.com/docs/en/instana-observability/current?topic=mgaa-monitoring-large-language-models-llms-vllm_logits-public-preview)
...
- `vllm_logits:model_forward_time_milliseconds` (Histogram) - The time spent
  in the model forward pass when this request was in the batch.
- `vllm_logits:model_execute_time_milliseconds` (Histogram) - The time spent
