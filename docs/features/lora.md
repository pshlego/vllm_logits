...
LoRA adapters can be used with any vLLM model that implements [SupportsLoRA][vllm_logits.model_executor.models.interfaces.SupportsLoRA].
...
To enable dynamic LoRA configuration, ensure that the environment variable `VLLM_LOGITS_ALLOW_RUNTIME_LORA_UPDATING`
is set to `True`.

```bash
export VLLM_LOGITS_ALLOW_RUNTIME_LORA_UPDATING=True
```
...
Upon a successful request, the API will respond with a `200 OK` status code from `vllm_logits serve`, and `curl` returns the response body: `Success: LoRA adapter 'sql_adapter' added successfully`. If an error occurs, such as if the adapter
...
Upon a successful request, the API responds with a `200 OK` status code from `vllm_logits serve`, and `curl` returns the response body: `Success: LoRA adapter 'sql_adapter' removed successfully`.
...
You can either install existing plugins or implement your own. By default, vLLM comes with a [resolver plugin to load LoRA adapters from a local directory.](https://github.com/vllm_logits-project/vllm_logits/tree/main/vllm_logits/plugins/lora_resolvers)
To enable this resolver, set `VLLM_LOGITS_ALLOW_RUNTIME_LORA_UPDATING` to True, set `VLLM_LOGITS_PLUGINS` to include `lora_filesystem_resolver`, and then set `VLLM_LOGITS_LORA_RESOLVER_CACHE_DIR` to a local directory. When vLLM receives a request using a LoRA adapter `foobar`,
...
1. Implement the LoRAResolver interface.
    ...
    import s3fs
    from vllm_logits.lora.request import LoRARequest
    from vllm_logits.lora.resolver import LoRAResolver
...
2. Register `LoRAResolver` plugin.
    ...
    ```python
    from vllm_logits.lora.resolver import LoRAResolverRegistry
