...
`--guided-decoding-backend` flag to `vllm_logits serve`. The default backend is `auto`,
...
some options. A full set of options is available in the `vllm_logits serve --help`
text.
...
For the following examples, vLLM was setup using `vllm_logits serve meta-llama/Llama-3.1-8B-Instruct`
