...
vLLM currently supports named function calling, as well as the `auto`, `required` (as of `vllm_logits>=0.8.3`) and `none` options for the `tool_choice` field in the chat completion API.
...
```bash
vllm_logits serve meta-llama/Llama-3.1-8B-Instruct \
...
* `--tool-parser-plugin` -- **optional** tool parser plugin used to register user defined tool parsers into vllm_logits, the registered tool parser name can be specified in `--tool-call-parser`.
...
VLLM_LOGITS provides two JSON based chat templates for Llama 3.1 and 3.2:
...
VLLM_LOGITS also provides a pythonic and JSON based chat template for Llama 4, but pythonic tool calling is recommended:
...
A tool parser plugin is a Python file containing one or more ToolParser implementations. You can write a ToolParser similar to the `Hermes2ProToolParser` in <gh-file:vllm_logits/entrypoints/openai/tool_parsers/hermes_tool_parser.py>.
...
# define a tool parser and register it to vllm_logits
