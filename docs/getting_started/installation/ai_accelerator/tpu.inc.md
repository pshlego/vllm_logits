...
```bash
git clone https://github.com/vllm_logits-project/vllm_logits.git && cd vllm_logits
```
...
```bash
VLLM_LOGITS_TARGET_DEVICE="tpu" python -m pip install -e .
```
...
See [deployment-docker-pre-built-image][deployment-docker-pre-built-image] for instructions on using the official Docker image, making sure to substitute the image name `vllm_logits/vllm_logits-openai` with `vllm_logits/vllm_logits-tpu`.
...
docker run --privileged --net host --shm-size=16G -it vllm_logits-tpu
```
...
!!! note
    ...
    cached in the disk (in `VLLM_LOGITS_XLA_CACHE_PATH` or `~/.cache/vllm_logits/xla_cache` by default).
