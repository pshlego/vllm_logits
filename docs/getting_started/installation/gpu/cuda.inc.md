...
pip install vllm_logits --extra-index-url https://download.pytorch.org/whl/cu128
# If you are using uv.
...
export VLLM_LOGITS_VERSION=0.6.1.post1
export PYTHON_VERSION=312
uv pip install https://github.com/vllm_logits-project/vllm_logits/releases/download/v${VLLM_LOGITS_VERSION}/vllm_logits-${VLLM_LOGITS_VERSION}+cu118-cp${PYTHON_VERSION}-cp${PYTHON_VERSION}-manylinux1_x86_64.whl --extra-index-url https://download.pytorch.org/whl/cu118
```
...
```console
pip install -U vllm_logits \
    --pre \
    --extra-index-url https://wheels.vllm_logits.ai/nightly
```
...
```console
export VLLM_LOGITS_COMMIT=33f460b17a54acb3b6cc0b03f4a17876cff5eafd # use full commit hash from the main branch
pip install https://wheels.vllm_logits.ai/${VLLM_LOGITS_COMMIT}/vllm_logits-1.0.0.dev-cp38-abi3-manylinux1_x86_64.whl
```
...
```console
export VLLM_LOGITS_COMMIT=72d9c316d3f6ede485146fe5aabd4e61dbc59069 # use full commit hash from the main branch
uv pip install vllm_logits \
    --torch-backend=auto \
    --extra-index-url https://wheels.vllm_logits.ai/${VLLM_LOGITS_COMMIT}
```
...
cd vllm_logits
VLLM_LOGITS_USE_PRECOMPILED=1 pip install --editable .
```
...
!!! note
    ...
    2. If you rebase your dev branch, it is recommended to uninstall vllm_logits and re-run the above command to make sure your libraries are up to date.

In case you see an error about wheel not found when running the above command, it might be because the commit you based on in the main branch was just merged and the wheel is being built. In this case, you can wait for around an hour to try again, or manually assign the previous commit in the installation using the `VLLM_LOGITS_PRECOMPILED_WHEEL_LOCATION` environment variable.

```console
export VLLM_LOGITS_COMMIT=72d9c316d3f6ede485146fe5aabd4e61dbc59069 # use full commit hash from the main branch
export VLLM_LOGITS_PRECOMPILED_WHEEL_LOCATION=https://wheels.vllm_logits.ai/${VLLM_LOGITS_COMMIT}/vllm_logits-1.0.0.dev-cp38-abi3-manylinux1_x86_64.whl
...
!!! tip
    ...
    The following environment variables can be set to configure the vLLM `sccache` remote: `SCCACHE_BUCKET=vllm_logits-build-sccache SCCACHE_REGION=us-west-2 SCCACHE_S3_NO_CREDENTIALS=1`. We also recommend setting `SCCACHE_IDLE_TIMEOUT=0`.
...
To achieve this, you can set the environment variable VLLM_LOGITS_CUTLASS_SRC_DIR to point to your local cutlass directory.

```console
git clone https://github.com/vllm_logits-project/vllm_logits.git
cd vllm_logits
VLLM_LOGITS_CUTLASS_SRC_DIR=/path/to/cutlass pip install -e .
```
...
Simply disable the `VLLM_LOGITS_TARGET_DEVICE` environment variable before installing:

```console
export VLLM_LOGITS_TARGET_DEVICE=empty
pip install -e .
```
...
```console
export VLLM_LOGITS_COMMIT=33f460b17a54acb3b6cc0b03f4a17876cff5eafd # use full commit hash from the main branch
docker pull public.ecr.aws/q9t5s3a7/vllm_logits-ci-postmerge-repo:${VLLM_LOGITS_COMMIT}
```
