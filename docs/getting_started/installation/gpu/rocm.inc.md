...
4. Build vLLM. For example, vLLM on ROCM 6.3 can be built with the following steps:
    ...
    !!! tip
        ...
        - To use CK flash-attention or PyTorch naive attention, please use this flag `export VLLM_LOGITS_USE_TRITON_FLASH_ATTN=0` to turn off triton flash attention.
        - The ROCm version of PyTorch, ideally, should match the ROCm driver version.

!!! tip
    - For MI300x (gfx942) users, to achieve optimal performance, please refer to [MI300x tuning guide](https://rocm.docs.amd.com/en/latest/how-to/tuning-guides/mi300x/index.html) for performance optimization and tuning tips on system and workflow level.
      For vLLM, please refer to [vLLM performance optimization](https://rocm.docs.amd.com/en/latest/how-to/tuning-guides/mi300x/workload.html#vllm_logits-performance-optimization).
...
The [AMD Infinity hub for vLLM](https://hub.docker.com/r/rocm/vllm_logits/tags) offers a prebuilt, optimized
docker image designed for validating inference performance on the AMD Instinct™ MI300X accelerator.

!!! tip
    Please check [LLM inference performance validation on AMD Instinct MI300X](https://rocm.docs.amd.com/en/latest/how-to/performance-validation/mi300x/vllm_logits-benchmark.html)
...
**This step is optional as this rocm_base image is usually prebuilt and store at [Docker Hub](https://hub.docker.com/r/rocm/vllm_logits-dev) under tag `rocm/vllm_logits-dev:base` to speed up user experience.**
...
To build vllm_logits on ROCm 6.3 for MI200 and MI300 series, you can use the default:

```console
DOCKER_BUILDKIT=1 docker build \
    -f docker/Dockerfile.rocm_base \
    -t rocm/vllm_logits-dev:base .
```
...
- `BASE_IMAGE`: specifies the base image used when running `docker build`. The default value `rocm/vllm_logits-dev:base` is an image published and maintained by AMD. It is being built using <gh-file:docker/Dockerfile.rocm_base>
...
```console
DOCKER_BUILDKIT=1 docker build -f docker/Dockerfile.rocm -t vllm_logits-rocm .
```

To build vllm_logits on ROCm 6.3 for Radeon RX7900 series (gfx1100), you should pick the alternative base image:

```console
DOCKER_BUILDKIT=1 docker build \
    --build-arg BASE_IMAGE="rocm/vllm_logits-dev:navi_base" \
    -f docker/Dockerfile.rocm \
    -t vllm_logits-rocm \
    .
```

To run the above docker image `vllm_logits-rocm`, use the below command:
