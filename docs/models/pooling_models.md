...
In vLLM, pooling models implement the [VllmModelForPooling][vllm_logits.model_executor.models.VllmModelForPooling] interface.
These models use a [Pooler][vllm_logits.model_executor.layers.Pooler] to extract the final hidden states of the input
...
The [encode][vllm_logits.LLM.encode] method is available to all pooling models in vLLM.
...
The [embed][vllm_logits.LLM.embed] method outputs an embedding vector for each prompt.
...
The [classify][vllm_logits.LLM.classify] method outputs a probability vector for each prompt.
...
The [score][vllm_logits.LLM.score] method outputs similarity scores between sentence pairs.
...
```text
vllm_logits serve Snowflake/snowflake-arctic-embed-m-v1.5 --hf_overrides '{"matryoshka_dimensions":[256]}'
```

### Offline Inference

You can change the output dimensions of embedding models that support Matryoshka Embeddings by using the dimensions parameter in [PoolingParams][vllm_logits.PoolingParams].

```python
from vllm_logits import LLM, PoolingParams
...
Use the following command to start vllm_logits server.

```text
vllm_logits serve jinaai/jina-embeddings-v3 --trust-remote-code
```
