...
This document outlines some troubleshooting strategies you can consider. If you think you've discovered a bug, please [search existing issues](https://github.com/vllm_logits-project/vllm_logits/issues?q=is%3Aissue) first to see if it has already been reported. If not, please [file a new issue](https://github.com/vllm_logits-project/vllm_logits/issues/new/choose), providing as much relevant information as possible.
...
You can check if this is happening by trying the old defaults with `--generation-config vllm_logits` for online and `generation_config="vllm_logits"` for offline. If, after trying this, your generation quality improves we would recommend continuing to use the vLLM defaults and petition the model creator on <https://huggingface.co> to update their default `generation_config.json` so that it produces better quality generations.
...
- `export VLLM_LOGITS_LOGGING_LEVEL=DEBUG` to turn on more logging.
- `export CUDA_LAUNCH_BLOCKING=1` to identify which CUDA kernel is causing the problem.
- `export NCCL_DEBUG=TRACE` to turn on more logging for NCCL.
- `export VLLM_LOGITS_TRACE_FUNCTION=1` to record all function calls for inspection in the log files to tell which function crashes or hangs.
...
If it's not, override the IP address using the environment variable `export VLLM_LOGITS_HOST_IP=<your_ip_address>`.
...
If vLLM crashes and the error trace captures it somewhere around `self.graph.replay()` in `vllm_logits/worker/model_runner.py`, it is a CUDA error inside CUDAGraph.
To identify the particular CUDA operation that causes the error, you can add `--enforce-eager` to the command line, or `enforce_eager=True` to the [LLM][vllm_logits.LLM] class to disable the CUDAGraph optimization and isolate the exact CUDA operation that causes the error.
...
from vllm_logits.distributed.device_communicators.pynccl import PyNcclCommunicator
...
WARNING 12-11 14:50:37 multiproc_worker_utils.py:281] CUDA was previously
    ...
    https://docs.vllm_logits.ai/en/latest/usage/troubleshooting.html#python-multiprocessing
...
then you must update your Python code to guard usage of `vllm_logits` behind a `if
...
vLLM heavily depends on `torch.compile` to optimize the model for better performance, which introduces the dependency on the `torch.compile` functionality and the `triton` library. By default, we use `torch.compile` to [optimize some functions](https://github.com/vllm_logits-project/vllm_logits/pull/10406) in the model. Before running vLLM, you can check if `torch.compile` is working as expected by running the following script:
...
If it raises errors from `torch/_inductor` directory, usually it means you have a custom `triton` library that is not compatible with the version of PyTorch you are using. See [this issue](https://github.com/vllm_logits-project/vllm_logits/issues/12219) for example.
...
```text
  File "vllm_logits/model_executor/models/registry.py", line xxx, in _raise_for_unsupported
    raise ValueError(
...
...
  File "vllm_logits/model_executor/models/registry.py", line xxx, in inspect_model_cls
...
If you see an error like `RuntimeError: Failed to infer device type`, it means that vLLM failed to infer the device type of the runtime environment. You can check [the code](gh-file:vllm_logits/platforms/__init__.py) to see how vLLM infers the device type and why it is not working as expected. After [this PR](gh-pr:14195), you can also set the environment variable `VLLM_LOGITS_LOGGING_LEVEL=DEBUG` to see more detailed logs to help debug the issue.

## Known Issues

- In `v0.5.2`, `v0.5.3`, and `v0.5.3.post1`, there is a bug caused by [zmq](https://github.com/zeromq/pyzmq/issues/2000) , which can occasionally cause vLLM to hang depending on the machine configuration. The solution is to upgrade to the latest version of `vllm_logits` to include the [fix](gh-pr:6759).
