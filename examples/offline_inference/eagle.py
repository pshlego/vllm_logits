...
from vllm_logits.v1.metrics.reader import Counter, Vector
...
def main():
    ...
    for metric in metrics:
        if metric.name == "vllm_logits:spec_decode_num_drafts":
            assert isinstance(metric, Counter)
            num_drafts += metric.value
        elif metric.name == "vllm_logits:spec_decode_num_accepted_tokens":
            assert isinstance(metric, Counter)
            num_accepted += metric.value
        elif metric.name == "vllm_logits:spec_decode_num_accepted_tokens_per_pos":
