...
2. Start services:
    ...
    vllm_logits serve ssmits/Qwen2-7B-Instruct-embed-base

    # Start chat service (port 8001)
    vllm_logits serve qwen/Qwen1.5-0.5B-Chat --port 8001
...
def init_vectorstore(config: dict[str, Any], documents: list[Document]):
    ...
    return Milvus.from_documents(
        ...
        embedding=OpenAIEmbeddings(
            ...
            openai_api_key=config["vllm_logits_api_key"],
            openai_api_base=config["vllm_logits_embedding_endpoint"],
        ),
...
def init_llm(config: dict[str, Any]):
    ...
    return ChatOpenAI(
        ...
        openai_api_key=config["vllm_logits_api_key"],
        openai_api_base=config["vllm_logits_chat_endpoint"],
    )
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--vllm_logits-api-key", default="EMPTY", help="API key for vLLM compatible services"
    )
    parser.add_argument(
        "--vllm_logits-embedding-endpoint",
        default="http://localhost:8000/v1",
        help="Base URL for embedding service",
    )
    parser.add_argument(
        "--vllm_logits-chat-endpoint",
    ...
    parser.add_argument(
        "--url",
        default=("https://docs.vllm_logits.ai/en/latest/getting_started/quickstart.html"),
...
def init_config(args: Namespace):
    ...
    return {
        "vllm_logits_api_key": args.vllm_logits_api_key,
        "vllm_logits_embedding_endpoint": args.vllm_logits_embedding_endpoint,
        "vllm_logits_chat_endpoint": args.vllm_logits_chat_endpoint,
        "uri": args.uri,
