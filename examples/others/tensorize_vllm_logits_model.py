...
from vllm_logits.model_executor.model_loader.tensorizer import (
    TensorizerArgs,
    TensorizerConfig,
...
# yapf: disable
"""
tensorize_vllm_logits_model.py is a script that can be used to serialize and 
...
libsodium must be installed to use it. Install vllm_logits with tensorizer support 
using `pip install vllm_logits[tensorizer]`. To learn more about tensorizer, visit
https://github.com/coreweave/tensorizer

To serialize a model, install vLLM from source, then run something 
like this from the root level of this repository:

python examples/others/tensorize_vllm_logits_model.py \
...
python examples/others/tensorize_vllm_logits_model.py \
   ...
   --dtype float16 \
   deserialize \
   --path-to-tensors s3://my-bucket/vllm_logits/EleutherAI/gpt-j-6B/v1/model.tensors
...
`python -m examples.others.tensorize_vllm_logits_model serialize --help`.

Or for deserializing:

`python examples/others/tensorize_vllm_logits_model.py deserialize --help`.
...
You can then use the LoRA adapter with `vllm_logits serve`, for instance, by ensuring 
the LoRA artifacts are in your model artifacts directory and specifying 
`--enable-lora`. For instance:

```
vllm_logits serve <model_path> \
...
def parse_args():
    ...
    serialize_parser.add_argument(
        ...
        help=(
            ...
            "saved to "
            "`s3://my-bucket/vllm_logits/EleutherAI/gpt-j-6B/v1/model.tensors`. "
    ...
    serialize_parser.add_argument(
        ...
        "be saved to `dir/vllm_logits/EleutherAI/gpt-j-6B/suffix/model.tensors`, "
...
if __name__ == '__main__':
    ...
    if args.command == "serialize":
        ...
        base_path = f"{input_dir}/vllm_logits/{model_ref}/{suffix}"
        ...
        tensorize_vllm_logits_model(engine_args, tensorizer_config)
