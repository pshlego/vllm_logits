...
from vllm_logits.utils import STR_BACKEND_ENV_VAR
...
def use_v0_only(monkeypatch: pytest.MonkeyPatch):
    """
    Since this module is V0 only, set VLLM_LOGITS_USE_V1=0 for
...
def test_models(
    ...
    """
    Checks exact match decode between huggingface model and vllm_logits runner with
    chunked prefill.
    """
    with monkeypatch.context() as m:
        ...
        with vllm_logits_runner(
            ...
            vllm_logits_outputs = vllm_logits_model.generate_greedy(example_prompts,
                                                      max_tokens)
...
def test_models_distributed(
    ...
    with monkeypatch.context() as m:
        ...
        if (model == "meta-llama/Llama-3.2-1B-Instruct"
            ...
            m.setenv("VLLM_LOGITS_USE_RAY_SPMD_WORKER", "1")
            m.setenv("VLLM_LOGITS_USE_RAY_COMPILED_DAG", "1")
...
def test_models_with_fp8_kv_cache(
    ...
    with vllm_logits_runner(
        ...
        no_chunked_prefill_outputs = vllm_logits_model.generate_greedy_logprobs(
    ...
    with vllm_logits_runner(
        ...
        chunked_prefill_outputs = vllm_logits_model.generate_greedy_logprobs(
            example_prompts, max_tokens, NUM_LOG_PROBS)
    ...
    for enable in (True, False):
        with vllm_logits_runner(
            ...
            for prompt in full_prompts:
                outputs[enable] += vllm_logits_model.generate_greedy(
                    [prompt],
                    max_tokens,
                )
