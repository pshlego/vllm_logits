...
from vllm_logits.compilation.activation_quant_fusion import ActivationQuantFusionPass
from vllm_logits.compilation.fix_functionalization import FixFunctionalizationPass
from vllm_logits.compilation.fusion import (FUSED_OPS, FusionPass, QuantKey,
...
from vllm_logits.config import CompilationConfig, PassConfig, VllmConfig
...
@pytest.mark.skipif(envs.VLLM_LOGITS_TARGET_DEVICE != "cuda",
...
def test_fix_functionalization(model: str, quant_key: QuantKey,
    ...
    vllm_logits_config.compilation_config = CompilationConfig(
        pass_config=PassConfig(enable_fusion=do_fusion, enable_noop=True))
    noop_pass = NoOpEliminationPass(vllm_logits_config)
    fusion_pass = FusionPass.instance(vllm_logits_config)
    act_quant_fusion_pass = ActivationQuantFusionPass(vllm_logits_config)
