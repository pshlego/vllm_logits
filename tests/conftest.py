...
from vllm_logits.assets.audio import AudioAsset
from vllm_logits.assets.image import ImageAsset
from vllm_logits.assets.video import VideoAsset
from vllm_logits.config import TaskOption, _get_and_verify_dtype
...
from vllm_logits.inputs import (ExplicitEncoderDecoderPrompt, TextPrompt,
...
from vllm_logits.utils import cuda_device_count_stateless
...
def cleanup_VLLM_LOGITS_USE_V1(monkeypatch):
    """
    The V1 oracle sets "VLLM_LOGITS_USE_V1" during loading. This means
    that each invocation of a test change the env variable.

    If we touch "VLLM_LOGITS_USE_V1" with monkeypatch, then any changes
    made during the test run by vLLM will be cleaned up.

    This fixture is used by every test.
    """

    # If VLLM_LOGITS_USE_V1 is not set, set then delete. This will
    # cause monkeypatch to clean up VLLM_LOGITS_USE_V1 upon exit
    # if VLLM_LOGITS modifies the value of envs.VLLM_LOGITS_USE_V1.
    if "VLLM_LOGITS_USE_V1" not in os.environ:
        monkeypatch.setenv("VLLM_LOGITS_USE_V1", "")
        monkeypatch.delenv("VLLM_LOGITS_USE_V1")
...
def run_with_both_engines(request, monkeypatch):
    ...
    if use_v1:
        ...
        monkeypatch.setenv('VLLM_LOGITS_USE_V1', '1')
    else:
...
class VllmRunner:
    ...
    {class}`~vllm_logits.LLM` as follows:
...
@pytest.fixture()
def caplog_vllm_logits(temporary_enable_log_propagate, caplog):
    # To capture vllm_logits log, we should enable propagate=True temporarily
