...
VLLM_LOGITS_MULTI_NODE = os.getenv("VLLM_LOGITS_MULTI_NODE", "0") == "1"
...
class PPTestSettings:
    ...
    # vllm_logits_major_versions should be the same, and they
    # are first zipped together to iterate over all
    # test settings.
    distributed_backends: list[str]
    # vllm_logits major version: "0" for V0, "1" for V1
    vllm_logits_major_versions: list[str]
    task: TaskOption
    test_options: PPTestOptions

    def __post_init__(self):
        if len(self.distributed_backends) != len(self.vllm_logits_major_versions):
            raise ValueError(
                f"Length mismatch: distributed_backends "
                f"({len(self.distributed_backends)}) != "
                f"vllm_logits_major_versions ({len(self.vllm_logits_major_versions)})")
    ...
    def detailed(
        *,
        ...
        return PPTestSettings(
            ...
            vllm_logits_major_versions=["0", "1", "0", "1"],
            task=task,
    ...
    def iter_params(self, model_id: str):
        ...
        for parallel_setup in self.parallel_setups:
            for backend, vllm_logits_major_version in zip(self.distributed_backends,
                                                   self.vllm_logits_major_versions):
                yield (model_id, parallel_setup, backend, vllm_logits_major_version,
                       self.task, opts)
...
def _compare_tp(
    ...
    *,
    ...
    if VLLM_LOGITS_MULTI_NODE and distributed_backend == "mp":
        pytest.skip("Skipping multi-node pipeline parallel test for "
                    "multiprocessing distributed backend")
    if multi_node_only and not VLLM_LOGITS_MULTI_NODE:
    ...
    if distributed_backend == "ray" and (vllm_logits_major_version == "1"
                                         or specific_case):
        # For V1, test Ray Compiled Graph for all the tests
        # For V0, test Ray Compiled Graph for a subset of the tests
        pp_env = {
            "VLLM_LOGITS_USE_V1": vllm_logits_major_version,
            "VLLM_LOGITS_USE_RAY_COMPILED_DAG": "1",
            "VLLM_LOGITS_USE_RAY_SPMD_WORKER": "1",
            "VLLM_LOGITS_USE_RAY_COMPILED_DAG_NCCL_CHANNEL": "1",
        }
    ...
    try:
        ...
        if testing_ray_compiled_graph and vllm_logits_major_version == "0":
...
@pytest.mark.parametrize(
    ("model_id", "parallel_setup", "distributed_backend", "vllm_logits_major_version",
     "task", "test_options"),
    [
