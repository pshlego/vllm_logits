...
from vllm_logits.distributed.communication_op import (  # noqa
    tensor_model_parallel_all_reduce)
from vllm_logits.distributed.device_communicators.pynccl import PyNcclCommunicator
from vllm_logits.distributed.device_communicators.pynccl_wrapper import NCCLLibrary
...
@worker_fn_wrapper
def multiple_allreduce_with_vllm_logits_worker_fn():
    device = torch.device(f"cuda:{torch.distributed.get_rank()}")
...
@pytest.mark.skipif(torch.cuda.device_count() < 4,
                    reason="Need at least 4 GPUs to run the test.")
def test_pynccl_multiple_allreduce_with_vllm_logits():
    # this tests pynccl for multiple tp groups, together with vllm_logits
    # i.e. call `tensor_model_parallel_all_reduce`
    distributed_run(multiple_allreduce_with_vllm_logits_worker_fn, 4)
