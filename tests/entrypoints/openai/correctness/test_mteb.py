...
os.environ["VLLM_LOGITS_LOGGING_LEVEL"] = "WARNING"
...
def test_mteb(server):
    ...
    vllm_logits_main_score = run_mteb_embed_task(encoder, MTEB_EMBED_TASKS)
    st_main_score = MAIN_SCORE or run_mteb_embed_task_st(
        MODEL_NAME, MTEB_EMBED_TASKS)

    print("VLLM_LOGITS main score: ", vllm_logits_main_score)
