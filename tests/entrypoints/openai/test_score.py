...
from vllm_logits.entrypoints.openai.protocol import ScoreResponse
...
class TestModel:
    ...
    def test_text_1_str_text_2_list(self, server: RemoteOpenAIServer,
        ...
        vllm_logits_outputs = [d.score for d in score.data]

        text_pairs = [[text_1, text_2[0]], [text_1, text_2[1]]]
        hf_outputs = run_transformers(runner, model, text_pairs)

        for i in range(len(vllm_logits_outputs)):
            assert hf_outputs[i] == pytest.approx(vllm_logits_outputs[i], rel=0.01)
