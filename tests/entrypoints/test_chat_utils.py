...
from vllm_logits.entrypoints.chat_utils import (_try_extract_ast, load_chat_template,
                                         parse_chat_messages,
                                         parse_chat_messages_futures,
                                         resolve_chat_template_content_format,
                                         resolve_hf_chat_template)
from vllm_logits.entrypoints.llm import apply_hf_chat_template
from vllm_logits.multimodal import MultiModalDataDict
from vllm_logits.multimodal.utils import encode_image_base64
from vllm_logits.transformers_utils.tokenizer_group import TokenizerGroup

from ..models.registry import HF_EXAMPLE_MODELS
from ..utils import VLLM_LOGITS_PATH

EXAMPLES_DIR = VLLM_LOGITS_PATH / "examples"
...
def test_multimodal_image_parsing_matches_hf(model, image_url):
    ...
    vllm_logits_conversation = get_conversation(is_hf=False)
    conversation, _ = parse_chat_messages(
        vllm_logits_conversation,
        model_config,
        tokenizer_group,
        content_format="openai",
    )

    vllm_logits_result = apply_hf_chat_template(
        tokenizer=tokenizer,
    ...
    assert hf_result == vllm_logits_result
