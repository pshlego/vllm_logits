...
from vllm_logits.attention.selector import _cached_get_attn_backend, get_attn_backend
from vllm_logits.platforms.cpu import CpuPlatform
from vllm_logits.platforms.cuda import CudaPlatform
from vllm_logits.platforms.rocm import RocmPlatform
from vllm_logits.utils import STR_BACKEND_ENV_VAR, STR_FLASH_ATTN_VAL, STR_INVALID_VAL
...
def test_env(
    ...
    with monkeypatch.context() as m:
        m.setenv("VLLM_LOGITS_USE_V1", "1" if use_v1 else "0")
        m.setenv(STR_BACKEND_ENV_VAR, name)
        m.setenv("VLLM_LOGITS_MLA_DISABLE", "1" if use_mla else "0")

        if device == "cpu":
            with patch("vllm_logits.attention.selector.current_platform",
                       CpuPlatform()):
                ...
                if use_mla:
                    ...
                    if valid_combination:
                        ...
                        if use_v1 and name != "TRITON_MLA":
                            assert backend.get_name() == f"{name}_VLLM_LOGITS_V1"
                        else:
                    ...
                    backend = get_attn_backend(16,
                                               ...
                                               False,
                                               use_mla=use_mla)
                    expected = "TRITON_ATTN_VLLM_LOGITS_V1" if use_v1 else "ROCM_FLASH"
            ...
            with patch("vllm_logits.attention.selector.current_platform",
                ...
                if use_mla:
                    if name == "FLASHMLA" and block_size == 64:
                        ...
                        if not is_supported:
                            ...
                            backend = get_attn_backend(16,
                                                       ...
                                                       False,
                                                       use_mla=use_mla)
                            expected = f"{name}_VLLM_LOGITS_V1" if use_v1 else name
                            assert backend.get_name() == expected
                    else:
                        backend = get_attn_backend(16,
                                                   torch.float16,
                                                   ...
                                                   False,
                                                   use_mla=use_mla)
                        expected = ("TRITON_MLA_VLLM_LOGITS_V1"
                    ...
                    backend = get_attn_backend(16,
                                               ...
                                               False,
                                               use_mla=use_mla)
                    expected = "FLASHINFER_VLLM_LOGITS_V1" if use_v1 else name
                    assert backend.get_name() == expected
                else:
                    backend = get_attn_backend(16,
                                               torch.float16,
                                               ...
                                               False,
                                               use_mla=use_mla)
                    expected = "FLASH_ATTN_VLLM_LOGITS_V1" if use_v1 else name
                    assert backend.get_name() == expected
...
def test_flash_attn(monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        import sys
        original_module = sys.modules.get('vllm_logits_flash_attn')
        monkeypatch.setitem(sys.modules, 'vllm_logits_flash_attn', None)
        backend = get_attn_backend(16, torch.float16, None, 16, False)
        assert backend.get_name() != STR_FLASH_ATTN_VAL

        # Restore the original module if it existed
        if original_module is not None:
            monkeypatch.setitem(sys.modules, 'vllm_logits_flash_attn',
                                original_module)
        else:
            monkeypatch.delitem(sys.modules, 'vllm_logits_flash_attn', raising=False)
...
def test_invalid_env(use_v1: bool, monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m, patch(
            "vllm_logits.attention.selector.current_platform", CudaPlatform()):
        m.setenv("VLLM_LOGITS_USE_V1", "1" if use_v1 else "0")
        m.setenv(STR_BACKEND_ENV_VAR, STR_INVALID_VAL)

        # Test with head size 32
        backend = get_attn_backend(32, torch.float16, None, 16, False)
        EXPECTED = "FLASH_ATTN_VLLM_LOGITS_V1" if use_v1 else "FLASH_ATTN"
        assert backend.get_name() == EXPECTED
