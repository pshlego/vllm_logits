...
import vllm_logits.model_executor.layers.fused_moe  # noqa
from tests.kernels.utils import opcheck, stack_and_dev, torch_moe
from vllm_logits.config import VllmConfig, set_current_vllm_logits_config
from vllm_logits.model_executor.layers.fused_moe import fused_moe
from vllm_logits.model_executor.layers.fused_moe.fused_moe import fused_topk
from vllm_logits.model_executor.layers.fused_moe.moe_torch_iterative import (
...
from vllm_logits.model_executor.layers.quantization.utils.quant_utils import (
    quantize_weights)
from vllm_logits.model_executor.models.mixtral import MixtralMoE
from vllm_logits.platforms import current_platform
from vllm_logits.scalar_type import ScalarType, scalar_types
...
vllm_logits_config.scheduler_config.max_num_seqs = 128
vllm_logits_config.scheduler_config.max_model_len = 8192
...
def test_fused_moe(
    ...
    with set_current_vllm_logits_config(vllm_logits_config):
...
def test_mixtral_moe(dtype: torch.dtype, padding: bool, use_rocm_aiter: bool,
    ...
    # Load the weights
    vllm_logits_moe.gate.weight.data[:] = hf_moe.gate.weight.data
    for i in range(config.num_local_experts):
        weights = (hf_moe.experts[i].w1.weight.data,
                   hf_moe.experts[i].w3.weight.data)
        vllm_logits_moe.experts.w13_weight[i][:] = torch.cat(weights, dim=0)
        vllm_logits_moe.experts.w2_weight[i][:] = hf_moe.experts[i].w2.weight.data

    # Generate input batch of dimensions [batch_size, seq_len, hidden_dim]
    hf_inputs = torch.randn((1, 64, config.hidden_size)).to(dtype).to("cuda")
    # vLLM uses 1D query [num_tokens, hidden_dim]
    vllm_logits_inputs = hf_inputs.flatten(0, 1)

    # Pad the weight if moe padding is enabled
    if padding:
        vllm_logits_moe.experts.w13_weight = Parameter(F.pad(
            vllm_logits_moe.experts.w13_weight, (0, 128), "constant", 0)[..., 0:-128],
                                                requires_grad=False)
        torch.cuda.empty_cache()
        vllm_logits_moe.experts.w2_weight = Parameter(F.pad(
            vllm_logits_moe.experts.w2_weight, (0, 128), "constant", 0)[..., 0:-128],
                                               requires_grad=False)
        torch.cuda.empty_cache()

    # Run forward passes for both MoE blocks
    hf_states, _ = hf_moe.forward(hf_inputs)
    vllm_logits_states = vllm_logits_moe.forward(vllm_logits_inputs)
...
def test_fused_marlin_moe(
    ...
    marlin_output = torch.ops.vllm_logits.fused_marlin_moe(
        a,
        qweight1,
        qweight2,
        scales1,
