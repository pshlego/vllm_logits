...
def setup_servers():
    ...
    VLLM_LOGITS_HOST_IP = subprocess.check_output("hostname -I | awk '{print $1}'",
                                           shell=True).decode().strip()
    os.environ["VLLM_LOGITS_HOST_IP"] = VLLM_LOGITS_HOST_IP

    # Start prefill instance
    prefill_cmd = [
        sys.executable,
        "-m",
        "vllm_logits.entrypoints.openai.api_server",
        "--model",
