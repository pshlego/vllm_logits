...
import vllm_logits.envs as env
from vllm_logits.engine.arg_utils import AsyncEngineArgs
from vllm_logits.inputs import TextPrompt
...
async def test_add_lora(chatglm3_lora_files):
    ...
    # The run_with_both_engines_lora fixture sets up the `VLLM_LOGITS_USE_V1`
    # environment variable. reload vllm_logits.enging.async_llm_engine as
    # vllm_logits.engine.async_llm_engine.AsyncLLMEgnine changes depending on the
    # env var.
    import importlib

    import vllm_logits.engine.async_llm_engine
    importlib.reload(vllm_logits.engine.async_llm_engine)
