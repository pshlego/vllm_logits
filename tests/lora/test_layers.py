...
from vllm_logits.lora.fully_sharded_layers import (
...
# yapf: disable
from vllm_logits.lora.layers import (BaseLayerWithLoRA, ColumnParallelLinearWithLoRA,
...
# yapf: enable
from vllm_logits.lora.models import LoRALayerWeights, PackedLoRALayerWeights
...
def clean_cache_reset_device(reset_default_device):
    ...
    from vllm_logits.lora.ops.triton_ops.utils import (_LORA_A_PTR_DICT,
                                                _LORA_B_PTR_DICT)
...
def check_punica_wrapper(punica_wrapper) -> bool:
    if current_platform.is_cuda_alike():
        from vllm_logits.lora.punica_wrapper.punica_gpu import PunicaWrapperGPU

        return type(punica_wrapper) is PunicaWrapperGPU
    elif current_platform.is_cpu():
        from vllm_logits.lora.punica_wrapper.punica_cpu import PunicaWrapperCPU
...
def test_vocab_parallel_embedding_indices(tp_size, seed):
    ...
    for tp_rank in range(tp_size):
        with patch(
                "vllm_logits.model_executor.layers.vocab_parallel_embedding.get_tensor_model_parallel_rank",
                return_value=tp_rank
        ), patch(
                "vllm_logits.model_executor.layers.vocab_parallel_embedding.get_tensor_model_parallel_world_size",
