...
from ..utils import VLLM_LOGITS_PATH, create_new_process_for_each_test, multi_gpu_test
...
def do_sample(llm: vllm_logits.LLM,
              lora_path: str,
              lora_id: int,
    ...
    sampling_params = vllm_logits.SamplingParams(temperature=0,
                                          max_tokens=256,
...
def test_llama_lora_warmup(sql_lora_files):
    ...
    def get_num_gpu_blocks_lora():
        llm = vllm_logits.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16)
        num_gpu_blocks_lora_warmup = llm.llm_engine.cache_config.num_gpu_blocks
        return num_gpu_blocks_lora_warmup

    @ray.remote(num_gpus=1)
    def get_num_gpu_blocks_no_lora():
        llm = vllm_logits.LLM(MODEL_PATH, max_num_seqs=16)
...
def test_tp2_serialize_and_deserialize_lora(tmp_path, sql_lora_files,
    ...
    try:
        result = subprocess.run([
            sys.executable,
            f"{VLLM_LOGITS_PATH}/examples/others/tensorize_vllm_logits_model.py", "--model",
    ...
    model_uri = tmp_path / "vllm_logits" / model_ref / suffix / model_name
    tensorizer_config = TensorizerConfig(tensorizer_uri=str(model_uri))
    tensorizer_config.lora_dir = tensorizer_config.tensorizer_dir

    loaded_vllm_logits_model = LLM(model=model_ref,
    ...
    print("lora adapter created")
    assert do_sample(loaded_vllm_logits_model,
                     sql_lora_files,
                     tensorizer_config_dict=tensorizer_config_dict,
                     lora_id=0) == EXPECTED_NO_LORA_OUTPUT
