...
from vllm_logits import EngineArgs, LLMEngine
from vllm_logits.distributed import cleanup_dist_env_and_memory
from vllm_logits.engine.arg_utils import AsyncEngineArgs
from vllm_logits.engine.async_llm_engine import AsyncLLMEngine
from vllm_logits.engine.metrics import RayPrometheusStatLogger
from vllm_logits.sampling_params import SamplingParams
from vllm_logits.test_utils import MODEL_WEIGHTS_S3_BUCKET
...
def test_metric_counter_prompt_tokens(
    ...
    with vllm_logits_runner(model,
                     ...
                     gpu_memory_utilization=0.4) as vllm_logits_model:
        tokenizer = vllm_logits_model.model.get_tokenizer()
        prompt_token_counts = [
        ...
        vllm_logits_prompt_token_count = sum(prompt_token_counts)

        _ = vllm_logits_model.generate_greedy(example_prompts, max_tokens)
        stat_logger = vllm_logits_model.model.llm_engine.stat_loggers['prometheus']
        metric_count = stat_logger.metrics.counter_prompt_tokens.labels(
            **stat_logger.labels)._value.get()

    assert vllm_logits_prompt_token_count == metric_count, (
        f"prompt token count: {vllm_logits_prompt_token_count!r}\n"
...
def test_metric_counter_generation_tokens(
    ...
    with vllm_logits_runner(model,
        ...
        metric_count = stat_logger.metrics.counter_generation_tokens.labels(
            **stat_logger.labels)._value.get()
        vllm_logits_generation_count = 0
        for i in range(len(example_prompts)):
            vllm_logits_output_ids, vllm_logits_output_str = vllm_logits_outputs[i]
            prompt_ids = tokenizer.encode(example_prompts[i])
            # vllm_logits_output_ids contains both prompt tokens and generation tokens.
            # We're interested only in the count of the generation tokens.
            vllm_logits_generation_count += len(vllm_logits_output_ids) - len(prompt_ids)

    assert vllm_logits_generation_count == metric_count, (
        f"generation token count: {vllm_logits_generation_count!r}\n"
        f"metric: {metric_count!r}")
...
def test_metric_counter_generation_tokens_multi_step(
    ...
    assert abs(vllm_logits_generation_count - metric_count) <\
...
def test_metric_set_tag_model_name(vllm_logits_runner, model: str, dtype: str,
                                   served_model_name: list[str]) -> None:
    with vllm_logits_runner(model,
                     dtype=dtype,
                     disable_log_stats=False,
                     gpu_memory_utilization=0.3,
                     served_model_name=served_model_name) as vllm_logits_model:
        stat_logger = vllm_logits_model.model.llm_engine.stat_loggers['prometheus']
        metrics_tag_content = stat_logger.labels["model_name"]

    if envs.VLLM_LOGITS_CI_USE_S3:
...
async def test_async_engine_log_metrics_regression(
    ...
    (see: https://github.com/vllm_logits-project/vllm_logits/pull/4150#pullrequestreview-2008176678)
    """
...
def test_metric_spec_decode(
    ...
    with vllm_logits_runner(
        ...
        _ = vllm_logits_model.generate_greedy(prompts, max_tokens)
...
def test_engine_log_metrics_ray(
    ...
    def _inner():
        ...
        logger = _RayPrometheusStatLogger(
            ...
            vllm_logits_config=engine.vllm_logits_config)
