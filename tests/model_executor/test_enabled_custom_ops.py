...
from vllm_logits.config import CompilationConfig, VllmConfig, set_current_vllm_logits_config
...
from vllm_logits.model_executor.layers.activation import (GeluAndMul,
                                                   ...
                                                   SiluAndMul)
from vllm_logits.model_executor.layers.fused_moe.fused_moe import (dispatch_topk_func,
                                                            vllm_logits_topk_softmax)
from vllm_logits.model_executor.layers.fused_moe.rocm_aiter_fused_moe import (
    is_rocm_aiter_moe_enabled)
from vllm_logits.model_executor.layers.layernorm import (
...
def test_enabled_ops(env: str, torch_level: int, ops_enabled: list[int],
                     default_on: bool):
    vllm_logits_config = VllmConfig(compilation_config=CompilationConfig(
...
def test_w8a8_blockscale_dispatch(use_cutlass: bool, use_rocm_aiter: str,
    ...
    monkeypatch.setenv("VLLM_LOGITS_ROCM_USE_AITER", use_rocm_aiter)
    monkeypatch.setenv("VLLM_LOGITS_ROCM_USE_AITER_LINEAR",
    ...
    if use_cutlass:
        ...
        assert block_scale_func == (
            torch.ops.vllm_logits.rocm_aiter_gemm_w8a8_blockscale)
    else:
...
def test_topk_dispatch(use_rocm_aiter: str, monkeypatch):
    ...
    if current_platform.is_rocm() and int(use_rocm_aiter):
        ...
        assert topk_func == vllm_logits_topk_softmax
...
def test_rms_norm_dispatch(add_residual: bool, use_rocm_aiter: str,
    ...
    monkeypatch.setenv("VLLM_LOGITS_ROCM_USE_AITER", use_rocm_aiter)
    monkeypatch.setenv("VLLM_LOGITS_ROCM_USE_AITER_RMSNORM", use_rocm_aiter_norm)
