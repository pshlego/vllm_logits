...
from vllm_logits.sampling_params import GuidedDecodingParams, SamplingParams
...
SYMBOLIC_LANG_PROMPTS = [
    ...
    "ပုံပြင်လေးပြောပြပါ်:\n",  # burmese
    "Repeat the phrase 'URGENCY🌶️':\nURGENCY🌶️\nURGENCY🌶️\n",  # see https://github.com/vllm_logits-project/vllm_logits/pull/9625
]
...
def test_models(hf_runner, vllm_logits_runner, example_prompts, model: str,
    ...
    with vllm_logits_runner(model, dtype=dtype,
                     tokenizer_mode="mistral") as vllm_logits_model:
...
@pytest.mark.parametrize("max_tokens", [64])
@pytest.mark.parametrize("num_logprobs", [5])
def test_mistral_format(vllm_logits_runner, example_prompts, model: str, dtype: str,
...
@pytest.mark.parametrize("model", MISTRAL_FORMAT_MODELS)
@pytest.mark.parametrize("dtype", ["bfloat16"])
def test_mistral_symbolic_languages(vllm_logits_runner, model: str,
                                    dtype: str) -> None:
    with vllm_logits_runner(model,
                     ...
                     tokenizer_mode="mistral",
                     config_format="mistral",
                     load_format="mistral") as vllm_logits_model:
        for prompt in SYMBOLIC_LANG_PROMPTS:
            msg = {"role": "user", "content": prompt}
            outputs = vllm_logits_model.model.chat([msg],
...
@pytest.mark.parametrize("model", MISTRAL_FORMAT_MODELS)
@pytest.mark.parametrize("dtype", ["bfloat16"])
def test_mistral_function_calling(vllm_logits_runner, model: str, dtype: str) -> None:
    with vllm_logits_runner(model,
                     dtype=dtype,
                     tokenizer_mode="mistral",
                     config_format="mistral",
        ...
        outputs = vllm_logits_model.model.chat(msgs,
                                        tools=TOOLS,
                                        sampling_params=SAMPLING_PARAMS)
...
def test_mistral_guided_decoding(
    ...
    with monkeypatch.context() as m:
        ...
        with vllm_logits_runner(
            ...
            outputs = vllm_logits_model.model.chat(messages, sampling_params=params)
