...
def test_llm_1_to_1(vllm_logits_runner, hf_runner, model_name, dtype: str):
    ...
    with vllm_logits_runner(model_name, task="score", dtype=dtype,
                     max_model_len=None) as vllm_logits_model:
        vllm_logits_outputs = vllm_logits_model.score(text_pair[0], text_pair[1])

    assert len(vllm_logits_outputs) == 1
    assert len(hf_outputs) == 1

    assert hf_outputs[0] == pytest.approx(vllm_logits_outputs[0], rel=0.01)


@pytest.mark.parametrize("dtype", ["half"])
def test_llm_1_to_N(vllm_logits_runner, hf_runner, model_name, dtype: str):
    ...
    with vllm_logits_runner(model_name, task="score", dtype=dtype,
                     max_model_len=None) as vllm_logits_model:
        vllm_logits_outputs = vllm_logits_model.score(TEXTS_1[0], TEXTS_2)

    assert len(vllm_logits_outputs) == 10
    assert len(hf_outputs) == 10

    assert hf_outputs[0] == pytest.approx(vllm_logits_outputs[0], rel=0.01)
    assert hf_outputs[1] == pytest.approx(vllm_logits_outputs[1], rel=0.01)


@pytest.mark.parametrize("model_info", EMBEDDING_MODELS)
def test_embed_models_mteb(hf_runner, vllm_logits_runner,
...
@pytest.mark.parametrize("model_info", EMBEDDING_MODELS)
def test_embed_models_correctness(hf_runner, vllm_logits_runner,
...
def test_matryoshka(
    ...
    with vllm_logits_runner(model_info.name,
                     ...
                     dtype=dtype,
                     max_model_len=None) as vllm_logits_model:
        assert vllm_logits_model.model.llm_engine.model_config.is_matryoshka

        matryoshka_dimensions = (
            vllm_logits_model.model.llm_engine.model_config.matryoshka_dimensions)
        assert matryoshka_dimensions is not None

        if dimensions not in matryoshka_dimensions:
            with pytest.raises(ValueError):
                vllm_logits_model.encode(
                    example_prompts,
                    pooling_params=PoolingParams(dimensions=dimensions))
        else:
            vllm_logits_outputs = vllm_logits_model.encode(
                example_prompts,
                pooling_params=PoolingParams(dimensions=dimensions))
