...
def test_smaller_truncation_size(vllm_logits_runner,
                                 model_name=MODEL_NAME,
                                 input_str=input_str):

    truncate_prompt_tokens = 10

    with vllm_logits_runner(model_name, task="embed",
                     max_model_len=max_model_len) as vllm_logits_model:
        vllm_logits_output = vllm_logits_model.model.encode(
            input_str, truncate_prompt_tokens=truncate_prompt_tokens)

    prompt_tokens = vllm_logits_output[0].prompt_token_ids

    assert len(prompt_tokens) == truncate_prompt_tokens


def test_max_truncation_size(vllm_logits_runner,
                             model_name=MODEL_NAME,
                             input_str=input_str):
...
def test_bigger_truncation_size(vllm_logits_runner,
                                model_name=MODEL_NAME,
                                input_str=input_str):

    truncate_prompt_tokens = max_model_len + 1

    with pytest.raises(ValueError), vllm_logits_runner(
            model_name, task="embed",
            max_model_len=max_model_len) as vllm_logits_model:

        llm_output = vllm_logits_model.model.encode(
            input_str, truncate_prompt_tokens=truncate_prompt_tokens)
