...
from vllm_logits.inputs.data import ExplicitEncoderDecoderPrompt, TextPrompt
...
def hf_to_vllm_logits_output(hf_output: tuple[list[int], str,
...
def run_test(
    ...
    *,
    ...
    with vllm_logits_runner(model,
                     ...
                     enforce_eager=True) as vllm_logits_model:
        vllm_logits_outputs_per_case = [
            vllm_logits_model.generate_encoder_decoder_greedy_logprobs(
                prompts,
                max_tokens,
    ...
    for hf_outputs, vllm_logits_outputs in zip(hf_outputs_per_case,
        ...
        check_logprobs_close(
            outputs_0_lst=[hf_to_vllm_logits_output(output) for output in hf_outputs],
...
def test_models(hf_runner: type[HfRunner], vllm_logits_runner: type[VllmRunner],
