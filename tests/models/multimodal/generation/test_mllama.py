...
from vllm_logits.attention.backends.flash_attn import FlashAttentionMetadata
from vllm_logits.attention.selector import (_Backend, _cached_get_attn_backend,
                                     global_force_attn_backend_context_manager)
from vllm_logits.model_executor.models.mllama import MllamaForConditionalGeneration
from vllm_logits.multimodal.image import rescale_image_size
...
def vllm_logits_to_hf_output(vllm_logits_output: tuple[list[int], str,
...
def run_test(
    ...
    vllm_logits_runner: type[VllmRunner],
    image_assets: ImageTestAssets,
    model: str,
    *,
...
def _run_test(
    ...
    *,
    ...
    """Inference result should be the same between hf and vllm_logits.

    All the image fixtures for the test are from IMAGE_ASSETS.
    For huggingface runner, we provide the PIL images as input.
    For vllm_logits runner, we provide MultiModalDataDict objects 
    and corresponding MultiModalConfig as input.
    Note, the text input is also adjusted to abide by vllm_logits contract.
    ...
    with vllm_logits_runner(
            ...
            limit_mm_per_prompt={"image":
                                 _LIMIT_IMAGE_PER_PROMPT}) as vllm_logits_model:
        vllm_logits_outputs_per_image = [
            vllm_logits_model.generate_greedy_logprobs(prompts,
                                                max_tokens,
                                                num_logprobs=num_logprobs,
                                                images=images)
    ...
    for hf_outputs, vllm_logits_outputs in zip(hf_outputs_per_image,
                                        vllm_logits_outputs_per_image):
        check_logprobs_close(
            outputs_0_lst=hf_outputs,
            outputs_1_lst=[
                vllm_logits_to_hf_output(vllm_logits_output, model)
                for vllm_logits_output in vllm_logits_outputs
            ],
            name_0="hf",
            name_1="vllm_logits",
        )
...
def test_models_single_leading_image(hf_runner, vllm_logits_runner, image_assets,
...
@pytest.mark.parametrize("num_logprobs", [5])
@pytest.mark.parametrize("attn_backend", LIST_ENC_DEC_SUPPORTED_BACKENDS)
def test_models_multi_leading_images(hf_runner, vllm_logits_runner, image_assets,
...
@pytest.mark.parametrize("num_logprobs", [5])
@pytest.mark.parametrize("attn_backend", LIST_ENC_DEC_SUPPORTED_BACKENDS)
def test_models_interleaved_images(hf_runner, vllm_logits_runner, image_assets, model,
...
@pytest.mark.parametrize("num_logprobs", [5])
@pytest.mark.parametrize("attn_backend", LIST_ENC_DEC_SUPPORTED_BACKENDS)
def test_regression(vllm_logits_runner, image_assets, model, dtype, max_tokens,
                    num_logprobs, attn_backend: _Backend) -> None:

    stop_sign = image_assets[0].pil_image

    with global_force_attn_backend_context_manager(attn_backend), vllm_logits_runner(
            model,
            dtype=dtype,
            max_model_len=8192,
            max_num_seqs=4,
        ...
        # Regression tests for https://github.com/vllm_logits-project/vllm_logits/issues/10648
        ...
        with pytest.raises(ValueError):
            vllm_logits_model.generate_greedy_logprobs([prompt],
                                                max_tokens,
                                                num_logprobs,
                                                images=[image])
