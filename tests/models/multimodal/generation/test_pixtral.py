...
from vllm_logits import RequestOutput, SamplingParams, TextPrompt, TokensPrompt
from vllm_logits.multimodal import MultiModalDataBuiltins
from vllm_logits.multimodal.inputs import PlaceholderRange
from vllm_logits.sequence import Logprob, SampleLogprobs

from ....utils import VLLM_LOGITS_PATH, large_gpu_test
...
FIXTURES_PATH = VLLM_LOGITS_PATH / "tests/models/fixtures"
...
def test_chat(
    ...
    with vllm_logits_runner(
        ...
        outputs = []
        for msg in MSGS:
            output = vllm_logits_model.model.chat(msg,
                                           sampling_params=SAMPLING_PARAMS)

            outputs.extend(output)

    logprobs = vllm_logits_runner._final_steps_generate_w_logprobs(outputs)
...
def test_multi_modal_placeholders(vllm_logits_runner, prompt,
    ...
    with vllm_logits_runner(
        ...
        outputs = vllm_logits_model.model.generate(prompt)
