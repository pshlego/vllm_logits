...
from vllm_logits.multimodal.video import rescale_video_size, sample_frames_from_video
    ...
    *,
    ...
    with vllm_logits_runner(model,
        ...
        outputs_per_case_for_embeddings_input = [
            vllm_logits_model.generate_greedy_logprobs(
                prompts,
                max_tokens,
                num_logprobs=num_logprobs,
                images=batch_make_image_embeddings(
                    images, processor, vllm_logits_model) if images else None,
                videos=batch_make_video_embeddings(
                    videos, processor, vllm_logits_model) if videos else None)
            for prompts, images, videos in inputs
        ]
...
def test_qwen2_vl_image_embeddings_input(vllm_logits_runner, image_assets, model,
...
@pytest.mark.parametrize("max_tokens", [128])
@pytest.mark.parametrize("num_logprobs", [10])
def test_qwen2_vl_multiple_image_embeddings_input(vllm_logits_runner, image_assets,
                                                  model, size_factors,
...
@pytest.mark.parametrize("max_tokens", [128])
@pytest.mark.parametrize("num_logprobs", [10])
def test_qwen2_vl_video_embeddings_input(vllm_logits_runner, video_assets, model,
                                         size_factors, dtype: str,
                                         max_tokens: int,
                                         num_logprobs: int) -> None:
    num_frames = 4
