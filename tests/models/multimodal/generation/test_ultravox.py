...
def server(request, audio_assets: AudioTestAssets):
    ...
    with RemoteOpenAIServer(MODEL_NAME,
                            args,
                            env_dict={"VLLM_LOGITS_AUDIO_FETCH_TIMEOUT":
                                      "30"}) as remote_server:
        yield remote_server
...
def run_multi_audio_test(
    ...
    **kwargs,
    ...
    assert all(tokens for tokens, *_ in vllm_logits_outputs)
...
@pytest.mark.parametrize("vllm_logits_kwargs", [
    pytest.param({}, marks=pytest.mark.cpu_model),
    pytest.param(CHUNKED_PREFILL_KWARGS),
])
def test_models_with_multiple_audios(vllm_logits_runner,
                                     audio_assets: AudioTestAssets, dtype: str,
                                     max_tokens: int, num_logprobs: int,
                                     vllm_logits_kwargs: dict) -> None:

    vllm_logits_prompt = _get_prompt(len(audio_assets), MULTI_AUDIO_PROMPT,
                              VLLM_LOGITS_PLACEHOLDER)
    run_multi_audio_test(
        vllm_logits_runner,
        [(vllm_logits_prompt, [audio.audio_and_sample_rate
