...
from vllm_logits.config import TaskOption
...
def run_test(
    *,
    ...
    vllm_logits_output_post_proc: Optional[Callable[[RunnerOutput, str], Any]],
    ...
    vllm_logits_runner_kwargs: Optional[dict[str, Any]],
    hf_model_kwargs: Optional[dict[str, Any]],
    patch_hf_runner: Optional[Callable[[HfRunner], HfRunner]],
    task: TaskOption = "auto",
    distributed_executor_backend: Optional[str] = None,
    tensor_parallel_size: int = 1,
    vllm_logits_embeddings: Optional[torch.Tensor] = None,
):
    """Modality agnostic test test executor for comparing HF/vLLM outputs."""
    # In the case of embeddings, vLLM takes separate input tensors
    vllm_logits_inputs = vllm_logits_embeddings if vllm_logits_embeddings is not None else inputs
    ...
    vllm_logits_runner_kwargs_: dict[str, Any] = {
        "disable_mm_preprocessor_cache": True,
    }
    if model_info.tokenizer:
        vllm_logits_runner_kwargs_["tokenizer_name"] = model_info.tokenizer
    if model_info.tokenizer_mode:
        vllm_logits_runner_kwargs_["tokenizer_mode"] = model_info.tokenizer_mode
    if model_info.hf_overrides:
        vllm_logits_runner_kwargs_["hf_overrides"] = model_info.hf_overrides

    if vllm_logits_runner_kwargs:
        vllm_logits_runner_kwargs_.update(vllm_logits_runner_kwargs)
    ...
    with vllm_logits_runner(model,
                     ...
                     task=task,
                     **vllm_logits_runner_kwargs_) as vllm_logits_model:
        tokenizer = vllm_logits_model.model.get_tokenizer()

        vllm_logits_kwargs: dict[str, Any] = {}
        if get_stop_token_ids is not None:
            vllm_logits_kwargs["stop_token_ids"] = get_stop_token_ids(tokenizer)
        if stop_str:
            vllm_logits_kwargs["stop"] = stop_str

        for prompts, image_data, video_data, audio_data in vllm_logits_inputs:
            mm_data = dict(images=image_data,
                           videos=video_data,
                           audios=audio_data)
            vllm_logits_kwargs_with_mm_data = vllm_logits_kwargs | mm_data
            vllm_logits_output = vllm_logits_model.generate_greedy_logprobs(
                prompts,
                max_tokens,
                num_logprobs=num_logprobs,
                **vllm_logits_kwargs_with_mm_data)
            vllm_logits_outputs_per_mm.append(vllm_logits_output)
    ...
    hf_outputs_per_mm, vllm_logits_outputs_per_mm = process_runner_outputs(
        model,
        first_runner_outputs=hf_outputs_per_mm,
        second_runner_outputs=vllm_logits_outputs_per_mm,
        first_runner_processor=hf_output_post_proc,
        second_runner_processor=vllm_logits_output_post_proc,
    )

    for hf_outputs, vllm_logits_outputs in zip(hf_outputs_per_mm,
                                        vllm_logits_outputs_per_mm):
