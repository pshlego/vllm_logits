...
from vllm_logits.sequence import SampleLogprobs
from vllm_logits.transformers_utils.tokenizer import patch_padding_side
...
def blip2_vllm_logits_to_hf_output(vllm_logits_output: RunnerOutput,
                            model: str) -> RunnerOutput:
    """Sanitize vllm_logits output [blip2 models] to be comparable with hf output."""
    _, output_str, out_logprobs = vllm_logits_output
...
def fuyu_vllm_logits_to_hf_output(vllm_logits_output: RunnerOutput,
                           model: str) -> RunnerOutput:
    """Sanitize vllm_logits output [fuyu models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_logits_output
...
def qwen_vllm_logits_to_hf_output(
        vllm_logits_output: RunnerOutput,
        model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    """Sanitize vllm_logits output [qwen models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_logits_output
...
def qwen2_vllm_logits_to_hf_output(
        vllm_logits_output: RunnerOutput,
        model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    """Sanitize vllm_logits output [qwen2 models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_logits_output
...
def kimiv_vl_vllm_logits_to_hf_output(
        vllm_logits_output: RunnerOutput,
        model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    """Sanitize vllm_logits output [kimi_vl models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_logits_output
...
def llava_image_vllm_logits_to_hf_output(vllm_logits_output: RunnerOutput,
                                  model: str) -> RunnerOutput:
    config = AutoConfig.from_pretrained(model)
    mm_token_id = config.image_token_index
    return _llava_vllm_logits_to_hf_output(vllm_logits_output, model, mm_token_id)


def llava_video_vllm_logits_to_hf_output(
        vllm_logits_output: RunnerOutput,
        model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    config = AutoConfig.from_pretrained(model)
...
def _llava_vllm_logits_to_hf_output(vllm_logits_output: RunnerOutput, model: str,
                             mm_token_id: int) -> RunnerOutput:
    """Sanitize vllm_logits output [Llava models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_logits_output
...
def llava_onevision_vllm_logits_to_hf_output(vllm_logits_output: RunnerOutput,
                                      model: str) -> RunnerOutput:
    """Sanitize vllm_logits output [llava-onevision] to compare with hf output."""
    output_ids, output_str, out_logprobs = vllm_logits_output
...
def mantis_vllm_logits_to_hf_output(vllm_logits_output: RunnerOutput,
                             model: str) -> RunnerOutput:
    """Sanitize vllm_logits output [mantis] to compare with hf output."""
    output_ids, output_str, out_logprobs = vllm_logits_output
...
def phi3v_vllm_logits_to_hf_output(vllm_logits_output: RunnerOutput,
                            model: str) -> RunnerOutput:
    """Sanitize vllm_logits output [phi3v] to be comparable with hf output."""
    _, output_str, out_logprobs = vllm_logits_output
...
def paligemma_vllm_logits_to_hf_output(vllm_logits_output: RunnerOutput,
                                model: str) -> RunnerOutput:
    """Sanitize vllm_logits output to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_logits_output
...
def h2ovl_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    class H2OVLProcessor:
        ...
        def __call__(self, text: str, images: Union[Image, list[Image]],
                     **kwargs):
            # yapf: disable
            from vllm_logits.model_executor.models.h2ovl import (
...
def skyworkr1v_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    class SkyworkR1VProcessor:
        ...
        def __call__(self, text: str, images: Union[Image, list[Image]],
                     **kwargs):
            from vllm_logits.model_executor.models.skyworkr1v import (
    ...
    class InternVLProcessor:
        ...
        def __call__(
            ...
            **kwargs,
            ...
            from vllm_logits.model_executor.models.internvl import (
                IMG_CONTEXT, IMG_END, IMG_START,
