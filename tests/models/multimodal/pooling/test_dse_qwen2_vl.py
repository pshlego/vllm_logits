...
def _run_test(
    ...
    *,
    ...
    with vllm_logits_runner(model,
                     ...
                     dtype=dtype,
                     enforce_eager=True,
                     max_model_len=8192) as vllm_logits_model:
        tokenizer = vllm_logits_model.model.get_tokenizer()
        texts = [
            # this is necessary because vllm_logits_model.encode will not apply any
            ...
            # vllm_logits will replace the pad token with the actual image,
            # which may be a placeholder image, later.
        ]
        vllm_logits_outputs = vllm_logits_model.encode(texts, images=input_images)
