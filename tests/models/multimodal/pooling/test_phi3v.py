...
from vllm_logits.assets.base import get_vllm_logits_public_assets
from vllm_logits.assets.image import VLM_IMAGES_DIR
...
def _run_test(
    ...
    *,
    ...
    with vllm_logits_runner(model, task="embed", dtype=dtype,
                     enforce_eager=True) as vllm_logits_model:
        vllm_logits_outputs = vllm_logits_model.encode(input_texts, images=input_images)
...
def test_models_image(
    ...
    input_texts_images.append((
        ...
        "\n<|assistant|>\n",
        Image.open(
            get_vllm_logits_public_assets(filename="cherry_blossom.jpg",
