...
def test_disable_sliding_window(model_len_len, ):
    ...
    vllm_logits_disabled_model = LLM(model, disable_sliding_window=True)
    vllm_logits_disabled_model.generate("Hi my name is")
    model_config = vllm_logits_disabled_model.llm_engine.model_config
    assert model_config.max_model_len == sliding_len, (
        "Max len expected to equal sliding_len of %s, but got %s", sliding_len,
        model_config.max_model_len)

    del vllm_logits_disabled_model
    cleanup_dist_env_and_memory()

    vllm_logits_enabled_model = LLM(model,
                             enforce_eager=True,
                             disable_sliding_window=False,
                             enable_prefix_caching=False)
    vllm_logits_enabled_model.generate("Hi my name is")
    model_config = vllm_logits_enabled_model.llm_engine.model_config
    assert model_config.max_model_len == full_len, (
        "Max len expected to equal full_len of %s, but got %s", full_len,
        model_config.max_model_len)

    del vllm_logits_enabled_model
    cleanup_dist_env_and_memory()
