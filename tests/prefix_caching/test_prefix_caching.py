...
from vllm_logits import SamplingParams, TokensPrompt
...
def test_mixed_requests(
    ...
    with monkeypatch.context() as m:
        ...
        with vllm_logits_runner(
            ...
            vllm_logits_outputs = vllm_logits_model.generate_greedy([cached_prompt],
                                                      max_tokens)

            # Run all the promopts
            greedy_params = SamplingParams(temperature=0.0,
                                           max_tokens=max_tokens)
            req_outputs = vllm_logits_model.model.generate(example_prompts,
                                                    greedy_params)
    ...
    with monkeypatch.context() as m:
        ...
        with vllm_logits_runner(
            ...
            for prompt in UNSTABLE_PROMPT_SEQUENCE:
                vllm_logits_model.generate(TokensPrompt(prompt_token_ids=prompt),
