...
from vllm_logits.model_executor.layers.quantization.compressed_tensors.compressed_tensors import (  # noqa: E501
    CompressedTensors24, CompressedTensorsLinearMethod,
    CompressedTensorsW4A16Fp4, CompressedTensorsW4A16Sparse24,
    CompressedTensorsW8A8Fp8, CompressedTensorsW8A8Int8,
    CompressedTensorsW8A16Fp8, CompressedTensorsWNA16)
from vllm_logits.model_executor.layers.quantization.utils.w8a8_utils import (
...
def use_v0_only(monkeypatch):
    """
    This module relies on V0 internals, so set VLLM_LOGITS_USE_V1=0.
    """
...
def test_compressed_tensors_w8a8_static_setup(vllm_logits_runner, model_args):
    ...
    with vllm_logits_runner(model_path, enforce_eager=True) as llm:
...
def test_compressed_tensors_w8a8_logprobs(
    ...
    if use_aiter:
        ...
        # this will enable VLLM_LOGITS_ROCM_USE_AITER_LINEAR
        monkeypatch.setenv("VLLM_LOGITS_ROCM_USE_AITER", "1")
    ...
    with vllm_logits_runner(model_path, dtype=dtype) as vllm_logits_model:
...
def test_compressed_tensors_no_enforce_eager(vllm_logits_runner):
    model_path = "nm-testing/tinyllama-oneshot-w8w8-test-static-shape-change"
    with vllm_logits_runner(model_path) as llm:
        output = llm.generate_greedy("Hello my name is", max_tokens=20)
        assert output
...
def test_compressed_tensors_w8a8_dynamic_per_token(
    ...
    with vllm_logits_runner(model_path, dtype=torch.float16) as llm:
...
def test_compressed_tensors_wNa16(vllm_logits_runner, wNa16_args):
    model, strategy, group, pack_factor, symmetric, has_g_idx = wNa16_args
    with vllm_logits_runner(model) as llm:
...
def test_compressed_tensors_w4a16_marlin24(vllm_logits_runner):
...
def test_compressed_tensors_fp8(vllm_logits_runner):
...
@pytest.mark.skipif(not current_platform.is_cuda(),
                    reason="This test is skipped on non-CUDA platform.")
def test_compressed_tensors_kv_cache(vllm_logits_runner):
    model_path = "nm-testing/TinyLlama-1.1B-compressed-tensors-kv-cache-scheme"
    with vllm_logits_runner(model_path, kv_cache_dtype="fp8") as llm:
...
def test_compressed_tensors_2of4_quant_fp8(vllm_logits_runner, args_2of4):
...
def test_compressed_tensors_2of4_quant_fp8_compressed(vllm_logits_runner, args_2of4):
    model, weight_strategy, input_strategy = args_2of4
    with vllm_logits_runner(model) as llm:
...
def test_compressed_tensors_2of4_quant_int8_compressed(vllm_logits_runner, args_2of4):
    model, weight_strategy, input_strategy = args_2of4
    with vllm_logits_runner(model) as llm:
...
def test_compressed_tensors_2of4_quant_int8(vllm_logits_runner, args_2of4):
    model, weight_strategy, input_strategy = args_2of4
    with vllm_logits_runner(model) as llm:
...
def test_compressed_tensors_2of4_sparse(vllm_logits_runner, args_2of4):
    model = args_2of4
    with vllm_logits_runner(model) as llm:
...
def test_compressed_tensors_2of4_sparse_compressed(vllm_logits_runner, args_2of4):
    model = args_2of4
    with vllm_logits_runner(model) as llm:
...
def test_compressed_tensors_nvfp4a16(vllm_logits_runner):
    # run weight only example
    model = "nm-testing/TinyLlama-1.1B-Chat-v1.0-FP4"
    with vllm_logits_runner(model, enforce_eager=True) as llm:
