...
from vllm_logits.model_executor.layers.quantization.fp8 import (Fp8KVCacheMethod,
                                                         Fp8LinearMethod)
...
def test_model_load_and_run(vllm_logits_runner, model_id: str, force_marlin: bool,
    ...
    if force_marlin:
        monkeypatch.setenv("VLLM_LOGITS_TEST_FORCE_FP8_MARLIN", "1")

    with vllm_logits_runner(model_id) as llm:
...
@pytest.mark.parametrize(
    "use_rocm_aiter", [True, False] if current_platform.is_rocm() else [False])
def test_kv_cache_model_load_and_run(vllm_logits_runner, model_id: str,
                                     use_rocm_aiter: bool, monkeypatch):
    if use_rocm_aiter:
        monkeypatch.setenv("VLLM_LOGITS_ROCM_USE_AITER", "1")

    # vllm_logits_runner.apply_model() relies on V0 internals.
    monkeypatch.setenv("VLLM_LOGITS_USE_V1", "0")
    with vllm_logits_runner(model_id, kv_cache_dtype="fp8") as llm:
        ...
        def check_model(model):
            ...
            if not current_platform.is_rocm():
                ...
                # vllm_logits/model_executor/layers/quantization/kv_cache.py
...
@pytest.mark.parametrize(
    "use_rocm_aiter", [True, False] if current_platform.is_rocm() else [False])
def test_load_fp16_model(vllm_logits_runner, kv_cache_dtype: str, force_marlin: bool,
                         use_rocm_aiter: bool, monkeypatch) -> None:
    if use_rocm_aiter:
        monkeypatch.setenv("VLLM_LOGITS_ROCM_USE_AITER", "1")
    ...
    with vllm_logits_runner("facebook/opt-125m",
                     quantization="fp8",
