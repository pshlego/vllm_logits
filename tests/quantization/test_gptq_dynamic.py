...
from vllm_logits.model_executor.layers.quantization.gptq import GPTQLinearMethod
...
def test_gptq_with_dynamic(vllm_logits_runner, model_id: str, use_marlin_kernel: bool,
                           monkeypatch):
    # vllm_logits_runner.apply_model() relies on V0 internals.
    monkeypatch.setenv("VLLM_LOGITS_USE_V1", "0")

    vllm_logits_model = vllm_logits_runner(model_id, dtype=torch.float16, max_model_len=2048)

    linear_method_cls = GPTQMarlinLinearMethod if use_marlin_kernel else (
        GPTQLinearMethod)

    for name, submodule in (vllm_logits_model.model.llm_engine.model_executor.
