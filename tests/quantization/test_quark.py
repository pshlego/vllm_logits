...
from vllm_logits.model_executor.layers.quantization.quark.quark import (  # noqa: E501
...
def test_quark_fp8_w_per_tensor_a_per_tensor(vllm_logits_runner, kv_cache_dtype, tp):
    model_path = "amd/Llama-3.1-8B-Instruct-FP8-KV-Quark-test"
    with vllm_logits_runner(model_path,
...
@pytest.mark.parametrize('tp', [1])
def test_quark_int8_w_per_tensor_a_per_tensor(vllm_logits_runner, tp):
    model_path = "amd/Llama-3.1-8B-Instruct-w-int8-a-int8-sym-test"
    with vllm_logits_runner(model_path, tensor_parallel_size=tp) as llm:
...
def test_quark_fp8_parity(vllm_logits_runner):
    ...
    with (vllm_logits_runner(quark_model_id, **llm_kwargs) as
          quark_handle, vllm_logits_runner(fp8_model_id, **llm_kwargs) as fp8_handle):
