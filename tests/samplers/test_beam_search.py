...
def test_beam_search_single_input(
    ...
    with vllm_logits_runner(model, dtype=dtype) as vllm_logits_model:
        vllm_logits_outputs = vllm_logits_model.generate_beam_search(example_prompts,
                                                       beam_width, max_tokens)

    for i in range(len(example_prompts)):
        hf_output_ids, hf_output_texts = hf_outputs[i]
        vllm_logits_output_ids, vllm_logits_output_texts = vllm_logits_outputs[i]
        for j, (hf_text,
                vllm_logits_text) in enumerate(zip(hf_output_texts,
                                            vllm_logits_output_texts)):
            print(f">>>{j}-th hf output:")
            print(hf_text)
            print(f">>>{j}-th vllm_logits output:")
            print(vllm_logits_text)
        assert len(hf_output_ids) == len(vllm_logits_output_ids)
        for j in range(len(hf_output_ids)):
            assert hf_output_ids[j] == vllm_logits_output_ids[j], (
                f"Test{i} output{j}:\nHF: {hf_output_ids}\n"
                f"vLLM: {vllm_logits_output_ids}")
...
def test_beam_search_passes_multimodal_data(
    ...
    with vllm_logits_runner(model, dtype=dtype) as vllm_logits_model:
        vllm_logits_outputs = vllm_logits_model.generate_beam_search(
            prompts,
            beam_width=beam_width,
            max_tokens=max_tokens,
            audios=audios,
    ...
    for i in range(len(prompts)):
        ...
        for j in range(len(hf_output_ids)):
            ...
            filtered_vllm_logits_output_ids = seq_with_no_audio_toks(
                vllm_logits_output_ids[j])

            # HF output IDs may contain the end of sequence
            if len(filtered_hf_output_ids
                   ) == len(filtered_vllm_logits_output_ids) + 1:
                assert filtered_hf_output_ids[-1] == eos_token_id
                filtered_hf_output_ids = filtered_hf_output_ids[:-1]

            assert filtered_hf_output_ids == filtered_vllm_logits_output_ids
