...
def test_logits_processor_force_generate(
    ...
    with vllm_logits_runner(model, dtype=dtype) as vllm_logits_model:
        ...
        vllm_logits_token_ids = tokenizer.encode(enforced_answers,
                                          add_special_tokens=False)
        max_tokens = len(vllm_logits_token_ids) * repeat_times

        def pick_vllm_logits(token_ids, logits):
            token_id = vllm_logits_token_ids[len(token_ids) % len(vllm_logits_token_ids)]
            logits[token_id] = torch.finfo(logits.dtype).max
            return logits

        params_with_logprobs = SamplingParams(
            logits_processors=[pick_vllm_logits],
            prompt_logprobs=3,
            max_tokens=max_tokens,
        )

        # test logits_processors when prompt_logprobs is not None
        vllm_logits_model.model._add_request(
            example_prompts[0],
        ...
        outputs = vllm_logits_model.model._run_engine(use_tqdm=False)
