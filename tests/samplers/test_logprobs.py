...
def use_v0_only(monkeypatch):
    ...
    set VLLM_LOGITS_USE_V1=0 for all tests in the module.
    """
...
def test_get_prompt_logprobs(
    ...
    with vllm_logits_runner(
        ...
        vllm_logits_sampling_params = SamplingParams(max_tokens=max_tokens,
                                              logprobs=num_top_logprobs,
                                              prompt_logprobs=num_top_logprobs,
                                              temperature=0.0,
                                              detokenize=detokenize)
        vllm_logits_results = vllm_logits_model.model.generate(
            example_prompts, sampling_params=vllm_logits_sampling_params)
    ...
    for vllm_logits_result, hf_logprob in zip(vllm_logits_results, hf_logprobs):
    ...
    for vllm_logits_result in vllm_logits_results:
        token_ids = vllm_logits_result.prompt_token_ids
        prompt_logprobs = vllm_logits_result.prompt_logprobs
...
@pytest.mark.parametrize("chunked_prefill_token_size", [1, 4, 16, -1])
@pytest.mark.parametrize("detokenize", [True, False])
def test_none_logprobs(vllm_logits_runner, model, chunked_prefill_token_size: int,
