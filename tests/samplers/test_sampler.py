...
from vllm_logits.sequence import SamplingParams, SequenceData, SequenceGroupMetadata
from vllm_logits.utils import Counter, is_pin_memory_available


@pytest.fixture(scope="function", autouse=True)
def use_v0_only(monkeypatch):
    """
    This file tests V0 internals, so set VLLM_LOGITS_USE_V1=0.
    """
...
def test_sampler_top_k_top_p(seed: int, device: str):
    ...
    with patch("vllm_logits.model_executor.layers.sampler._sample", mock_sample), \
         patch("vllm_logits.model_executor.layers.sampler."
...
def test_flashinfer_fallback(seed: int, device: str):
    if not envs.VLLM_LOGITS_USE_FLASHINFER_SAMPLER:
    ...
    with patch(
            "vllm_logits.model_executor.layers.sampler."
...
def test_sampler_include_gpu_probs_tensor(device: str):
    ...
    mock_inplace = Mock()
    with patch(
            "vllm_logits.model_executor.layers.sampler._modify_greedy_probs_inplace",
            mock_inplace):
