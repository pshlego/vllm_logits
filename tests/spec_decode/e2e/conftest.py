...
from vllm_logits.sequence import PromptLogprobs, SampleLogprobs
...
def run_equality_correctness_test(
    ...
    with vllm_logits_runner(**org_args) as vllm_logits_model:
        org_outputs = vllm_logits_model.generate_w_logprobs(prompts, sampling_params)

    with vllm_logits_runner(**sd_args) as vllm_logits_model:
        if ensure_all_accepted or expected_acceptance_rate is not None:
            # Force log interval to be 0 to catch all metrics.
            stat_logger = vllm_logits_model.model.llm_engine.stat_loggers[
                'prometheus']
            stat_logger.local_interval = -100

        sd_outputs = vllm_logits_model.generate_w_logprobs(prompts, sampling_params)
