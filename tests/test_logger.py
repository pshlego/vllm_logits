...
from vllm_logits.logger import (_DATE_FORMAT, _FORMAT, _configure_vllm_logits_root_logger,
                         enable_trace_function_call, init_logger)
from vllm_logits.logging_utils import NewLineFormatter
from vllm_logits.logging_utils.dump_input import prepare_object_to_dump
...
def test_default_vllm_logits_root_logger_configuration():
    """This test presumes that VLLM_LOGITS_CONFIGURE_LOGGING (default: True) and
    VLLM_LOGITS_LOGGING_CONFIG_PATH (default: None) are not configured and default
    behavior is activated."""
    logger = logging.getLogger("vllm_logits")
...
@patch("vllm_logits.logger.VLLM_LOGITS_CONFIGURE_LOGGING", 1)
@patch("vllm_logits.logger.VLLM_LOGITS_LOGGING_CONFIG_PATH", None)
def test_descendent_loggers_depend_on_and_propagate_logs_to_root_logger():
    """This test presumes that VLLM_LOGITS_CONFIGURE_LOGGING (default: True) and
    VLLM_LOGITS_LOGGING_CONFIG_PATH (default: None) are not configured and default
    behavior is activated."""
    root_logger = logging.getLogger("vllm_logits")
    root_handler = root_logger.handlers[0]

    unique_name = f"vllm_logits.{uuid4()}"
...
@patch("vllm_logits.logger.VLLM_LOGITS_CONFIGURE_LOGGING", 0)
@patch("vllm_logits.logger.VLLM_LOGITS_LOGGING_CONFIG_PATH", None)
def test_logger_configuring_can_be_disabled():
    """This test calls _configure_vllm_logits_root_logger again to test custom logging
    config behavior, however mocks are used to ensure no changes in behavior or
    configuration occur."""

    with patch("vllm_logits.logger.dictConfig") as dict_config_mock:
...
@patch("vllm_logits.logger.VLLM_LOGITS_CONFIGURE_LOGGING", 1)
@patch(
    "vllm_logits.logger.VLLM_LOGITS_LOGGING_CONFIG_PATH",
...
def test_an_error_is_raised_when_custom_logging_config_is_invalid_json():
    ...
    with NamedTemporaryFile(encoding="utf-8", mode="w") as logging_config_file:
        ...
        with patch("vllm_logits.logger.VLLM_LOGITS_LOGGING_CONFIG_PATH",
...
def test_custom_logging_config_is_parsed_and_used_when_provided():
    ...
    valid_logging_config = {
        "loggers": {
            "vllm_logits.test_logger.logger": {
                "handlers": [],
                "propagate": False,
            }
        },
    ...
    with NamedTemporaryFile(encoding="utf-8", mode="w") as logging_config_file:
        ...
        with patch("vllm_logits.logger.VLLM_LOGITS_LOGGING_CONFIG_PATH",
                   logging_config_file.name), patch(
                       "vllm_logits.logger.dictConfig") as dict_config_mock:
            _configure_vllm_logits_root_logger()
    ...
    with NamedTemporaryFile(encoding="utf-8", mode="w") as logging_config_file:
        ...
        with patch("vllm_logits.logger.VLLM_LOGITS_LOGGING_CONFIG_PATH",
            ...
            expected_message_snippet = (
                ...
                "VLLM_LOGITS_LOGGING_CONFIG_PATH was given.")
            assert expected_message_snippet in str(ex_info)

        # Remember! The root logger is assumed to have been configured as
        # though VLLM_LOGITS_CONFIGURE_LOGGING=1 and VLLM_LOGITS_LOGGING_CONFIG_PATH=None.
        root_logger = logging.getLogger("vllm_logits")
        other_logger_name = f"vllm_logits.test_logger.{uuid4()}"
