...
import torch
import zmq
from vllm_logits_test_utils.monitor import monitor

from vllm_logits.config import ParallelConfig, VllmConfig, set_current_vllm_logits_config
from vllm_logits.utils import (CacheInfo, FlexibleArgumentParser, LRUCache,
...
def test_get_open_port(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m:
        m.setenv("VLLM_LOGITS_PORT", "5678")
...
def test_memory_profiling():
    ...
    from vllm_logits.distributed.device_communicators.cuda_wrapper import (
        CudaRTLibrary)
    lib = CudaRTLibrary()
...
def test_bind_kv_cache_pp():
    with patch("vllm_logits.utils.cuda_device_count_stateless", lambda: 2):
        # this test runs with 1 GPU, but we simulate 2 GPUs
        cfg = VllmConfig(
            parallel_config=ParallelConfig(pipeline_parallel_size=2))
    with set_current_vllm_logits_config(cfg):
        from vllm_logits.attention import Attention
...
def test_model_specification(parser_with_config, cli_config_file,
    ...
    with pytest.raises(
            ValueError,
            match=
        ("With `vllm_logits serve`, you should provide the model as a positional "
