...
def use_v1_only(monkeypatch: pytest.MonkeyPatch):
    """
    Since Multi-LoRA is only supported on the v1 TPU backend, set VLLM_LOGITS_USE_V1=1
...
def setup_vllm_logits(num_loras: int) -> vllm_logits.LLM:
    return vllm_logits.LLM(model="Qwen/Qwen2.5-3B-Instruct",
...
def test_single_lora():
    ...
    output = llm.generate(prompt,
                          sampling_params=vllm_logits.SamplingParams(max_tokens=256,
                                                              temperature=0),
...
def test_lora_hotswapping():
    ...
    for i, req in enumerate(lora_requests):
        output = llm.generate(prompt,
                              sampling_params=vllm_logits.SamplingParams(
