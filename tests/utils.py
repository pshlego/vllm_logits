...
from vllm_logits.entrypoints.cli.serve import ServeSubcommand
from vllm_logits.model_executor.model_loader import get_model_loader
from vllm_logits.platforms import current_platform
from vllm_logits.transformers_utils.tokenizer import get_tokenizer
from vllm_logits.utils import (FlexibleArgumentParser, GB_bytes,
...
if current_platform.is_rocm():
    ...
    from vllm_logits.third_party.pynvml import (nvmlDeviceGetHandleByIndex,
...
VLLM_LOGITS_PATH = Path(__file__).parent.parent
...
class RemoteOpenAIServer:
    ...
    def __init__(self,
                 model: str,
                 vllm_logits_serve_args: list[str],
                 *,
        ...
        if auto_port:
            if "-p" in vllm_logits_serve_args or "--port" in vllm_logits_serve_args:
                raise ValueError("You have manually specified the port "
                                 "when `auto_port=True`.")

            # Don't mutate the input args
            vllm_logits_serve_args = vllm_logits_serve_args + [
                "--port", str(get_open_port())
            ]
        if seed is not None:
            if "--seed" in vllm_logits_serve_args:
                raise ValueError("You have manually specified the seed "
                                 f"when `seed={seed}`.")

            vllm_logits_serve_args = vllm_logits_serve_args + ["--seed", str(seed)]

        parser = FlexibleArgumentParser(
            description="vLLM's remote OpenAI server.")
        subparsers = parser.add_subparsers(required=False, dest="subparser")
        parser = ServeSubcommand().subparser_init(subparsers)
        args = parser.parse_args(["--model", model, *vllm_logits_serve_args])
        ...
        env['VLLM_LOGITS_WORKER_MULTIPROC_METHOD'] = 'spawn'
        if env_dict is not None:
            env.update(env_dict)
        self.proc = subprocess.Popen(
            ["vllm_logits", "serve", model, *vllm_logits_serve_args],
            env=env,
            stdout=sys.stdout,
            stderr=sys.stderr,
        )
...
def compare_all_settings(model: str,
    ...
    for i, (args, env) in enumerate(zip(all_args, all_envs)):
        if can_force_load_format:
            ...
            # if not, we can use VLLM_LOGITS_TEST_FORCE_LOAD_FORMAT
            # environment variable to force the load format,
            # e.g. in quantization tests.
            args = args + ["--load-format", envs.VLLM_LOGITS_TEST_FORCE_LOAD_FORMAT]
...
def fork_new_process_for_each_test(
    ...
    See https://github.com/vllm_logits-project/vllm_logits/issues/7053 for more details.
    """
