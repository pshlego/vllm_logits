...
from vllm_logits.config import ModelConfig, SchedulerConfig, VllmConfig
...
from vllm_logits.v1.core.kv_cache_manager import KVCacheManager
# disable yapf here as it formats differently than isort such that both fail
# yapf: disable
from vllm_logits.v1.core.kv_cache_utils import (FreeKVCacheBlockQueue, KVCacheBlock,
...
def test_none_hash(monkeypatch):
    import vllm_logits.v1.core.kv_cache_utils

    # case 1: PYTHONHASHSEED is not set, use random
    with monkeypatch.context() as m:
        m.delenv('PYTHONHASHSEED', raising=False)
        reloaded_kv_cache_utils = importlib.reload(vllm_logits.v1.core.kv_cache_utils)
...
def test_kv_cache_block():
    ...
    block_hash = vllm_logits.v1.core.kv_cache_utils.BlockHash(hash_value=123,
                                                       token_ids=(1, 2, 3))
...
def test_hash_block_tokens(hash_fn):
    ...
    assert isinstance(block_hash, vllm_logits.v1.core.kv_cache_utils.BlockHash)
...
def test_hash_request_tokens(hash_fn):
    ...
    assert isinstance(block_hashes[0], vllm_logits.v1.core.kv_cache_utils.BlockHash)
    assert isinstance(block_hashes[1], vllm_logits.v1.core.kv_cache_utils.BlockHash)
