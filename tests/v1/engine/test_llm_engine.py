...
from vllm_logits.v1.metrics.reader import Counter, Gauge, Histogram, Metric, Vector

MODEL = "facebook/opt-125m"
DTYPE = "half"


def _vllm_logits_model(apc: bool, vllm_logits_runner, monkeypatch):
    """Set up VllmRunner instance."""
    monkeypatch.setenv("VLLM_LOGITS_USE_V1", "1")
    return vllm_logits_runner(
        MODEL,
        dtype=DTYPE,
        max_model_len=128,
        enforce_eager=True,
...
@pytest.fixture(
    ...
    # Prefix caching
    params=[False, True])
def vllm_logits_model(vllm_logits_runner, request, monkeypatch):
    """VllmRunner test fixture parameterized by APC True/False."""
    with _vllm_logits_model(request.param, vllm_logits_runner, monkeypatch) as vllm_logits_model:
        yield vllm_logits_model


@pytest.fixture(scope="function")
def vllm_logits_model_apc(vllm_logits_runner, monkeypatch):
    """VllmRunner test fixture with APC."""
    with _vllm_logits_model(True, vllm_logits_runner, monkeypatch) as vllm_logits_model:
        yield vllm_logits_model
...
def test_parallel_sampling(vllm_logits_model, example_prompts) -> None:
    """Test passes if parallel sampling `n>1` yields `n` unique completions.
    
    Args:
      vllm_logits_model: VllmRunner instance under test.
      example_prompt: test fixture providing prompts for testing.
    """
    sampling_params_list, n_list = _get_test_sampling_params(example_prompts)
    model: LLM = vllm_logits_model.model
...
def test_engine_metrics(vllm_logits_runner, monkeypatch, example_prompts):
    max_tokens = 100
    ...
    with vllm_logits_runner(
        ...
        num_requests_running = find_metric("vllm_logits:num_requests_running")
        assert len(num_requests_running) == 1
        assert isinstance(num_requests_running[0], Gauge)
        assert num_requests_running[0].value == .0

        generation_tokens = find_metric("vllm_logits:generation_tokens")
        ...
        num_accepted_tokens_per_pos = find_metric(
            "vllm_logits:spec_decode_num_accepted_tokens_per_pos")
