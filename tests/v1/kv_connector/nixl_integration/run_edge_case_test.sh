...
cleanup_instances() {
  ...
  pkill -f "vllm_logits serve" || true
  sleep 2
}
...
run_tests_for_model() {
  ...
  BASE_CMD="CUDA_VISIBLE_DEVICES=0 VLLM_LOGITS_NIXL_SIDE_CHANNEL_PORT=5559 vllm_logits serve $model_name \
  --port $PREFILL_PORT \
  --enforce-eager \
  ...
  BASE_CMD="CUDA_VISIBLE_DEVICES=1 VLLM_LOGITS_NIXL_SIDE_CHANNEL_PORT=6000 vllm_logits serve $model_name \
  --port $DECODE_PORT \
  --enforce-eager \
  --disable-log-requests \
  --gpu-memory-utilization 0.2 \
