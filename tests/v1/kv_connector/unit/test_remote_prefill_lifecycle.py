...
from vllm_logits.v1.outputs import EMPTY_MODEL_RUNNER_OUTPUT
from vllm_logits.v1.request import FinishReason, RequestStatus

from .utils import (assert_scheduler_empty, create_model_runner_output,
                    create_request, create_scheduler, create_vllm_logits_config)


def test_basic_lifecycle():
    """Test lifecycle of a remote prefill."""

    vllm_logits_config = create_vllm_logits_config()
    scheduler = create_scheduler(vllm_logits_config)
...
def test_cannot_schedule_after_recv():
    ...
    TOTAL_NUM_BLOCKS = 6
    vllm_logits_config = create_vllm_logits_config()
    scheduler = create_scheduler(vllm_logits_config, num_blocks=TOTAL_NUM_BLOCKS)
