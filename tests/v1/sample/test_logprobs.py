...
@pytest.fixture(
    ...
    # Parameterize APC
    params=[False, True])
def vllm_logits_model(vllm_logits_runner, request) -> Generator[VllmRunner, None, None]:
    with vllm_logits_runner(
            MODEL,
            dtype=DTYPE,
            max_logprobs=7,
    ...
    ) as vllm_logits_model:
        yield vllm_logits_model
...
def _run_and_validate(
    ...
    vllm_logits_results = vllm_logits_model.model.generate(
        test_prompts, sampling_params=vllm_logits_sampling_params)

    for vllm_logits_result, hf_logprob, hf_output, logprob_prompt_logprob in zip(
            vllm_logits_results, hf_logprobs, hf_outputs,
        ...
        if temperature == 0.0:
            assert (vllm_logits_result.prompt_token_ids +
                    vllm_logits_result.outputs[0].token_ids == hf_output[0])
        else:
            # Sampled tokens won't match if not greedy
            assert (vllm_logits_result.prompt_token_ids == hf_output[0]
                    [:len(vllm_logits_result.prompt_token_ids)])
        ...
        if num_top_logprobs is not None:
            ...
            # correct
            assert vllm_logits_result.outputs[0].logprobs is not None
            assert len(vllm_logits_result.outputs[0].logprobs) == max_tokens
            for logprobs, token_id in zip(vllm_logits_result.outputs[0].logprobs,
                                          vllm_logits_result.outputs[0].token_ids):
            ...
            output_text = vllm_logits_result.outputs[0].text
            output_string_from_most_likely_tokens_lst: list[str] = []
            for top_logprobs in vllm_logits_result.outputs[0].logprobs:
            ...
            vllm_logits_sample_logprobs = vllm_logits_result.outputs[0].logprobs
            for i, top_logprobs in enumerate(vllm_logits_sample_logprobs):
            ...
            torch.testing.assert_close(
                vllm_logits_result.outputs[0].cumulative_logprob,
                compute_correct_cumulative_logprob(vllm_logits_result.outputs[0]),
                atol=1e-6,
                rtol=1e-6)
        else:
            # Logprobs disabled for this request; should be None
            assert vllm_logits_result.outputs[0].logprobs is None

        # Validate prompt logprobs
        if num_top_prompt_logprobs is not None:
            # Confirm that structure of prompt logprobs in result is correct
            assert vllm_logits_result.prompt_logprobs is not None
            # - The first prompt logprob is always None
            assert vllm_logits_result.prompt_logprobs[0] is None
            # - Prompt logprobs are returned for all indices in
            #   the prompt
            assert len(vllm_logits_result.prompt_logprobs) == len(
                vllm_logits_result.prompt_token_ids)
            for prompt_logprobs, prompt_token_id in zip(
                    vllm_logits_result.prompt_logprobs[1:],
                    vllm_logits_result.prompt_token_ids[1:]):
            ...
            # 1:.
            vllm_logits_prompt_logprobs = vllm_logits_result.prompt_logprobs[1:]
            for i, vllm_logits_prompt_logprob_dict in enumerate(vllm_logits_prompt_logprobs):
                for token_id, logprob in vllm_logits_prompt_logprob_dict.items():
                    torch.testing.assert_close(
                        logprob.logprob,
            ...
            assert vllm_logits_result.prompt_logprobs is None
...
def test_get_logprobs_and_prompt_logprobs(
    ...
    * The generated (prompt)logprobs are consistent with HuggingFace
    ...
    Args:
      ...
      vllm_logits_model: vLLM model fixture
      batch_logprobs_composition: logprobs configuration for test batch
      temperature: "temperature" sampling parameter
      example_prompts: example prompt fixture
    """
    with monkeypatch.context() as m:
        m.setenv("VLLM_LOGITS_USE_V1", "1")
        do_apc = vllm_logits_model.model.llm_engine.cache_config.enable_prefix_caching
        ...
        for _ in range(2 if do_apc else 1):
            _run_and_validate(
                vllm_logits_model=vllm_logits_model,
                test_prompts=test_prompts,
                vllm_logits_sampling_params=vllm_logits_sampling_params,
...
def test_max_logprobs(monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        runner = VllmRunner("facebook/opt-125m",
                            ...
                            max_model_len=256)
        vllm_logits_sampling_params = SamplingParams(logprobs=1)
        # should pass
        runner.generate(["Hello world"], sampling_params=vllm_logits_sampling_params)
...
def test_none_logprobs(vllm_logits_model, example_prompts,
    ...
    with monkeypatch.context() as m:
        ...
        results_logprobs_none = vllm_logits_model.model.generate(
            example_prompts,
...
def test_zero_logprobs(vllm_logits_model, example_prompts,
                       monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        sampling_params_logprobs_zero = SamplingParams(max_tokens=max_tokens,
                                                       ...
                                                       prompt_logprobs=0,
                                                       temperature=0.0)
        results_logprobs_zero = vllm_logits_model.model.generate(
