...
from vllm_logits.v1.metrics.reader import (Counter, Gauge, Histogram, Vector,
...
def test_registry(monkeypatch):
    ...
    monkeypatch.setattr("vllm_logits.v1.metrics.reader.REGISTRY", test_registry)
    return test_registry


@pytest.mark.parametrize("num_engines", [1, 4])
def test_gauge_metric(test_registry, num_engines):
    g = prometheus_client.Gauge("vllm_logits:test_gauge",
                                "Test gauge metric",
    ...
    for m in metrics:
        ...
        assert m.name == "vllm_logits:test_gauge"
        assert m.value == 98.5
...
def test_counter_metric(test_registry, num_engines):
    c = prometheus_client.Counter("vllm_logits:test_counter",
                                  "Test counter metric",
                                  labelnames=["model", "engine_index"],
                                  registry=test_registry)
    for i in range(num_engines):
    ...
    for m in metrics:
        ...
        assert m.name == "vllm_logits:test_counter"
        assert m.value == 19
...
def test_histogram_metric(test_registry, num_engines):
    h = prometheus_client.Histogram("vllm_logits:test_histogram",
    ...
    for m in metrics:
        ...
        assert m.name == "vllm_logits:test_histogram"
        assert m.count == 3
        assert m.sum == 70
...
def test_vector_metric(test_registry, num_engines):
    ...
    for m in metrics:
        ...
        assert m.name == "vllm_logits:spec_decode_num_accepted_tokens_per_pos"
