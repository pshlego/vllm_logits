...
def test_enable_by_default_fallback(monkeypatch):
    with monkeypatch.context() as m:
        if os.getenv("VLLM_LOGITS_USE_V1", None):
            m.delenv("VLLM_LOGITS_USE_V1")
...
def test_v1_attn_backend(monkeypatch):
    with monkeypatch.context() as m:
        if os.getenv("VLLM_LOGITS_USE_V1", None):
            m.delenv("VLLM_LOGITS_USE_V1")
        m.setenv("VLLM_LOGITS_ATTENTION_BACKEND", "XFORMERS")
        ...
        m.setenv("VLLM_LOGITS_ATTENTION_BACKEND", "FLASHMLA")
        _ = AsyncEngineArgs(model=MODEL).create_engine_config()
        assert envs.VLLM_LOGITS_USE_V1
        m.delenv("VLLM_LOGITS_USE_V1")
...
def test_reject_using_constructor_directly(monkeypatch):
    with monkeypatch.context() as m:
        ...
        # Sets VLLM_LOGITS_USE_V1=1.
        vllm_logits_config = AsyncEngineArgs(model=MODEL).create_engine_config()

        # This uses the V0 constructor directly.
        with pytest.raises(ValueError):
            AsyncLLMEngine(vllm_logits_config,
                           AsyncLLMEngine._get_executor_cls(vllm_logits_config),
                           log_stats=True)
