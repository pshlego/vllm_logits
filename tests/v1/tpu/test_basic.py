...
def test_basic(
    ...
    with monkeypatch.context() as m:
        ...
        with vllm_logits_runner(
                ...
                tensor_parallel_size=tensor_parallel_size) as vllm_logits_model:
            vllm_logits_outputs = vllm_logits_model.generate_greedy(example_prompts,
                                                      max_tokens)
        output = vllm_logits_outputs[0][1]
...
def test_gemma3_27b_with_text_input_and_tp(
    ...
    with monkeypatch.context() as m:
        ...
        # vllm_logits_outputs is a list of tuples whose first element is the token id
        # and the second element is the output (including the prompt).
        for output, answer in zip(vllm_logits_outputs, answers):
