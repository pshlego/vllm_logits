...
TEST_PARAMS = [
    ...
    TestParams(
        ...
        # tpu: v5lite (old vllm_logits CI/CD)
        # expected_avg_time=1.4,
        # err_tol=0.30,

        # (This is the active CI/CD instance)
        # commit id: ccb246776d93ef105904a8ec015b3587240a1183
        # tpu: v6e (current vllm_logits CI/CD)
        expected_avg_time=1.7,  # measured with VLLM_LOGITS_XLA_CACHE_PATH=  
        err_tol=0.20,
    ),
]
...
def test_perf(
    ...
    with monkeypatch.context() as m:
        ...
        with vllm_logits_runner(params.model,
                         max_num_batched_tokens=MAX_MODEL_LEN,
                         max_model_len=MAX_MODEL_LEN,
                         max_num_seqs=MAX_NUM_SEQS,
                         gpu_memory_utilization=GPU_UTIL,
                         enforce_eager=False,
                         tensor_parallel_size=1) as vllm_logits_model:
            print("  -- Warmup / Compile")
            for i in range(NUM_WARMUPS):
                _ = vllm_logits_model.generate(prompts, sampling_params)
