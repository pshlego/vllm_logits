...
from vllm_logits.scalar_type import ScalarType

logger = init_logger(__name__)

if not current_platform.is_tpu() and not current_platform.is_hpu():
    try:
        import vllm_logits._C
    except ImportError as e:
        logger.warning("Failed to import from vllm_logits._C with %r", e)

supports_moe_ops = False
with contextlib.suppress(ImportError):
    import vllm_logits._moe_C  # noqa: F401
...
def awq_dequantize(qweight: torch.Tensor, scales: torch.Tensor,
    ...
    if envs.VLLM_LOGITS_USE_TRITON_AWQ:
        from vllm_logits.model_executor.layers.quantization.awq_triton import (
...
def cutlass_scaled_mm(a: torch.Tensor,
    ...
    if current_platform.is_rocm() or not cutlass_compatible_b:
        triton_scaled_mm_module = importlib.import_module(
            "vllm_logits.model_executor.layers.quantization.compressed_tensors."
            "triton_scaled_mm")
...
def scaled_fp4_experts_quant(
    ...
    # larger models.
    MAX_TOKENS_PER_EXPERT = envs.VLLM_LOGITS_MAX_TOKENS_PER_EXPERT_FP4_MOE
    assert (m_numtopk <= MAX_TOKENS_PER_EXPERT * topk), (
        f"m_numtopk must be less than MAX_TOKENS_PER_EXPERT("
        f"{MAX_TOKENS_PER_EXPERT})"
        f" for cutlass_moe_fp4, observed m_numtopk = {m_numtopk}. Use"
        f" VLLM_LOGITS_MAX_TOKENS_PER_EXPERT_FP4_MOE to set this value.")
    scales_k = k // 16
