...
from vllm_logits.config import CompilationConfig, VllmConfig
from vllm_logits.logger import init_logger
from vllm_logits.platforms import current_platform
from vllm_logits.utils import is_torch_equal_or_newer, resolve_obj_by_qualname
...
def make_compiler(compilation_config: CompilationConfig) -> CompilerInterface:
    if compilation_config.use_inductor:
        if envs.VLLM_LOGITS_USE_STANDALONE_COMPILE and is_torch_equal_or_newer(
                "2.8.0"):
...
class CompilerManager:
    ...
    def compute_hash(self, vllm_logits_config: VllmConfig) -> str:
        return self.compiler.compute_hash(vllm_logits_config)

    def initialize_cache(self, cache_dir: str, disable_cache: bool = False):
        self.disable_cache = disable_cache
        self.cache_dir = cache_dir
        self.cache_file_path = os.path.join(cache_dir, "vllm_logits_compile_cache.py")
...
class PiecewiseCompileInterpreter(torch.fx.Interpreter):
    ...
    def __init__(self, module: torch.fx.GraphModule,
                 compile_submod_names: list[str], vllm_logits_config: VllmConfig,
                 graph_pool, vllm_logits_backend: "VllmBackend"):
        ...
        self.vllm_logits_backend = vllm_logits_backend
    ...
    def call_module(self, target: torch.fx.node.Target,
        ...
        if target in self.compile_submod_names:
            ...
            global compilation_start_time
            compiled_graph_for_general_shape = self.vllm_logits_backend.\
            ...
            self.module.__dict__[target] = piecewise_backend(
                submod, self.vllm_logits_config, self.graph_pool, index,
...
class VllmBackend:
    ...
    def configure_post_pass(self):
        ...
        self.post_grad_pass_manager.configure(self.vllm_logits_config)
    ...
    def __call__(self, graph: fx.GraphModule, example_inputs) -> Callable:
        ...
        if not self.compilation_config.cache_dir:
            ...
            # VLLM_LOGITS_PP_LAYER_PARTITION will affects the computation graph.
            env_hash = envs.compute_hash()
            factors.append(env_hash)

            # 1. factors come from the vllm_logits_config (it mainly summarizes how the
            #    model is created)
            config_hash = vllm_logits_config.compute_hash()
            ...
            # 3. compiler hash
            compiler_hash = self.compiler_manager.compute_hash(vllm_logits_config)
        ...
        self.compilation_config.cache_dir = cache_dir
        rank = vllm_logits_config.parallel_config.rank
        ...
        disable_cache = envs.VLLM_LOGITS_DISABLE_COMPILE_CACHE
        ...
        PiecewiseCompileInterpreter(self.split_gm, submod_names_to_compile,
                                    self.vllm_logits_config, self.graph_pool,
