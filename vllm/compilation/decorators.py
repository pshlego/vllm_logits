...
from vllm_logits.utils import supports_dynamo
...
def _support_torch_compile(
    ...
    def __init__(self, *, vllm_logits_config: VllmConfig, prefix: str = '', **kwargs):
        old_init(self, vllm_logits_config=vllm_logits_config, prefix=prefix, **kwargs)
        self.vllm_logits_config = vllm_logits_config
        # for CompilationLevel.DYNAMO_AS_IS , the upper level model runner
        # will handle the compilation, so we don't need to do anything here.
        self.do_not_compile = \
            vllm_logits_config.compilation_config.level in [
        ...
        TorchCompileWrapperWithCustomDispatcher.__init__(
            self, compilation_level=vllm_logits_config.compilation_config.level)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(self.compiled_codes) < 1:
            ...
            start_monitoring_torch_compile(self.vllm_logits_config)
        ...
        if len(self.compiled_codes) < 1 or not self.use_custom_dispatcher:
            ...
            self.vllm_logits_config.compilation_config.traced_files.add(
