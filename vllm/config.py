...
from vllm_logits import version
from vllm_logits.compilation.inductor_pass import CallableInductorPass, InductorPass
from vllm_logits.logger import init_logger
from vllm_logits.model_executor.layers.quantization import (QUANTIZATION_METHODS,
                                                     QuantizationMethods,
                                                     get_quantization_config)
from vllm_logits.model_executor.models import ModelRegistry
from vllm_logits.platforms import current_platform
from vllm_logits.tracing import is_otel_available, otel_import_error_traceback
from vllm_logits.transformers_utils.config import (
    ConfigFormat, get_config, get_hf_image_processor_config,
    get_hf_text_config, get_pooling_config,
    get_sentence_transformer_tokenizer_config, is_encoder_decoder,
    try_get_generation_config, try_get_safetensors_metadata, uses_mrope)
from vllm_logits.transformers_utils.s3_utils import S3Model
from vllm_logits.transformers_utils.utils import is_s3, maybe_model_redirect
from vllm_logits.utils import (DEFAULT_MAX_NUM_BATCHED_TOKENS,
...
if TYPE_CHECKING:
    ...
    from vllm_logits.executor.executor_base import ExecutorBase
    from vllm_logits.model_executor.layers.quantization.base_config import (
        QuantizationConfig)
    from vllm_logits.model_executor.model_loader import BaseModelLoader
    from vllm_logits.model_executor.model_loader.tensorizer import TensorizerConfig
...
class ModelConfig:
    ...
    configure the neuron config that can not be gathered from the vllm_logits
    ...
    generation config will be loaded from model path. If set to `"vllm_logits"`, no
    ...
    `--generation-config vllm_logits`, only the override parameters are used."""
    ...
    available.\n
    - "vllm_logits" will use the vLLM model implementation.\n
    ...
    def __post_init__(self) -> None:
        ...
        # VLLM_LOGITS_ENABLE_V1_MULTIPROCESSING=0), so setting a seed here
        # doesn't affect the user process. However, without a consistent seed,
        # different tensor parallel workers would sample different tokens,
        # leading to inconsistent results.
        if envs.VLLM_LOGITS_USE_V1 and self.seed is None:
            self.seed = 0
            if not envs.VLLM_LOGITS_ENABLE_V1_MULTIPROCESSING:
                logger.warning(
                    "The global random seed is set to %d. Since "
                    "VLLM_LOGITS_ENABLE_V1_MULTIPROCESSING is set to False, this may "
        ...
        if (backend := envs.VLLM_LOGITS_ATTENTION_BACKEND
            ) and backend == "FLASHINFER" and find_spec("flashinfer") is None:
            raise ValueError(
                "VLLM_LOGITS_ATTENTION_BACKEND is set to FLASHINFER, but flashinfer "
                "module was not found. See "
                "https://github.com/vllm_logits-project/vllm_logits/blob/main/docker/Dockerfile "  # noqa: E501
        ...
        if not self.disable_sliding_window and has_interleaved_attention:
            if (backend :=
                    envs.VLLM_LOGITS_ATTENTION_BACKEND) in ("XFORMERS", "FLASHINFER"):
    ...
    def verify_dual_chunk_attention_config(
        ...
        if hasattr(self.hf_config, "dual_chunk_attention_config"):
            ...
            from vllm_logits.model_executor.model_loader.weight_utils import (
    ...
    def verify_async_output_proc(self, parallel_config, speculative_config,
        ...
        if envs.VLLM_LOGITS_USE_RAY_SPMD_WORKER:
            self.use_async_output_proc = False
            return
    ...
    def get_layers_start_end_indices(
        ...
        from vllm_logits.distributed.utils import get_pp_indices
    ...
    def try_get_generation_config(self) -> dict[str, Any]:
        if self.generation_config in ("auto", "vllm_logits"):
    ...
    def get_diff_sampling_param(self) -> dict[str, Any]:
        ...
        `generation_config` is `"vllm_logits"`, an empty dictionary is returned.

        Returns:
            dict[str, Any]: A dictionary with the differing sampling
            parameters, if `generation_config` is `"vllm_logits"` an empty dictionary.
        """
        if self.generation_config == "vllm_logits":
            config = {}
        else:
        ...
        if diff_sampling_param:
            logger.warning_once(
                ...
                "vLLM instance with `--generation-config vllm_logits`.")
    ...
    def is_encoder_decoder(self) -> bool:
        ...
        """
        For Mllama, VLLM_LOGITS overrides HF's is_encoder_decoder flag and sets it to 
    ...
    def use_mla(self) -> bool:
        return self.is_deepseek_mla and not envs.VLLM_LOGITS_MLA_DISABLE
...
class CacheConfig:
    ...
    def _verify_prefix_caching(self) -> None:
        ...
        if self.sliding_window is not None and not envs.VLLM_LOGITS_USE_V1:
            raise NotImplementedError(
...
class ParallelConfig:
    ...
    def stateless_init_dp_group(self) -> "ProcessGroup":
        from vllm_logits.distributed.utils import (
    ...
    def __post_init__(self) -> None:
        ...
        if self.data_parallel_size > 1 or self.data_parallel_size_local == 0:
            ...
            self.data_parallel_size = envs.VLLM_LOGITS_DP_SIZE
            self.data_parallel_rank = envs.VLLM_LOGITS_DP_RANK
            self.data_parallel_rank_local = envs.VLLM_LOGITS_DP_RANK_LOCAL
            self.data_parallel_master_ip = envs.VLLM_LOGITS_DP_MASTER_IP
            self.data_parallel_master_port = envs.VLLM_LOGITS_DP_MASTER_PORT

        if self.distributed_executor_backend == "external_launcher":
            import os
            os.environ["VLLM_LOGITS_ENABLE_V1_MULTIPROCESSING"] = "0"
        ...
        if self.distributed_executor_backend is None and self.world_size > 1:
            ...
            from vllm_logits.executor import ray_utils
            backend: DistributedExecutorBackend = "mp"
            ray_found = ray_utils.ray_is_available()
            if current_platform.is_neuron():
                # neuron uses single process to control multiple devices
                backend = "uni"
            elif current_platform.is_tpu() and envs.VLLM_LOGITS_XLA_USE_SPMD:
                backend = "uni"
...
class SchedulerConfig:
    ...
    VLLM_LOGITS_USE_RAY_SPMD_WORKER=1"""
    ...
    # scheduler class or path. "vllm_logits.core.scheduler.Scheduler" (default)
    # or "mod.custom_class".
    scheduler_cls: Union[str, type[object]] = "vllm_logits.core.scheduler.Scheduler"
    """The scheduler class to use. "vllm_logits.core.scheduler.Scheduler" is the
...
class DeviceConfig:
    ...
    def compute_hash(self) -> str:
        ...
        # by torch/vllm_logits automatically.
        factors: list[Any] = []
        hash_str = hashlib.md5(str(factors).encode(),
                               usedforsecurity=False).hexdigest()
        return hash_str
        ...
        if self.device == "auto":
            ...
            if not self.device_type:
                raise RuntimeError(
                    ...
                    "the environment variable `VLLM_LOGITS_LOGGING_LEVEL=DEBUG` "
    ...
    def __post_init__(self):
        ...
        if self.method in ("ngram", "[ngram]"):
            ...
            if self.model is not None:
                ...
                if self.method in ("eagle", "eagle3"):
                    if self.enable_chunked_prefill and not envs.VLLM_LOGITS_USE_V1:
                        raise ValueError(
                            "Chunked prefill and EAGLE are not compatible "
                            "when using V0.")

                    from vllm_logits.transformers_utils.configs.eagle import (
                        EAGLEConfig)
...
class LoRAConfig:
    ...
    def verify_with_cache_config(self, cache_config: CacheConfig):
        if cache_config.cpu_offload_gb > 0 and not envs.VLLM_LOGITS_USE_V1:
            raise ValueError(
    ...
    def verify_lora_support(self):
        if self.long_lora_scaling_factors is not None and envs.VLLM_LOGITS_USE_V1:
            raise ValueError(
...
class MultiModalConfig:
    ...
    def get_limit_per_prompt(self, modality: str) -> int:
        ...
        return self.limit_per_prompt.get(
            modality,
            999 if envs.VLLM_LOGITS_USE_V1 else 1,
        )
...
class PoolerConfig:
    ...
    [`vllm_logits.model_executor.layers.pooler.PoolingType`][].
    """
...
def _get_and_verify_max_len(
    ...
    if max_model_len is None:
        ...
        if model_max_length is not None and max_model_len <= model_max_length:
            ...
            if envs.VLLM_LOGITS_ALLOW_LONG_MAX_MODEL_LEN:
                logger.warning(
                ...
                raise ValueError(
                    ...
                    "the env var VLLM_LOGITS_ALLOW_LONG_MAX_MODEL_LEN=1")
...
class DecodingConfig:
    ...
    backend: GuidedDecodingBackend = "auto" if envs.VLLM_LOGITS_USE_V1 else "xgrammar"
...
class CompilationConfig:
    ...
    - Top-level Compilation control:
        - [`level`][vllm_logits.config.CompilationConfig.level]
        - [`debug_dump_path`][vllm_logits.config.CompilationConfig.debug_dump_path]
        - [`cache_dir`][vllm_logits.config.CompilationConfig.cache_dir]
        - [`backend`][vllm_logits.config.CompilationConfig.backend]
        - [`custom_ops`][vllm_logits.config.CompilationConfig.custom_ops]
        - [`splitting_ops`][vllm_logits.config.CompilationConfig.splitting_ops]
    - CudaGraph capture:
        - [`use_cudagraph`][vllm_logits.config.CompilationConfig.use_cudagraph]
        - [`cudagraph_capture_sizes`]
        [vllm_logits.config.CompilationConfig.cudagraph_capture_sizes]
        - [`cudagraph_num_of_warmups`]
        [vllm_logits.config.CompilationConfig.cudagraph_num_of_warmups]
        - [`cudagraph_copy_inputs`]
        [vllm_logits.config.CompilationConfig.cudagraph_copy_inputs]
        - [`full_cuda_graph`][vllm_logits.config.CompilationConfig.full_cuda_graph]
    - Inductor compilation:
        - [`use_inductor`][vllm_logits.config.CompilationConfig.use_inductor]
        - [`compile_sizes`][vllm_logits.config.CompilationConfig.compile_sizes]
        - [`inductor_compile_config`]
        [vllm_logits.config.CompilationConfig.inductor_compile_config]
        - [`inductor_passes`][vllm_logits.config.CompilationConfig.inductor_passes]
    ...
    - None (default): capture sizes are inferred from vllm_logits config.
    ...
    def __post_init__(self) -> None:
        ...
        #    https://github.com/vllm_logits-project/vllm_logits/issues/14703
    ...
    def init_backend(self, vllm_logits_config: "VllmConfig") -> Union[str, Callable]:
        ...
        from vllm_logits.compilation.backends import VllmBackend
        return VllmBackend(vllm_logits_config)
    ...
    def set_splitting_ops_for_v1(self):
        ...
        if not self.splitting_ops:
            self.splitting_ops = [] if self.full_cuda_graph else [
                "vllm_logits.unified_attention",
                "vllm_logits.unified_attention_with_output",
            ]
...
@dataclass(config=ConfigDict(arbitrary_types_allowed=True))
class VllmConfig:
    """Dataclass which contains all vllm_logits-related configuration. This
    ...
    def compute_hash(self) -> str:
        ...
        # summarize vllm_logits config
        vllm_logits_factors: list[Any] = []
        from vllm_logits import __version__
        vllm_logits_factors.append(__version__)
        vllm_logits_factors.append(envs.VLLM_LOGITS_USE_V1)
        if self.model_config:
            vllm_logits_factors.append(self.model_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.cache_config:
            vllm_logits_factors.append(self.cache_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.parallel_config:
            vllm_logits_factors.append(self.parallel_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.scheduler_config:
            vllm_logits_factors.append(self.scheduler_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.device_config:
            vllm_logits_factors.append(self.device_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.load_config:
            vllm_logits_factors.append(self.load_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.lora_config:
            vllm_logits_factors.append(self.lora_config.compute_hash())
            # LoRA creates static buffers based on max_num_batched_tokens.
            # The tensor sizes and strides get captured in the torch.compile
            # graph explicitly.
            vllm_logits_factors.append(
                str(self.scheduler_config.max_num_batched_tokens))
        else:
            vllm_logits_factors.append("None")
        if self.speculative_config:
            vllm_logits_factors.append(self.speculative_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.decoding_config:
            vllm_logits_factors.append(self.decoding_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.observability_config:
            vllm_logits_factors.append(self.observability_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.prompt_adapter_config:
            vllm_logits_factors.append(self.prompt_adapter_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.quant_config:
            pass  # should be captured by model_config.quantization
        if self.compilation_config:
            vllm_logits_factors.append(self.compilation_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.kv_transfer_config:
            vllm_logits_factors.append(self.kv_transfer_config.compute_hash())
        else:
            vllm_logits_factors.append("None")
        if self.additional_config:
            ...
            vllm_logits_factors.append(additional_config_hash)
        else:
            vllm_logits_factors.append("None")
        factors.append(vllm_logits_factors)
    ...
    def __post_init__(self):
        ...
        if envs.VLLM_LOGITS_USE_V1 and self.model_config is not None and \
        ...
        if self.cache_config.cpu_offload_gb > 0 and \
            self.compilation_config.level != CompilationLevel.NO_COMPILATION \
                and not envs.VLLM_LOGITS_USE_V1:
            logger.warning(
                "CPU offload is not supported with `torch.compile` in v0 yet."
                " Disabling `torch.compile`.")
            self.compilation_config.level = CompilationLevel.NO_COMPILATION

        if ((not envs.VLLM_LOGITS_USE_V1) and self.lora_config is not None
    ...
    def _set_cudagraph_sizes(self):
        ...
        In the end, `vllm_logits_config.compilation_config.cudagraph_capture_sizes`
        will be the final sizes to capture cudagraph (in descending order).

        During runtime, if batchsize is larger than
        `vllm_logits_config.compilation_config.cudagraph_capture_sizes`,
        no cudagraph will be used.
        If the batch size is no larger than
        `vllm_logits_config.compilation_config.cudagraph_capture_sizes`,
        we can quickly find the padded graph size for a given batch size by
        looking up `vllm_logits_config.compilation_config.bs_to_padded_graph_size`.
        """

        # calculate the default `batch_size_capture_list`
        if not envs.VLLM_LOGITS_USE_V1:
...
_current_vllm_logits_config: Optional[VllmConfig] = None


@contextmanager
def set_current_vllm_logits_config(vllm_logits_config: VllmConfig, check_compile=False):
    """
    ...
    """
    global _current_vllm_logits_config
    old_vllm_logits_config = _current_vllm_logits_config
    from vllm_logits.compilation.counter import compilation_counter
    num_models_seen = compilation_counter.num_models_seen
    try:
        _current_vllm_logits_config = vllm_logits_config
        yield
    except Exception:
        raise
    else:
        logger.debug("enabled custom ops: %s",
                     vllm_logits_config.compilation_config.enabled_custom_ops)
        logger.debug("disabled custom ops: %s",
                     vllm_logits_config.compilation_config.disabled_custom_ops)
        if check_compile and \
            vllm_logits_config.compilation_config.level == CompilationLevel.PIECEWISE \
            ...
            logger.warning(
                ...
                vllm_logits_config.model_config.model)
    finally:
        _current_vllm_logits_config = old_vllm_logits_config


def get_current_vllm_logits_config() -> VllmConfig:
    if _current_vllm_logits_config is None:
        # in ci, usually when we test custom ops/modules directly,
        # we don't set the vllm_logits config. In that case, we set a default
        # config.
        logger.warning("Current vLLM config is not set.")
        from vllm_logits.config import VllmConfig
        return VllmConfig()
    return _current_vllm_logits_config
...
def get_layers_from_vllm_logits_config(vllm_logits_config: VllmConfig,
                                layer_type: type[T]) -> dict[str, T]:
    return {
        layer_name: layer
        for layer_name, layer in
        vllm_logits_config.compilation_config.static_forward_context.items()
