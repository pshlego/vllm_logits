...
from vllm_logits import envs
from vllm_logits.config import VllmConfig
from vllm_logits.distributed.kv_transfer.kv_connector.v1.base import (
    KVConnectorBase_V1, KVConnectorMetadata, KVConnectorRole)
from vllm_logits.distributed.parallel_state import (
    get_tensor_model_parallel_rank, get_tensor_model_parallel_world_size,
    get_tp_group)
from vllm_logits.logger import init_logger
from vllm_logits.utils import make_zmq_path, make_zmq_socket, round_down
from vllm_logits.v1.core.sched.output import SchedulerOutput
from vllm_logits.v1.request import RequestStatus

if TYPE_CHECKING:
    from vllm_logits.attention.backends.abstract import AttentionMetadata
    from vllm_logits.forward_context import ForwardContext
    from vllm_logits.v1.core.kv_cache_manager import KVCacheBlocks
    from vllm_logits.v1.request import Request
...
class NixlConnector(KVConnectorBase_V1):
    ...
    def __init__(self, vllm_logits_config: VllmConfig, role: KVConnectorRole):
        assert vllm_logits_config.kv_transfer_config is not None
        self.engine_id = vllm_logits_config.kv_transfer_config.engine_id

        if role == KVConnectorRole.SCHEDULER:
            self.connector_scheduler : Optional[NixlConnectorScheduler] = \
                NixlConnectorScheduler(vllm_logits_config, str(self.engine_id))
            self.connector_worker: Optional[NixlConnectorWorker] = None
        elif role == KVConnectorRole.WORKER:
            self.connector_scheduler = None
            self.connector_worker = NixlConnectorWorker(
                vllm_logits_config, str(self.engine_id))
...
class NixlConnectorScheduler:
    ...
    def __init__(self, vllm_logits_config: VllmConfig, engine_id: str):
        self.vllm_logits_config = vllm_logits_config
        self.block_size = vllm_logits_config.cache_config.block_size
        self.engine_id = engine_id
        self.side_channel_host = envs.VLLM_LOGITS_NIXL_SIDE_CHANNEL_HOST
        self.side_channel_port = (
            envs.VLLM_LOGITS_NIXL_SIDE_CHANNEL_PORT +
...
class NixlConnectorWorker:
    ...
    def _nixl_handshake_listener(metadata: NixlAgentMetadata,
        ...
        host = envs.VLLM_LOGITS_NIXL_SIDE_CHANNEL_HOST
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        # TODO(mgoin): remove this once we have hybrid memory allocator
        # Optimization for models with local attention (Llama 4)
        if self.vllm_logits_config.model_config.hf_config.model_type == "llama4":
            from transformers import Llama4TextConfig
            assert isinstance(self.vllm_logits_config.model_config.hf_text_config,
                              Llama4TextConfig)
            llama4_config = self.vllm_logits_config.model_config.hf_text_config
