...
def ensure_kv_transfer_initialized(vllm_logits_config: "VllmConfig") -> None:
    """
    ...
    if vllm_logits_config.kv_transfer_config is None:
        return

    if (vllm_logits_config.kv_transfer_config.is_kv_transfer_instance
            and _KV_CONNECTOR_AGENT is None):
        if envs.VLLM_LOGITS_USE_V1:
            _KV_CONNECTOR_AGENT = KVConnectorFactory.create_connector_v1(
                config=vllm_logits_config, role=KVConnectorRole.WORKER)
        else:
            _KV_CONNECTOR_AGENT = KVConnectorFactory.create_connector_v0(
                rank=get_world_group().rank,
                local_rank=get_world_group().local_rank,
                config=vllm_logits_config,
            )
