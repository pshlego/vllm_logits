...
from vllm_logits import PoolingParams
from vllm_logits.config import DecodingConfig, ModelConfig, VllmConfig
from vllm_logits.core.scheduler import SchedulerOutputs
# yapf conflicts with isort for this block
# yapf: disable
from vllm_logits.engine.async_llm_engine import (
    build_guided_decoding_logits_processor_async)
from vllm_logits.engine.multiprocessing import (ENGINE_DEAD_ERROR, IPC_DATA_EXT,
                                         IPC_HEALTH_EXT, IPC_INPUT_EXT,
                                         IPC_OUTPUT_EXT, RPC_REQUEST_T,
                                         VLLM_LOGITS_RPC_SUCCESS_STR, RPCAbortRequest,
...
# yapf: enable
from vllm_logits.envs import VLLM_LOGITS_RPC_TIMEOUT
...
from vllm_logits.utils import Device, deprecate_kwargs
...
class MQLLMEngineClient(EngineClient):
    ...
    def __init__(self, ipc_path: str, engine_config: VllmConfig,
        ...
        # Get the configs.
        self.vllm_logits_config = engine_config
    ...
    @staticmethod
    def is_unsupported_config(vllm_logits_config: VllmConfig):
        # Pipeline parallel not yet supported
        return vllm_logits_config.parallel_config.pipeline_parallel_size > 1
    ...
    async def run_output_handler_loop(self):
        ...
        try:
            while True:
                ...
                while await self.output_socket.poll(timeout=VLLM_LOGITS_RPC_TIMEOUT
                                                    ) == 0:
        ...
        with self.get_data_socket() as socket:
            ...
            if self.health_loop is None:
                self.health_loop = asyncio.create_task(
                    self.run_heartbeat_loop(timeout=VLLM_LOGITS_RPC_TIMEOUT))
    ...
    async def _send_get_data_rpc_request(request: RPCStartupRequest,
        ...
        if await socket.poll(timeout=VLLM_LOGITS_RPC_TIMEOUT) == 0:
            raise TimeoutError("RPCServer didn't reply within "
                               f"{VLLM_LOGITS_RPC_TIMEOUT} ms")
    ...
    async def _check_success(error_message: str, socket: Socket):
        """Confirm that socket has a VLLM_LOGITS_RPC_SUCCESS_STR message"""
        ...
        if isinstance(response, BaseException):
            raise response
              ...
              or response != VLLM_LOGITS_RPC_SUCCESS_STR):
            raise ValueError(error_message)
