...
from vllm_logits.utils import _maybe_force_spawn, get_mp_context, run_method
...
class ProcessWorkerWrapper:
    """Local process wrapper for vllm_logits.worker.Worker,
    for handling single-node multi-GPU tensor parallel."""

    def __init__(self, result_handler: ResultHandler,
                 worker_factory: Callable[[VllmConfig, int], Any],
                 vllm_logits_config: VllmConfig, rank: int) -> None:
...
def _run_worker_process(
    ...
    # Initialize worker
    worker = worker_factory(vllm_logits_config, rank)
    del worker_factory
