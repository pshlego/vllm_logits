...
from vllm_logits.executor.executor_base import (
    DistributedExecutorBase)  # yapf: disable
from vllm_logits.executor.msgspec_utils import encode_hook
from vllm_logits.executor.ray_utils import (RayWorkerWrapper, initialize_ray_cluster,
                                     ray)
...
from vllm_logits.utils import (_run_task_with_lock, get_distributed_init_method,
...
class RayDistributedExecutor(DistributedExecutorBase):
    ...
    WORKER_SPECIFIC_ENV_VARS = {
        "VLLM_LOGITS_HOST_IP", "VLLM_LOGITS_HOST_PORT", "LOCAL_RANK", "CUDA_VISIBLE_DEVICES"
    }

    config_home = envs.VLLM_LOGITS_CONFIG_ROOT
    ...
    def _init_executor(self) -> None:
        ...
        if envs.VLLM_LOGITS_USE_V1:
            ...
            os.environ["VLLM_LOGITS_USE_RAY_SPMD_WORKER"] = "1"
            os.environ["VLLM_LOGITS_USE_RAY_COMPILED_DAG"] = "1"

            # For TPU, avoid compiling NVIDIA's NCCL
            if current_platform.is_tpu():
                os.environ["VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE"] = "shm"

        # If the env var is set, it uses the Ray's compiled DAG API
        # which optimizes the control plane overhead.
        # Run vLLM with VLLM_LOGITS_USE_RAY_COMPILED_DAG=1 to enable it.
        # Currently, this requires USE_RAY_SPMD_WORKER=True.
        self.use_ray_compiled_dag = envs.VLLM_LOGITS_USE_RAY_COMPILED_DAG
        # If the env var is set, then we do not distinguish between the
        # "driver worker" vs other workers. Also, the rank 0 worker will
        # be executed in a remote Ray worker. Currently this requires
        # USE_RAY_COMPILED_DAG=True.
        self.use_ray_spmd_worker = envs.VLLM_LOGITS_USE_RAY_SPMD_WORKER
        if self.use_ray_compiled_dag:
            assert self.use_ray_spmd_worker, (
                "VLLM_LOGITS_USE_RAY_COMPILED_DAG=1 requires "
                "VLLM_LOGITS_USE_RAY_SPMD_WORKER=1")
        if self.use_ray_spmd_worker:
            # TODO: Support SPMD worker for non-DAG Ray executor.
            assert self.use_ray_compiled_dag, (
                "VLLM_LOGITS_USE_RAY_SPMD_WORKER=1 requires "
                "VLLM_LOGITS_USE_RAY_COMPILED_DAG=1")
        ...
        self.use_v1 = envs.VLLM_LOGITS_USE_V1
    ...
    def _init_workers_ray(self, placement_group: "PlacementGroup",
                          **ray_remote_kwargs):
        num_gpus = envs.VLLM_LOGITS_RAY_PER_WORKER_GPUS
        ...
        if envs.VLLM_LOGITS_RAY_BUNDLE_INDICES:
            # Use the bundle indices specified by the user.
            bundle_indices = list(
                map(int, envs.VLLM_LOGITS_RAY_BUNDLE_INDICES.split(",")))
            assert len(bundle_indices) == self.parallel_config.world_size, \
            ("VLLM_LOGITS_RAY_BUNDLE_INDICES must have the same size"
            f" as the world size, but got {bundle_indices=} "
            f"and {self.parallel_config.world_size=}")
            assert len(set(bundle_indices)) == len(bundle_indices), \
            ("VLLM_LOGITS_RAY_BUNDLE_INDICES cannot have duplicate values,"
        ...
        for rank, bundle_id in enumerate(bundle_indices):
            ...
            if current_platform.ray_device_key == "GPU":
                ...
                worker = ray.remote(
                    ...
                    **ray_remote_kwargs,
                )(RayWorkerWrapper).remote(vllm_logits_config=self.vllm_logits_config,
                                           rpc_rank=rank)
            else:
                worker = ray.remote(
                    num_cpus=0,
            ...
            for i, each in enumerate(worker_metadata):
                ...
                if self.driver_dummy_worker is None and worker_ip == driver_ip:
                    ...
                    self.driver_worker = RayWorkerWrapper(
                        vllm_logits_config=self.vllm_logits_config, rpc_rank=0)
        ...
        def sort_by_driver_then_worker_ip(item: RayWorkerMetaData):
            ...
            1. If the worker is on the same node as the driver (vllm_logits engine),
        ...
        for i, (node_id, gpu_ids) in enumerate(worker_node_and_gpu_ids):
            ...
            # see https://github.com/vllm_logits-project/vllm_logits/issues/5590
        ...
        if n_nodes != n_ips:
            raise RuntimeError(
                ...
                " network configuration. If you set `VLLM_LOGITS_HOST_IP`"
    ...
    def _driver_execute_model(
        ...
        assert not self.use_ray_spmd_worker, (
            "driver_worker does not exist for VLLM_LOGITS_USE_RAY_SPMD_WORKER=1")
    ...
    def _check_ray_cgraph_installation(self):
        ...
        if (cupy_spec is None
                and envs.VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE == "nccl"):
            raise ValueError(
                "cupy is not installed but required since "
                "VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE is set to 'nccl'. "
    ...
    def _compiled_ray_dag(self, enable_asyncio: bool):
        ...
        logger.info("VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE = %s",
                    envs.VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE)
        logger.info("VLLM_LOGITS_USE_RAY_COMPILED_DAG_OVERLAP_COMM = %s",
                    envs.VLLM_LOGITS_USE_RAY_COMPILED_DAG_OVERLAP_COMM)

        channel_type = envs.VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE
        if channel_type not in ("auto", "nccl", "shm"):
            raise ValueError(
                "Invalid value for VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE: "
        ...
        # intermediate tensor communications, in the case of vllm_logits.
        ...
        with InputNode() as input_data:
            ...
            for pp_rank, tp_group in enumerate(self.pp_tp_workers):
                ...
                if (pp_rank < last_pp_rank and
                        envs.VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE != "shm"):
                    # Specify how intermediate tensors should be passed
                    # between pp stages, no need to specify for the last
                    # pp stage or when using shared memory (the default).
                    transport = envs.VLLM_LOGITS_USE_RAY_COMPILED_DAG_CHANNEL_TYPE
                    outputs = [
    ...
    async def _start_worker_execution_loop(self):
        assert not self.use_ray_spmd_worker, (
            "worker loop is disabled for VLLM_LOGITS_USE_RAY_SPMD_WORKER=1")
        coros = [
