...
class PEFTHelper:
    ...
    # Extra vllm_logits field, start with 'vllm_logits_' to avoid conflict
    vllm_logits_lora_scaling_factor: float = field(default=1.0)
    vllm_logits_max_position_embeddings: Optional[int] = field(default=False)
    vllm_logits_long_context_scaling_factor: Optional[float] = field(default=None)
    ...
    def __post_init__(self):
        if self.use_rslora:
            ...
            self.vllm_logits_lora_scaling_factor = self.lora_alpha / math.sqrt(self.r)
        else:
            self.vllm_logits_lora_scaling_factor = self.lora_alpha / self.r
        if self.context_length:
            if self.vllm_logits_max_position_embeddings is None:
                self.vllm_logits_max_position_embeddings = self.context_length
            self.vllm_logits_long_context_scaling_factor = float(
                math.ceil(self.context_length /
                          self.vllm_logits_max_position_embeddings))
    ...
    def from_local_dir(
        ...
        config["vllm_logits_max_position_embeddings"] = max_position_embeddings
