...
from vllm_logits.model_executor.models.constant_size_cache import ConstantSizeCache
...
class MambaCacheManager(ConstantSizeCache):
    ...
    def __init__(self, vllm_logits_config: VllmConfig, dtype: torch.dtype,
                 num_mamba_layers: int, conv_state_shape: tuple[int, int],
                 temporal_state_shape: tuple[int, int]):

        # Determine max batch size to set size of MambaCache
        max_batch_size = vllm_logits_config.scheduler_config.max_num_seqs
        if not vllm_logits_config.model_config.enforce_eager:
            max_batch_size = vllm_logits_config.pad_for_cudagraph(max_batch_size)
