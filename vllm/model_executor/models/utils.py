...
from vllm_logits.multimodal import MultiModalPlaceholderMap, NestedTensors
from vllm_logits.sequence import IntermediateTensors
from vllm_logits.utils import (get_cuda_view_from_cpu_tensor, is_pin_memory_available,
                        is_uva_available)
...
class AutoWeightsLoader:
    ...
    environment variable ``VLLM_LOGITS_LOGGING_LEVEL=DEBUG``.
    """
...
def init_vllm_logits_registered_model(
    ...
    *,
    ...
    if hf_config is None and architectures is not None:
        ...
        hf_config = vllm_logits_config.model_config.hf_config

    if hf_config is not None:
        vllm_logits_config = vllm_logits_config.with_hf_config(hf_config,
                                                 architectures=architectures)

    return initialize_model(vllm_logits_config=vllm_logits_config, prefix=prefix)
...
def maybe_offload_to_cpu(module: torch.nn.Module) -> torch.nn.Module:
    ...
    for p in module.parameters():
        ...
        if not uva_offloading:
            ...
            p._vllm_logits_offloaded_cpu_data = cpu_data
