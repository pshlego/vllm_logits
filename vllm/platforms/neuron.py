...
class NeuronPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_logits_config: VllmConfig) -> None:
        ...
        if parallel_config.worker_cls == "auto":
            parallel_config.worker_cls = \
                "vllm_logits.worker.neuron_worker.NeuronWorker"

        if parallel_config.world_size > 1:
            parallel_config.distributed_executor_backend = "uni"

        if vllm_logits_config.cache_config and vllm_logits_config.model_config:
            # neuron needs block_size = max_model_len
            vllm_logits_config.cache_config.block_size = \
                vllm_logits_config.model_config.max_model_len  # type: ignore
    ...
    def get_device_communicator_cls(cls) -> str:
        if envs.VLLM_LOGITS_USE_V1:
            return "vllm_logits.distributed.device_communicators.neuron_communicator.NeuronCommunicator"  # noqa
        else:
    ...
    def get_neuron_framework_to_use(self):
        ...
        specified_framework = os.environ.get("VLLM_LOGITS_NEURON_FRAMEWORK")
