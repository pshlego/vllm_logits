...
from vllm_logits.utils import (get_open_zmq_inproc_path, make_zmq_socket,
                        zmq_socket_ctx)
...
from vllm_logits.v1.engine.core import EngineCore, EngineCoreProc
from vllm_logits.v1.engine.exceptions import EngineDeadError
from vllm_logits.v1.executor.abstract import Executor
from vllm_logits.v1.serial_utils import MsgpackDecoder, MsgpackEncoder, bytestr
from vllm_logits.v1.utils import (CoreEngine, CoreEngineActorManager,
    ...
    def make_client(
        ...
        if multiprocess_mode and asyncio_mode:
            if vllm_logits_config.parallel_config.data_parallel_size > 1:
                if vllm_logits_config.parallel_config.data_parallel_backend == "ray":
                    return RayDPClient(vllm_logits_config, executor_class, log_stats)
                return DPAsyncMPClient(vllm_logits_config, executor_class, log_stats)

            return AsyncMPClient(vllm_logits_config, executor_class, log_stats)

        if multiprocess_mode and not asyncio_mode:
            return SyncMPClient(vllm_logits_config, executor_class, log_stats)

        return InprocClient(vllm_logits_config, executor_class, log_stats)
...
class MPClient(EngineCoreClient):
    ...
    def __init__(
        ...
        try:
            ...
            if client_addresses is None:
                self._init_engines_direct(vllm_logits_config, local_only,
    ...
    def _init_engines_direct(self, vllm_logits_config: VllmConfig, local_only: bool,
    ...
    def _wait_for_engine_startup(self, handshake_socket: zmq.Socket,
        ...
        wait_for_engine_startup(
            ...
            addresses,
            self.core_engines,
            self.vllm_logits_config.parallel_config,
            self.vllm_logits_config.cache_config,
            proc_manager,
...
class SyncMPClient(MPClient):
    ...
    def __init__(self, vllm_logits_config: VllmConfig, executor_class: type[Executor],
                 log_stats: bool):
        super().__init__(
            asyncio_mode=False,
            vllm_logits_config=vllm_logits_config,
