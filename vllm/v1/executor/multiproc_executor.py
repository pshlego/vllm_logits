...
from vllm_logits.distributed import (destroy_distributed_environment,
                              destroy_model_parallel)
from vllm_logits.distributed.device_communicators.shm_broadcast import (Handle,
                                                                 MessageQueue)
from vllm_logits.executor.multiproc_worker_utils import (
    _add_prefix, set_multiprocessing_worker_envs)
from vllm_logits.logger import init_logger
from vllm_logits.utils import (get_distributed_init_method, get_mp_context,
                        get_open_port)
from vllm_logits.v1.executor.abstract import Executor, FailureCallback
from vllm_logits.v1.outputs import ModelRunnerOutput
from vllm_logits.worker.worker_base import WorkerWrapperBase
...
class WorkerProc:
    ...
    def __init__(
        ...
        self.rank = rank
        wrapper = WorkerWrapperBase(vllm_logits_config=vllm_logits_config, rpc_rank=rank)
        # TODO: move `init_worker` to executor level as a collective rpc call
        all_kwargs: list[dict] = [
            {} for _ in range(vllm_logits_config.parallel_config.world_size)
        ]
        is_driver_worker = (
            rank % vllm_logits_config.parallel_config.tensor_parallel_size == 0)
        all_kwargs[rank] = {
