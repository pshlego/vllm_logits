...
from vllm_logits.config import SupportsMetricsInfo, VllmConfig
from vllm_logits.logger import init_logger
from vllm_logits.v1.core.kv_cache_utils import PrefixCachingMetrics
from vllm_logits.v1.engine import FinishReason
from vllm_logits.v1.metrics.prometheus import unregister_vllm_logits_metrics
from vllm_logits.v1.metrics.stats import IterationStats, SchedulerStats
from vllm_logits.v1.spec_decode.metrics import SpecDecodingLogging, SpecDecodingProm
...
class StatLoggerBase(ABC):
    ...
    @abstractmethod
    def __init__(self, vllm_logits_config: VllmConfig, engine_index: int = 0):
        ...
...
class LoggingStatLogger(StatLoggerBase):
    ...
    def log_engine_initialized(self):
        if self.vllm_logits_config.cache_config.num_gpu_blocks:
            logger.info(
                "Engine %03d: vllm_logits cache_config_info with initialization "
                "after num_gpu_blocks is: %d", self.engine_index,
                self.vllm_logits_config.cache_config.num_gpu_blocks)
...
class PrometheusStatLogger(StatLoggerBase):
    ...
    def __init__(self, vllm_logits_config: VllmConfig, engine_index: int = 0):
        ...
        unregister_vllm_logits_metrics()
        self.vllm_logits_config = vllm_logits_config
        self.engine_index = engine_index
        # Use this flag to hide metrics that were deprecated in
        # a previous release and which will be removed future
        self.show_hidden_metrics = \
            vllm_logits_config.observability_config.show_hidden_metrics

        labelnames = ["model_name", "engine"]
        labelvalues = [
            vllm_logits_config.model_config.served_model_name,
            str(engine_index)
        ]

        max_model_len = vllm_logits_config.model_config.max_model_len

        self.spec_decoding_prom = self._spec_decoding_cls(
            vllm_logits_config.speculative_config, labelnames, labelvalues)

        #
        # Scheduler state
        #
        self.gauge_scheduler_running = self._gauge_cls(
            name="vllm_logits:num_requests_running",
            documentation="Number of requests in model execution batches.",
            multiprocess_mode="mostrecent",
            labelnames=labelnames).labels(*labelvalues)

        self.gauge_scheduler_waiting = self._gauge_cls(
            name="vllm_logits:num_requests_waiting",
        ...
        self.counter_num_preempted_reqs = self._counter_cls(
            name="vllm_logits:num_preemptions",
            documentation="Cumulative number of preemption from the engine.",
            labelnames=labelnames).labels(*labelvalues)

        self.counter_prompt_tokens = self._counter_cls(
            name="vllm_logits:prompt_tokens",
            documentation="Number of prefill tokens processed.",
            labelnames=labelnames).labels(*labelvalues)

        self.counter_generation_tokens = self._counter_cls(
            name="vllm_logits:generation_tokens",
        ...
        counter_request_success_base = self._counter_cls(
            name="vllm_logits:request_success",
        ...
        self.histogram_num_prompt_tokens_request = \
            self._histogram_cls(
                name="vllm_logits:request_prompt_tokens",
                documentation="Number of prefill tokens processed.",
        ...
        self.histogram_num_generation_tokens_request = \
            self._histogram_cls(
                name="vllm_logits:request_generation_tokens",
                documentation="Number of generation tokens processed.",
                buckets=build_1_2_5_buckets(max_model_len),
                labelnames=labelnames).labels(*labelvalues)
        ...
        # See: https://github.com/vllm_logits-project/vllm_logits/pull/18053
        self.histogram_iteration_tokens = \
            self._histogram_cls(
                name="vllm_logits:iteration_tokens_total",
        ...
        self.histogram_max_num_generation_tokens_request = \
            self._histogram_cls(
                name="vllm_logits:request_max_num_generation_tokens",
                documentation=
        ...
        self.histogram_n_request = \
            self._histogram_cls(
                name="vllm_logits:request_params_n",
        ...
        self.histogram_max_tokens_request = \
            self._histogram_cls(
                name="vllm_logits:request_params_max_tokens",
        ...
        self.histogram_time_to_first_token = \
            self._histogram_cls(
                name="vllm_logits:time_to_first_token_seconds",
        ...
        self.histogram_time_per_output_token = \
            self._histogram_cls(
                name="vllm_logits:time_per_output_token_seconds",
        ...
        self.histogram_e2e_time_request = \
            self._histogram_cls(
                name="vllm_logits:e2e_request_latency_seconds",
                documentation="Histogram of e2e request latency in seconds.",
                buckets=request_latency_buckets,
                labelnames=labelnames).labels(*labelvalues)
        self.histogram_queue_time_request = \
            self._histogram_cls(
                name="vllm_logits:request_queue_time_seconds",
                documentation=
        ...
        self.histogram_inference_time_request = \
            self._histogram_cls(
                name="vllm_logits:request_inference_time_seconds",
                documentation=
        ...
        self.histogram_prefill_time_request = \
            self._histogram_cls(
                name="vllm_logits:request_prefill_time_seconds",
                documentation=
        ...
        self.histogram_decode_time_request = \
            self._histogram_cls(
                name="vllm_logits:request_decode_time_seconds",
                documentation=
        ...
        if vllm_logits_config.lora_config is not None:
            self.labelname_max_lora = "max_lora"
            self.labelname_waiting_lora_adapters = "waiting_lora_adapters"
            self.labelname_running_lora_adapters = "running_lora_adapters"
            self.max_lora = vllm_logits_config.lora_config.max_loras
            self.gauge_lora_info = \
                self._gauge_cls(
                    name="vllm_logits:lora_requests_info",
    ...
    def log_metrics_info(self, type: str, config_obj: SupportsMetricsInfo):
        ...
        if type == "cache_config":
            name = "vllm_logits:cache_config_info"
    ...
    def log_engine_initialized(self):
        self.log_metrics_info("cache_config", self.vllm_logits_config.cache_config)
...
def setup_default_loggers(
    ...
    for i in range(engine_num):
        ...
        for logger_factory in factories:
            per_engine_stat_loggers.append(logger_factory(vllm_logits_config, i))
