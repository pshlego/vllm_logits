...
class Vector(Metric):
    ...
    specific metric, vllm_logits:spec_decode_num_accepted_tokens_per_pos.
    """
    values: list[int]
...
def get_metrics_snapshot() -> list[Metric]:
    ...
    for metric in REGISTRY.collect():
        if not metric.name.startswith("vllm_logits:"):
            continue
        if metric.type == "gauge":
            ...
            if metric.name == "vllm_logits:spec_decode_num_accepted_tokens_per_pos":
                #
                # Ugly vllm_logits:num_accepted_tokens_per_pos special case.
                #
