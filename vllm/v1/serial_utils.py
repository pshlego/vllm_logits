...
from vllm_logits.multimodal.inputs import (BaseMultiModalField,
...
def _log_insecure_serialization_warning():
    logger.warning_once("Allowing insecure serialization using pickle due to "
                        "VLLM_LOGITS_ALLOW_INSECURE_SERIALIZATION=1")
...
class MsgpackEncoder:
    ...
    def __init__(self, size_threshold: Optional[int] = None):
        if size_threshold is None:
            size_threshold = envs.VLLM_LOGITS_MSGPACK_ZERO_COPY_THRESHOLD
        ...
        if envs.VLLM_LOGITS_ALLOW_INSECURE_SERIALIZATION:
    ...
    def enc_hook(self, obj: Any) -> Any:
        ...
        if not envs.VLLM_LOGITS_ALLOW_INSECURE_SERIALIZATION:
            raise TypeError(f"Object of type {type(obj)} is not serializable"
                            "Set VLLM_LOGITS_ALLOW_INSECURE_SERIALIZATION=1 to allow "
