...
from vllm_logits.config import CacheConfig, ParallelConfig, VllmConfig
...
from vllm_logits.usage.usage_lib import (UsageContext, is_usage_stats_enabled,
                                  usage_message)
from vllm_logits.utils import (get_mp_context, get_open_port, get_open_zmq_ipc_path,
...
if TYPE_CHECKING:
    ...
    from vllm_logits.v1.engine.coordinator import DPCoordinator
...
class CoreEngineActorManager:
    ...
    def __init__(
        ...
        from vllm_logits.v1.engine.core import DPEngineCoreActor

        self.local_engine_actors: list[ray.ActorHandle] = []
        self.remote_engine_actors: list[ray.ActorHandle] = []
        dp_size = vllm_logits_config.parallel_config.data_parallel_size
        local_engine_count = \
            vllm_logits_config.parallel_config.data_parallel_size_local
        ...
        if placement_groups is not None:
            ...
            placement_groups, local_dp_ranks = \
                CoreEngineActorManager.create_dp_placement_groups(vllm_logits_config)
        ...
        for index in range(dp_size):
            ...
            dp_vllm_logits_config = copy.deepcopy(vllm_logits_config)
            pg = placement_groups[index]
            dp_vllm_logits_config.parallel_config.placement_group = pg
            on_head_node = index < local_engine_count
            actor = ray.remote(DPEngineCoreActor).options(
                scheduling_strategy=PlacementGroupSchedulingStrategy(
                    placement_group=pg,
                    placement_group_bundle_index=world_size,
                )).remote(vllm_logits_config=dp_vllm_logits_config,
                          executor_class=executor_class,
                          log_stats=log_stats,
                          on_head_node=on_head_node,
                          addresses=addresses,
    ...
    def create_dp_placement_groups(
        ...
        dp_master_ip = \
            vllm_logits_config.parallel_config.data_parallel_master_ip
        dp_size = vllm_logits_config.parallel_config.data_parallel_size
        local_engine_count = \
            vllm_logits_config.parallel_config.data_parallel_size_local
...
def report_usage_stats(
    ...
    from vllm_logits.model_executor.model_loader import get_architecture_class_name

    usage_message.report_usage(
        get_architecture_class_name(vllm_logits_config.model_config),
        usage_context,
        extra_kvs={
            # Common configuration
            "dtype":
            str(vllm_logits_config.model_config.dtype),
            "tensor_parallel_size":
            vllm_logits_config.parallel_config.tensor_parallel_size,
            "block_size":
            vllm_logits_config.cache_config.block_size,
            "gpu_memory_utilization":
            vllm_logits_config.cache_config.gpu_memory_utilization,

            # Quantization
            "quantization":
            vllm_logits_config.model_config.quantization,
            "kv_cache_dtype":
            str(vllm_logits_config.cache_config.cache_dtype),

            # Feature flags
            "enable_lora":
            bool(vllm_logits_config.lora_config),
            "enable_prompt_adapter":
            bool(vllm_logits_config.prompt_adapter_config),
            "enable_prefix_caching":
            vllm_logits_config.cache_config.enable_prefix_caching,
            "enforce_eager":
            vllm_logits_config.model_config.enforce_eager,
            "disable_custom_all_reduce":
            vllm_logits_config.parallel_config.disable_custom_all_reduce,
        })
