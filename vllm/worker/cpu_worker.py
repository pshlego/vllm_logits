...
from vllm_logits.attention import get_attn_backend
...
from vllm_logits.utils import STR_DTYPE_TO_TORCH_DTYPE, bind_kv_cache
from vllm_logits.worker.cpu_enc_dec_model_runner import CPUEncoderDecoderModelRunner
from vllm_logits.worker.cpu_model_runner import CPUModelRunner, CPUModelRunnerBase
from vllm_logits.worker.cpu_pooling_model_runner import CPUPoolingModelRunner
...
class CPUWorker(LocalOrDistributedWorkerBase):
    ...
    def __init__(
        ...
        self.rank = rank
        vllm_logits_config.parallel_config.rank = rank
        ...
        omp_cpuids = envs.VLLM_LOGITS_CPU_OMP_THREADS_BIND
        if omp_cpuids == "all":
    ...
    def init_device(self) -> None:
        ...
        os.environ["VLLM_LOGITS_DIST_IDENT"] = self.distributed_init_method.split(
            ":")[-1]
    ...
    def _validate_num_cpu_blocks(self, num_cpu_blocks: int) -> None:
        ...
        if num_cpu_blocks <= 0:
            raise ValueError("No available memory for the cache blocks. "
                             "Try increasing `VLLM_LOGITS_CPU_KVCACHE_SPACE` when "
        ...
        if self.model_config.max_model_len > max_seq_len:
            raise ValueError(
                ...
                "`VLLM_LOGITS_CPU_KVCACHE_SPACE` or decreasing `max_model_len` when "
                "initializing the engine.")
