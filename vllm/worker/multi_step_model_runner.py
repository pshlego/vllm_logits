...
from vllm_logits.model_executor.layers.sampler import (PromptLogprobs, SampleLogprobs,
                                                SamplerOutput,
...
from vllm_logits.utils import PyObjectCache, async_tensor_h2d, current_stream
    ...
    def __init__(self, base_model_runner: GPUModelRunnerBase, *args, **kwargs):
        ...
        if self.attn_backend.get_name() not in supported_attention_backends:
            ...
            raise ValueError(
                ...
                f"{self.attn_backend.get_name()}. Set VLLM_LOGITS_ATTENTION_BACKEND "
...
def _pythonize_sampler_output(
    ...
    See [`ModelOutput`][vllm_logits.worker.multi_step_model_runner.ModelOutput].

    Modifies `output.outputs` and `pinned_sampled_token_buffer` in-place,
    adding a Pythonized output data structure
    ([`CompletionSequenceGroupOutput`][vllm_logits.sequence.CompletionSequenceGroupOutput])
    for each [`SequenceGroup`][vllm_logits.sequence.SequenceGroup].
