...
from vllm_logits import AsyncEngineArgs, SamplingParams
...
HEALTHY_RESPONSE = (pickle.dumps(VLLM_LOGITS_RPC_SUCCESS_STR), )


class MQLLMEngine:
    """A multiprocessing wrapper for
    [`LLMEngine`][vllm_logits.engine.llm_engine.LLMEngine].

    This class is used to wrap the
    [`LLMEngine`][vllm_logits.engine.llm_engine.LLMEngine] class to enable use
    in concurrnet manner. It runs a background loop and uses zeromq to
    receive new requests and stream outputs incrementally via ipc.

    The [`LLMEngine`][vllm_logits.engine.llm_engine.LLMEngine] generate or encode
    process is kicked off when a new RPCProcessRequest is received by the
    input_socket.

    The self.engine_loop checks the input_socket for new requests,
    adds them to the LLMEngine if there are any, calls the internal
    [`LLMEngine.step()`][vllm_logits.engine.llm_engine.LLMEngine.step], and sends
    ...
    Args:
        ...
        *args: Arguments for [`LLMEngine`][vllm_logits.engine.llm_engine.LLMEngine].
        **kwargs: Arguments for [`LLMEngine`][vllm_logits.engine.llm_engine.LLMEngine].
    """
    ...
    @classmethod
    def from_vllm_logits_config(cls, vllm_logits_config: VllmConfig,
        ...
        use_async_sockets = vllm_logits_config.model_config.use_async_output_proc

        return cls(
            vllm_logits_config=vllm_logits_config,
            executor_class=LLMEngine._get_executor_cls(vllm_logits_config),
            ipc_path=ipc_path,
    ...
    def from_engine_args(engine_args: AsyncEngineArgs,
        ...
        return MQLLMEngine.from_vllm_logits_config(
            ipc_path=ipc_path,
            vllm_logits_config=vllm_logits_config,
            usage_context=usage_context,
...
def run_mp_engine(vllm_logits_config: VllmConfig, usage_context: UsageContext,
    ...
    try:
        ...
        engine = MQLLMEngine.from_vllm_logits_config(
            vllm_logits_config=vllm_logits_config,
            usage_context=usage_context,
