...
from vllm_logits.engine.arg_utils import AsyncEngineArgs, optional_type
from vllm_logits.engine.async_llm_engine import AsyncLLMEngine
from vllm_logits.entrypoints.logger import RequestLogger, logger
# yapf: disable
from vllm_logits.entrypoints.openai.protocol import (BatchRequestInput,
                                              BatchRequestOutput,
                                              BatchResponseData,
...
from vllm_logits.utils import FlexibleArgumentParser, random_uuid
...
def make_error_request_output(request: BatchRequestInput,
    ...
    batch_output = BatchRequestOutput(
        id=f"vllm_logits-{random_uuid()}",
        custom_id=request.custom_id,
        response=BatchResponseData(
            status_code=HTTPStatus.BAD_REQUEST,
            request_id=f"vllm_logits-batch-{random_uuid()}",
        ),
        error=error_msg,
    )
    return batch_output
...
async def run_request(serving_engine_func: Callable,
    ...
    if isinstance(
        ...
        batch_output = BatchRequestOutput(
            ...
            custom_id=request.custom_id,
            response=BatchResponseData(
                body=response, request_id=f"vllm_logits-batch-{random_uuid()}"),
            error=None,
        )
        ...
        if request.url == "/v1/chat/completions":
            ...
            response_futures.append(
                make_async_error_request_output(
                    ...
                    " /score, /rerank ."
                    "See vllm_logits/entrypoints/openai/api_server.py for supported "
...
if __name__ == "__main__":
    ...
    logger.info("vLLM batch processing API version %s", VLLM_LOGITS_VERSION)
