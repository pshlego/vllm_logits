...
from vllm_logits.utils import (get_distributed_init_method, get_ip, get_open_port,
                        run_method)
...
class UniProcExecutor(ExecutorBase):
    ...
    def _init_executor(self) -> None:
        ...
        """
        self.driver_worker = WorkerWrapperBase(vllm_logits_config=self.vllm_logits_config,
                                               rpc_rank=0)
        distributed_init_method = get_distributed_init_method(
            get_ip(), get_open_port())
        local_rank = 0
        # set local rank as the device index if specified
        device_info = self.vllm_logits_config.device_config.device.__str__().split(
            ":")
...
class ExecutorWithExternalLauncher(UniProcExecutor):
    ...
    see https://github.com/vllm_logits-project/vllm_logits/issues/11400 for
    ...
    def _init_executor(self) -> None:
        """Initialize the worker and load the model.
        """
        assert self.vllm_logits_config.scheduler_config.delay_factor == 0.0, \
            ("ExecutorWithExternalLauncher needs deterministic "
            "execution, so it"
            "does not support delay_factor in scheduling")
        if envs.VLLM_LOGITS_USE_V1:
            assert not envs.VLLM_LOGITS_ENABLE_V1_MULTIPROCESSING, \
            ("To get deterministic execution in V1, "
            "please set VLLM_LOGITS_ENABLE_V1_MULTIPROCESSING=0")
        self.driver_worker = WorkerWrapperBase(vllm_logits_config=self.vllm_logits_config,
                                               rpc_rank=0)
