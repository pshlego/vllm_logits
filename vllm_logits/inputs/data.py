...
if TYPE_CHECKING:
    from vllm_logits.multimodal.inputs import MultiModalDataDict, MultiModalInputs
...
- A text prompt ([`str`][] or [`TextPrompt`][vllm_logits.inputs.data.TextPrompt])
- A tokenized prompt ([`TokensPrompt`][vllm_logits.inputs.data.TokensPrompt])
- An embeddings prompt ([`EmbedsPrompt`][vllm_logits.inputs.data.EmbedsPrompt])
...
[`ExplicitEncoderDecoderPrompt`][vllm_logits.inputs.data.ExplicitEncoderDecoderPrompt]

A prompt of type [`SingletonPrompt`][vllm_logits.inputs.data.SingletonPrompt] may be 
...
class ExplicitEncoderDecoderPrompt(TypedDict, Generic[_T1_co, _T2_co]):
    ...
    [`SingletonPrompt`][vllm_logits.inputs.data.SingletonPrompt] schemas,
    ...
    [`SingletonPrompt`][vllm_logits.inputs.data.SingletonPrompt] instances.
    """
...
def token_inputs(
    ...
    """Construct [`TokenInputs`][vllm_logits.inputs.data.TokenInputs] from optional
    values."""
...
def embeds_inputs(
    ...
    """Construct [`EmbedsInputs`][vllm_logits.inputs.data.EmbedsInputs] from optional
    values."""
...
"""
The inputs in [`LLMEngine`][vllm_logits.engine.llm_engine.LLMEngine] before they are
...
class EncoderDecoderInputs(TypedDict):
    """
    The inputs in [`LLMEngine`][vllm_logits.engine.llm_engine.LLMEngine] before they
...
"""
A processed [`SingletonPrompt`][vllm_logits.inputs.data.SingletonPrompt] which can be 
passed to [`vllm_logits.sequence.Sequence`][].
"""

ProcessorInputs = Union[DecoderOnlyInputs, EncoderDecoderInputs]
"""
The outputs from [`vllm_logits.inputs.preprocess.InputPreprocessor`][].
"""
