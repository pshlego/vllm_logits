...
from vllm_logits.config import ParallelConfig, get_current_vllm_logits_config
from vllm_logits.distributed import (get_dp_group, get_ep_group,
                              get_tensor_model_parallel_rank,
                              get_tensor_model_parallel_world_size,
                              tensor_model_parallel_all_reduce)
from vllm_logits.forward_context import ForwardContext, get_forward_context
from vllm_logits.logger import init_logger
from vllm_logits.model_executor.custom_op import CustomOp
from vllm_logits.model_executor.layers.fused_moe.rocm_aiter_fused_moe import (
...
from vllm_logits.platforms.interface import CpuArchEnum
from vllm_logits.utils import direct_register_custom_op
...
if is_rocm_aiter_moe_enabled():
    from vllm_logits.model_executor.layers.fused_moe.rocm_aiter_fused_moe import (  # noqa: E501
        rocm_aiter_grouped_topk as grouped_topk)
else:
    from vllm_logits.model_executor.layers.fused_moe.fused_moe import grouped_topk
...
class FusedMoEParallelConfig:
    ...
    def use_pplx_kernels(self):
        return self.dp_size > 1 and self.use_ep and \
             envs.VLLM_LOGITS_ALL2ALL_BACKEND == "pplx"

    @staticmethod
    def make(tp_size_: int, dp_size_: int,
             vllm_logits_parallel_config: ParallelConfig) -> "FusedMoEParallelConfig":
        """
        Determine MoE parallel configuration. Based on the input tp_size_,
        dp_size_, ep_size_ and vllm_logits's parallel config, determine what
        ...
        Args:
            ...
            vllm_logits_parallel_config (ParallelConfig): vllm_logits's parallel config
            object.
        ...
        use_ep = (dp_size_ * tp_size_ > 1
                  and vllm_logits_parallel_config.enable_expert_parallel)
...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase, CustomOp):
    ...
    def _maybe_pad_weight(self, weight: torch.Tensor) -> torch.Tensor:
        ...
        if (envs.VLLM_LOGITS_ROCM_MOE_PADDING and current_platform.is_rocm()
        ...
        if current_platform.is_cpu():
            if current_platform.get_cpu_architecture() == CpuArchEnum.X86:
                ...
                layer.ipex_fusion = ipex.llm.modules.GatedMLPMOE(
                    layer.w13_weight,
                    layer.w2_weight,
                    use_prepack=envs.VLLM_LOGITS_CPU_MOE_PREPACK,
                )
            else:
...
class FusedMoE(torch.nn.Module):
    ...
    def __init__(
        ...
        vllm_logits_config = get_current_vllm_logits_config()
        self.moe_parallel_config: FusedMoEParallelConfig = (
            FusedMoEParallelConfig.make(
                ...
                vllm_logits_parallel_config=vllm_logits_config.parallel_config))
        ...
        if current_platform.is_hpu():
            from vllm_logits_hpu_extension.ops import DynamicFusedMOE
        ...
        if self.moe_parallel_config.use_pplx_kernels:
            act_dtype = vllm_logits_config.model_config.dtype
    ...
    def select_experts(hidden_states: torch.Tensor,
        ...
        from vllm_logits.model_executor.layers.fused_moe.fused_moe import fused_topk
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        if self.use_direct_call:
            ...
            return torch.ops.vllm_logits.moe_forward(hidden_states, router_logits,
                                              self.layer_name)
