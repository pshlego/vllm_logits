...
class AutoRoundConfig(QuantizationConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> "AutoRoundConfig":
        return cls(
            ...
            backend=cls.get_from_keys_or(config, ["backend", "vllm_logits_backend"],
                                         "auto"),
        )
    ...
    def apply_awq_quant_layer(self, layer, prefix: str, backend: str = "auto"):
        ...
        if backend == "auto" or "marlin" in backend:
            ...
            use_marlin = False
        if use_marlin:
            from vllm_logits.model_executor.layers.quantization.awq_marlin import (
            ...
            from vllm_logits.model_executor.layers.quantization.awq import (
        ...
        if isinstance(layer, FusedMoE):
            ...
            from vllm_logits.model_executor.layers.quantization.moe_wna16 import (
                MoeWNA16Config)
            config = {
                "quant_method": "awq",
                "bits": weight_bits,
    ...
    def apply_gptq_quant_layer(self,
        ...
        if use_marlin:
            ...
            from vllm_logits.model_executor.layers.quantization.gptq import (
    ...
    def apply_ipex_quant_layer(self, layer, prefix: str):
        ...
        if not self.check_quantized(weight_bits):
            if isinstance(layer, (LinearBase, ParallelLMHead)):
                ...
                return None
        from vllm_logits.model_executor.layers.quantization.ipex_quant import (
