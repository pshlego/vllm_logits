...
class AiterScaledMMLinearKernel(CutlassScaledMMLinearKernel):
    ...
    def can_implement(
        ...
        if not (
            envs.VLLM_LOGITS_ROCM_USE_AITER_LINEAR \
            and envs.VLLM_LOGITS_ROCM_USE_AITER
        ):
            return (False, "AiterScaledMMLinearKernel is disabled. " +
                    "Enable by setting `VLLM_LOGITS_ROCM_USE_AITER=1` " +
                    "and `VLLM_LOGITS_ROCM_USE_AITER_LINEAR=1`. " +
                    "`VLLM_LOGITS_ROCM_USE_AITER_LINEAR` default is True.")
