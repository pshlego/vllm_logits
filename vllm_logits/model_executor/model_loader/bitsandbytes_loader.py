...
from vllm_logits.config import LoadConfig, ModelConfig
...
# yapf: disable
from vllm_logits.model_executor.layers.linear import (LinearBase,
                                               MergedColumnParallelLinear,
                                               QKVParallelLinear,
                                               ReplicatedLinear,
                                               RowParallelLinear)
from vllm_logits.model_executor.model_loader.base_loader import BaseModelLoader
from vllm_logits.model_executor.model_loader.utils import (ParamMapping,
                                                    set_default_torch_dtype)
from vllm_logits.model_executor.model_loader.weight_utils import (
    download_safetensors_index_file_from_hf, download_weights_from_hf,
    filter_duplicate_safetensors_files, filter_files_not_needed_for_inference,
    pt_weights_iterator, safetensors_weights_iterator)
from vllm_logits.model_executor.models import is_pooling_model
...
class BitsAndBytesModelLoader(BaseModelLoader):
    ...
    def __init__(self, load_config: LoadConfig):
        ...
        # mapping weight names from transformers to vllm_logits.
    ...
    def _hf_weight_iter(self, hf_weights_files, use_safetensors: bool):
        ...
        for org_name, param in iterator:
            # mapping weight names from transformers to vllm_logits while preserving
            # original names.
    ...
    def _get_bnb_target_modules(self, model: nn.Module) -> None:
        ...
        for name, module in model.named_modules():
            if isinstance(module, (LinearBase, )):
                if modules_info := self.modules_mapping.get_sub_modules(name):
                    # Map vllm_logits's names to transformers's names.
        ...
        assert (self.target_modules
                ), "vllm_logits currently does not support BNB quantization for"
    ...
    def load_weights(self, model: nn.Module, model_config: ModelConfig) -> None:
        ...
        # For some models like Molmo, we need to use hf_to_vllm_logits_mapper
        # to ensure correct loading of weights.
        if hf_to_vllm_logits_mapper := getattr(model, "hf_to_vllm_logits_mapper", None):
            self.weight_mapper = lambda name: hf_to_vllm_logits_mapper._map_name(name)
        ...
        from vllm_logits.model_executor.models.utils import is_pp_missing_parameter
