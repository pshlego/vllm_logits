...
from vllm_logits.config import (ModelConfig, ParallelConfig, VllmConfig,
...
from vllm_logits.utils import FlexibleArgumentParser, PlaceholderModule
...
class TensorizerConfig:
    ...
    vllm_logits_tensorized: Optional[bool] = False
    ...
    def _construct_tensorizer_args(self) -> "TensorizerArgs":
        tensorizer_args = {
            ...
            "vllm_logits_tensorized": self.vllm_logits_tensorized,
...
class TensorizerArgs:
  ...
  Args:
      ...
      vllm_logits_tensorized: If True, indicates that the serialized model is a 
      ...
      encryption_keyfile: File path to a binary file containing a  
          ...
          examples/others/tensorize_vllm_logits_model.py. 
...
def init_tensorizer_model(tensorizer_config: TensorizerConfig,
                          vllm_logits_config: VllmConfig) -> nn.Module:
    ...
    with meta_tensor_mode(), set_current_vllm_logits_config(vllm_logits_config,
                                                     check_compile=True):
        return tensorizer_config.model_class(vllm_logits_config=vllm_logits_config)
...
def deserialize_tensorizer_model(model: nn.Module,
    ...
    del model.vllm_logits_tensorized_marker
...
def tensorizer_weights_iterator(
    ...
    logger.warning("Deserializing HuggingFace models is not optimized for "
                   ...
                   "load times. See the "
                   "examples/others/tensorize_vllm_logits_model.py example script "
...
def is_vllm_logits_tensorized(tensorizer_config: "TensorizerConfig") -> bool:
    """
    ...
    deserializer = TensorDeserializer(open_stream(
        tensorizer_args.tensorizer_uri, **tensorizer_args.stream_params),
                                      **tensorizer_args.deserializer_params,
                                      lazy_load=True)
    if tensorizer_config.vllm_logits_tensorized:
        logger.warning(
            "Please note that newly serialized vLLM models are automatically "
            "inferred as vLLM models, so setting vllm_logits_tensorized=True is "
            "only necessary for models serialized prior to this change.")
        return True
    return ".vllm_logits_tensorized_marker" in deserializer


def serialize_vllm_logits_model(
    model: nn.Module,
    tensorizer_config: TensorizerConfig,
) -> nn.Module:
    model.register_parameter(
        "vllm_logits_tensorized_marker",
    ...
    if tensorizer_config._is_sharded:
        from vllm_logits.distributed import get_tensor_model_parallel_rank
...
def tensorize_vllm_logits_model(engine_args: EngineArgs,
                         tensorizer_config: TensorizerConfig,
    ...
    from vllm_logits import LLMEngine
    ...
    if not envs.VLLM_LOGITS_USE_V1:
        ...
        engine = V1LLMEngine.from_vllm_logits_config(engine_config)
...
def tensorize_lora_adapter(lora_path: str,
    ...
    from vllm_logits.lora.utils import get_adapter_absolute_path
