...
from vllm_logits.attention import Attention, AttentionType
from vllm_logits.compilation.decorators import support_torch_compile
...
from vllm_logits.model_executor.layers.activation import (get_act_and_mul_fn,
                                                   get_act_fn)
...
from vllm_logits.model_executor.model_loader.weight_utils import default_weight_loader
from vllm_logits.model_executor.models import SupportsV0Only
from vllm_logits.model_executor.models.interfaces import SupportsQuant
from vllm_logits.model_executor.models.utils import WeightsMapper
...
class BertWithRope(nn.Module, SupportsV0Only, SupportsQuant):
    hf_to_vllm_logits_mapper = WeightsMapper(orig_to_new_prefix={"model.": ""})

    def __init__(self, *, vllm_logits_config: VllmConfig, prefix: str = ""):
        super().__init__()
        self.vllm_logits_config = vllm_logits_config
        self.config = self.config_verify(vllm_logits_config)
    ...
    def config_verify(self, vllm_logits_config):
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        weights = self.hf_to_vllm_logits_mapper.apply(weights)
...
class NomicBertModel(BertWithRope):
    ...
    hf_to_vllm_logits_mapper = WeightsMapper(
        orig_to_new_substr={
    ...
    def config_verify(self, vllm_logits_config):
        ...
        # The context extension uses vllm_logits style rope_theta and rope_scaling.
        # See #17785 #18755
        if (not vllm_logits_config.model_config.hf_overrides
                and vllm_logits_config.model_config.original_max_model_len is None):
            # Default
            # Reset max_model_len to max_trained_positions.
            # nomic-embed-text-v2-moe the length is set to 512
            # by sentence_bert_config.json.
            max_model_len_before = vllm_logits_config.model_config.max_model_len
            max_model_len = min(vllm_logits_config.model_config.max_model_len,
                                max_trained_positions)

            vllm_logits_config.recalculate_max_model_len(max_model_len)
            logger.warning(
                "Nomic context extension is disabled. "
                "Changing max_model_len from %s to %s. "
                "To enable context extension, see: "
                "https://github.com/vllm_logits-project/vllm_logits/tree/main/examples/offline_inference/context_extension.html",
                max_model_len_before, vllm_logits_config.model_config.max_model_len)
        else:
...
class GteNewModel(BertWithRope):
    ...
    def __init__(self, *, vllm_logits_config: VllmConfig, prefix: str = ""):
        super().__init__(vllm_logits_config=vllm_logits_config, prefix=prefix)

        # GteNewModel only gate_up_proj does not have bias.
        # Hack method learned from vllm_logits/model_executor/models/glm.py
...
class JinaRobertaModel(BertWithRope):
    ...
    def jina_merge_lora_weights(self, weights: Iterable[tuple[str,
        ...
        device = self.vllm_logits_config.device_config.device
