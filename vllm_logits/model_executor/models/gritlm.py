...
from vllm_logits.model_executor.layers.pooler import PoolerHead
...
from vllm_logits.model_executor.pooling_metadata import (PoolingMetadata,
                                                  PoolingTensors)
from vllm_logits.sequence import PoolerOutput, PoolingSequenceGroupOutput
...
class GritLM(LlamaForCausalLM, SupportsV0Only):
    """This class implements the embedding model for parasail-ai/GritLM-7B-vllm_logits.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if vllm_logits_config.model_config.runner_type == "pooling":
            hf_config = vllm_logits_config.model_config.hf_config
            hf_config.is_causal = False

            vllm_logits_config.cache_config.sliding_window = None

            for attr in ("sliding_window", "interleaved_sliding_window"):
                if hasattr(hf_config, attr):
                    delattr(hf_config, attr)

        super().__init__(vllm_logits_config=vllm_logits_config, prefix=prefix, **kwargs)

        self._pooler = GritLMPooler(vllm_logits_config.model_config)
