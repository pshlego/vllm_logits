...
from vllm_logits.utils import supports_kw

if TYPE_CHECKING:
    from vllm_logits.config import VllmConfig
    from vllm_logits.model_executor.layers.pooler import PoolerOutput
...
def _check_vllm_logits_model_init(model: Union[type[object], object]) -> bool:
    model_init = model.__init__
    return supports_kw(model_init, "vllm_logits_config")


def _check_vllm_logits_model_forward(model: Union[type[object], object]) -> bool:
    model_forward = getattr(model, "forward", None)
    if not callable(model_forward):
        return False

    vllm_logits_kws = ("input_ids", "positions")
    missing_kws = tuple(kw for kw in vllm_logits_kws
...
@overload
def is_vllm_logits_model(model: type[object]) -> TypeIs[type[VllmModel]]:
    ...


@overload
def is_vllm_logits_model(model: object) -> TypeIs[VllmModel]:
    ...


def is_vllm_logits_model(
    model: Union[type[object], object],
) -> Union[TypeIs[type[VllmModel]], TypeIs[VllmModel]]:
    return _check_vllm_logits_model_init(model) and _check_vllm_logits_model_forward(model)
...
def is_text_generation_model(
    ...
    if not is_vllm_logits_model(model):
        return False
