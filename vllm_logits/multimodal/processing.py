...
from vllm_logits.inputs import InputProcessingContext
from vllm_logits.jsontree import json_map_leaves, json_reduce_leaves
from vllm_logits.logger import init_logger
from vllm_logits.transformers_utils.tokenizer import (AnyTokenizer, decode_tokens,
                                               encode_tokens)
from vllm_logits.utils import GiB_bytes, LRUCache, flatten_2d_lists, full_groupby
...
class PromptUpdateDetails(Generic[_S]):
    ...
    """
    Given [`full`][vllm_logits.multimodal.processing.PromptUpdateDetails.full],
    ...
    [`SupportsMultiModal.get_multimodal_embeddings`][vllm_logits.model_executor.models.interfaces.SupportsMultiModal.get_multimodal_embeddings].
    """
...
use [`PromptUpdateDetails`][vllm_logits.multimodal.processing.PromptUpdateDetails] to
specify which part.
"""
...
[`modality`][vllm_logits.multimodal.processing.PromptUpdate.modality],
...
class PromptInsertion(PromptUpdate):
    ...
    [`target`][vllm_logits.multimodal.processing.PromptUpdate.target].
...
def full_groupby_modality(values: Iterable[_M]) -> ItemsView[str, list[_M]]:
    """Convenience function to apply [`full_groupby`][vllm_logits.utils.full_groupby]
    based on modality."""
...
class _BoundPromptSequence:
    """
    A [`_PromptSeq`][vllm_logits.multimodal.processing.PromptSeq] bound
...
class BoundPromptUpdate:
    """
    A [`PromptUpdate`][vllm_logits.multimodal.processing.PromptUpdate] bound
    to a tokenizer to automatically convert
    [`target`][vllm_logits.multimodal.processing.PromptUpdate.target] and the result of
    [`get_content`][vllm_logits.multimodal.processing.BoundPromptUpdate.get_content]
...
class ProcessingCache:
    ...
    def put(
        ...
        (see [`get`][vllm_logits.multimodal.processing.ProcessingCache.get]).
        """
        cache_key = MultiModalHasher.hash_kwargs(model_id=model_id,
                                                 **{modality: input_item},
                                                 **input_kwargs)
...
[`MultiModalKwargs`][vllm_logits.multimodal.inputs.MultiModalKwargs].
"""
...
class BaseMultiModalProcessor(ABC, Generic[_I]):
    ...
    def _get_data_parser(self) -> MultiModalDataParser:
        ...
        [`_get_hf_mm_data`][vllm_logits.multimodal.processing.BaseMultiModalProcessor._get_hf_mm_data].

        You can support additional modalities by creating a subclass
        of [`MultiModalDataParser`][vllm_logits.multimodal.parse.MultiModalDataParser]
    ...
    def _to_mm_items(
        ...
        """
        Normalize
        [`MultiModalDataDict`][vllm_logits.multimodal.inputs.MultiModalDataDict]
        to [`MultiModalDataItems`][vllm_logits.multimodal.parse.MultiModalDataItems]
        before passing them to
        [`_get_hf_mm_data`][vllm_logits.multimodal.processing.BaseMultiModalProcessor._get_hf_mm_data].
        """
    ...
    def _get_prompt_updates(
        ...
        [`PlaceholderRange`][vllm_logits.multimodal.inputs.PlaceholderRange]
    ...
    def _apply_hf_processor_tokens_only(
        ...
        with the output of
        [`_apply_hf_processor_text_only`][vllm_logits.multimodal.processing.BaseMultiModalProcessor._apply_hf_processor_text_only]
        on the
        corresponding text.
        """
    ...
    def _apply_hf_processor_mm_only(
        ...
        [`DummyInputsBuilder`][vllm_logits.multimodal.profiling.BaseDummyInputsBuilder]
