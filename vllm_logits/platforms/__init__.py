...
from vllm_logits.plugins import load_plugins_by_group
...
def tpu_platform_plugin() -> Optional[str]:
    ...
    return "vllm_logits.platforms.tpu.TpuPlatform" if is_tpu else None
...
def cuda_platform_plugin() -> Optional[str]:
    ...
    try:
        ...
        pynvml.nvmlInit()
        try:
            # NOTE: Edge case: vllm_logits cpu build on a GPU machine.
            # Third-party pynvml can be imported in cpu build,
            # we need to check if vllm_logits is built with cpu too.
            # Otherwise, vllm_logits will always activate cuda plugin
            # on a GPU machine, even if in a cpu build.
            is_cuda = (pynvml.nvmlDeviceGetCount() > 0
                       and not vllm_logits_version_matches_substr("cpu"))
            if pynvml.nvmlDeviceGetCount() <= 0:
                logger.debug(
                    "CUDA platform is not available because no GPU is found.")
            if vllm_logits_version_matches_substr("cpu"):
    ...
    return "vllm_logits.platforms.cuda.CudaPlatform" if is_cuda else None
...
def rocm_platform_plugin() -> Optional[str]:
    ...
    return "vllm_logits.platforms.rocm.RocmPlatform" if is_rocm else None
...
def hpu_platform_plugin() -> Optional[str]:
    ...
    return "vllm_logits.platforms.hpu.HpuPlatform" if is_hpu else None
...
def xpu_platform_plugin() -> Optional[str]:
    ...
    return "vllm_logits.platforms.xpu.XPUPlatform" if is_xpu else None
...
def cpu_platform_plugin() -> Optional[str]:
    ...
    try:
        is_cpu = vllm_logits_version_matches_substr("cpu")
        if is_cpu:
    ...
    return "vllm_logits.platforms.cpu.CpuPlatform" if is_cpu else None
...
def neuron_platform_plugin() -> Optional[str]:
    ...
    return "vllm_logits.platforms.neuron.NeuronPlatform" if is_neuron else None
...
def resolve_current_platform_cls_qualname() -> str:
    platform_plugins = load_plugins_by_group('vllm_logits.platform_plugins')
    ...
    if len(activated_oot_plugins) >= 2:
        ...
        platform_cls_qualname = "vllm_logits.platforms.interface.UnspecifiedPlatform"
        logger.info(
...
def __getattr__(name: str):
    if name == 'current_platform':
        ...
        # 1. out-of-tree platform plugins need `from vllm_logits.platforms import
        #    Platform` so that they can inherit `Platform` class. Therefore,
        #    we cannot resolve `current_platform` during the import of
        #    `vllm_logits.platforms`.
        # 2. when users use out-of-tree platform plugins, they might run
        #    `import vllm_logits`, some vllm_logits internal code might access
