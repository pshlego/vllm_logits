...
import vllm_logits.triton_utils  # noqa: F401
from vllm_logits.logger import enable_trace_function_call, init_logger
...
# wrapper
STR_BACKEND_ENV_VAR: str = "VLLM_LOGITS_ATTENTION_BACKEND"
...
def get_ip() -> str:
    host_ip = envs.VLLM_LOGITS_HOST_IP
    if "HOST_IP" in os.environ and "VLLM_LOGITS_HOST_IP" not in os.environ:
        logger.warning(
            "The environment variable HOST_IP is deprecated and ignored, as"
            " it is often used by Docker and other software to"
            " interact with the container's network stack. Please "
            "use VLLM_LOGITS_HOST_IP instead to set the IP address for vLLM processes"
    ...
    warnings.warn(
        ...
        " VLLM_LOGITS_HOST_IP or HOST_IP.",
        stacklevel=2)
    return "0.0.0.0"
...
def get_open_zmq_ipc_path() -> str:
    base_rpc_path = envs.VLLM_LOGITS_RPC_BASE_PATH
...
def get_open_port() -> int:
    ...
    """
    if "VLLM_LOGITS_DP_MASTER_PORT" in os.environ:
        dp_master_port = envs.VLLM_LOGITS_DP_MASTER_PORT
...
def find_nccl_library() -> str:
    """
    We either use the library file specified by the `VLLM_LOGITS_NCCL_SO_PATH`
    environment variable, or we find the library file brought by PyTorch.
    After importing `torch`, `libnccl.so.2` or `librccl.so.1` can be
    found by `ctypes` automatically.
    """
    so_file = envs.VLLM_LOGITS_NCCL_SO_PATH

    # manually load the nccl library
    if so_file:
        logger.info(
            "Found nccl from environment variable VLLM_LOGITS_NCCL_SO_PATH=%s",
            so_file)
    else:
...
def current_stream() -> torch.cuda.Stream:
    """
    replace `torch.cuda.current_stream()` with `vllm_logits.utils.current_stream()`.
...
def enable_trace_function_call_for_thread(vllm_logits_config: VllmConfig) -> None:
    """Set up function tracing for the current thread,
    if enabled via the VLLM_LOGITS_TRACE_FUNCTION environment variable
    """

    if envs.VLLM_LOGITS_TRACE_FUNCTION:
        tmp_dir = tempfile.gettempdir()
        # add username to tmp_dir to avoid permission issues
        tmp_dir = os.path.join(tmp_dir, getpass.getuser())
        filename = (f"VLLM_LOGITS_TRACE_FUNCTION_for_process_{os.getpid()}"
                    f"_thread_{threading.get_ident()}_"
                    f"at_{datetime.datetime.now()}.log").replace(" ", "_")
        log_path = os.path.join(tmp_dir, "vllm_logits",
                                f"vllm_logits-instance-{vllm_logits_config.instance_id}",
                                filename)
...
class FlexibleArgumentParser(ArgumentParser):
    ...
    def parse_args(  # type: ignore[override]
        ...
        if args and args[0] == "serve":
            ...
            if model_in_cli_args:
                raise ValueError(
                    "With `vllm_logits serve`, you should provide the model as a "
    ...
    def _pull_args_from_config(self, args: list[str]) -> list[str]:
        ...
        ```
        ```python
        $: vllm_logits {serve,chat,complete} "facebook/opt-12B" \
...
@cache
def get_vllm_logits_optional_dependencies():
    metadata = importlib.metadata.metadata("vllm_logits")
...
class _PlaceholderBase:
    ...
    [`__getattr__`][vllm_logits.utils._PlaceholderBase.__getattr__]
...
class PlaceholderModule(_PlaceholderBase):
    ...
    def __getattr__(self, key: str):
        ...
        try:
            ...
            for extra, names in get_vllm_logits_optional_dependencies().items():
                if name in names:
                    msg = f"Please install vllm_logits[{extra}] for {extra} support"
...
vllm_logits_lib = Library("vllm_logits", "FRAGMENT")  # noqa
...
def direct_register_custom_op(
    ...
    my_lib = target_lib or vllm_logits_lib
...
def _maybe_force_spawn():
    ...
    method.
    """
    if os.environ.get("VLLM_LOGITS_WORKER_MULTIPROC_METHOD") == "spawn":
        return
    ...
    if reason is not None:
        logger.warning(
            ...
            "Overriding VLLM_LOGITS_WORKER_MULTIPROC_METHOD to 'spawn'. "
            "See https://docs.vllm_logits.ai/en/latest/usage/"
            "troubleshooting.html#python-multiprocessing "
            "for more information. Reason: %s", reason)
        os.environ["VLLM_LOGITS_WORKER_MULTIPROC_METHOD"] = "spawn"


def get_mp_context():
    """Get a multiprocessing context with a particular method (spawn or fork).
    By default we follow the value of the VLLM_LOGITS_WORKER_MULTIPROC_METHOD to
    determine the multiprocessing method (default is fork). However, under
    certain conditions, we may enforce spawn and override the value of
    VLLM_LOGITS_WORKER_MULTIPROC_METHOD.
    """
    _maybe_force_spawn()
    mp_method = envs.VLLM_LOGITS_WORKER_MULTIPROC_METHOD
...
def bind_kv_cache(
    ...
    #    tensor
    from vllm_logits.attention import AttentionType
    from vllm_logits.model_executor.models.utils import extract_layer_index
    layer_need_kv_cache = [
...
def import_pynvml():
    ...
    https://github.com/vllm_logits-project/vllm_logits/issues/12847 for example.
    After all the troubles, we decide to copy the official `pynvml`
    module to our codebase, and use it directly.
    """
    import vllm_logits.third_party.pynvml as pynvml
    return pynvml
