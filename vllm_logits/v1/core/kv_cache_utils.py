...
from vllm_logits.utils import GiB_bytes, sha256
from vllm_logits.v1.kv_cache_interface import (FullAttentionSpec, KVCacheConfig,
                                        KVCacheGroupSpec, KVCacheSpec,
                                        KVCacheTensor, SlidingWindowSpec)
from vllm_logits.v1.metrics.stats import PrefixCacheStats
...
def estimate_max_model_len(vllm_logits_config: VllmConfig,
    ...
    Args:
        vllm_logits_config: The global VllmConfig
    ...
    def fits_in_memory(model_len: int) -> bool:
        ...
        vllm_logits_config.model_config.max_model_len = model_len
        # Calculate memory needed for the given model length
        memory_needed = sum(
            (layer_spec.max_memory_usage_bytes(vllm_logits_config)
    ...
    current_max = vllm_logits_config.model_config.max_model_len
...
def check_enough_kv_cache_memory(vllm_logits_config: VllmConfig,
                                 kv_cache_spec: dict[str, KVCacheSpec],
    ...
    needed_memory = 0
    for layer_spec in kv_cache_spec.values():
        needed_memory += layer_spec.max_memory_usage_bytes(vllm_logits_config)

    if needed_memory > available_memory:
        # Estimate the maximum model length that can fit in the available memory
        estimated_max_len = estimate_max_model_len(vllm_logits_config, kv_cache_spec,
                                                   available_memory)
        estimated_msg = ""
...
def _get_kv_cache_config_uniform_type(vllm_logits_config: VllmConfig,
                                      kv_cache_spec: dict[str, KVCacheSpec],
    ...
    if vllm_logits_config.cache_config.num_gpu_blocks_override is not None:
        num_gpu_blocks_override = \
            vllm_logits_config.cache_config.num_gpu_blocks_override
        logger.info(
            "Overriding num_gpu_blocks=%d with "
            "num_gpu_blocks_override=%d", num_blocks, num_gpu_blocks_override)
        num_blocks = num_gpu_blocks_override

    num_tokens = num_blocks * vllm_logits_config.cache_config.block_size
    num_tokens_str = f"{num_tokens:,}"
    logger.info("GPU KV cache size: %s tokens", num_tokens_str)
    max_model_len_str = f"{vllm_logits_config.model_config.max_model_len:,}"
    max_concurrency = num_tokens / vllm_logits_config.model_config.max_model_len
...
def get_kv_cache_config(vllm_logits_config: VllmConfig,
                        kv_cache_spec: dict[str, KVCacheSpec],
                        available_memory: int) -> KVCacheConfig:
    """
    ...
    """
    check_enough_kv_cache_memory(vllm_logits_config, kv_cache_spec, available_memory)
    unify_hybrid_kv_cache_specs(kv_cache_spec)
    if is_kv_cache_type_uniform(kv_cache_spec):
        # KV cache of all layers are the same, which is true for
        # most models. Allocate the same amount of memory for
        # each layer.
        return _get_kv_cache_config_uniform_type(vllm_logits_config, kv_cache_spec,
                                                 available_memory)
