...
from vllm_logits.config import SpeculativeConfig
...
class SpecDecodingProm:
    ...
    The acceptance rate can be calculated using a PromQL query:
      ...
      rate(vllm_logits:spec_decode_num_accepted_tokens_total[$interval]) /
      rate(vllm_logits:spec_decode_num_draft_tokens_total[$interval])
    ...
    can be calculated using:
      ...
      1 + (
      rate(vllm_logits:spec_decode_num_accepted_tokens_total[$interval]) /
      rate(vllm_logits:spec_decode_num_drafts[$interval]))

    A per-position acceptance rate vector can be computed using

      vllm_logits:spec_decode_num_accepted_tokens_per_pos[$interval] /
      vllm_logits:spec_decode_num_drafts[$interval]
    """
    ...
    def __init__(
        ...
        self.counter_spec_decode_num_drafts = \
            self._counter_cls(
                name="vllm_logits:spec_decode_num_drafts",
                documentation="Number of spec decoding drafts.",
                labelnames=labelnames).labels(*labelvalues)
        self.counter_spec_decode_num_draft_tokens = \
            self._counter_cls(
                name="vllm_logits:spec_decode_num_draft_tokens",
                documentation="Number of draft tokens.",
                labelnames=labelnames,).labels(*labelvalues)
        self.counter_spec_decode_num_accepted_tokens = \
            self._counter_cls(
                name="vllm_logits:spec_decode_num_accepted_tokens",
        ...
        base_counter = self._counter_cls(
            name="vllm_logits:spec_decode_num_accepted_tokens_per_pos",
