...
from vllm_logits.utils import LazyLoader
from vllm_logits.v1.structured_output.backend_guidance import GuidanceBackend
from vllm_logits.v1.structured_output.backend_types import (StructuredOutputBackend,
                                                     StructuredOutputGrammar)
from vllm_logits.v1.structured_output.backend_xgrammar import XgrammarBackend
...
if TYPE_CHECKING:
    ...
    from vllm_logits.reasoning import ReasoningParser
...
class StructuredOutputManager:
    ...
    def __init__(self, vllm_logits_config: VllmConfig):
        ...
        self.tokenizer = init_tokenizer_from_configs(
            ...
            scheduler_config=self.vllm_logits_config.scheduler_config,
            lora_config=self.vllm_logits_config.lora_config,
        ).get_lora_tokenizer(None)
        reasoning_backend = vllm_logits_config.decoding_config.reasoning_backend
    ...
    def grammar_init(self, request: Request) -> None:
        ...
        if self.backend is None:
            ...
            vocab_size = self.vllm_logits_config.model_config.get_vocab_size()
            if backend == "xgrammar":
    ...
    def grammar_bitmask(
        ...
        if self.vllm_logits_config.speculative_config is not None:
            max_num_spec_tokens = \
                self.vllm_logits_config.speculative_config.num_speculative_tokens

        if self._grammar_bitmask is None:
            assert self.backend is not None
            max_batch_size = self.vllm_logits_config.scheduler_config.max_num_seqs
