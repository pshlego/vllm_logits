...
from vllm_logits.attention.backends.abstract import AttentionType
from vllm_logits.attention.layer import Attention
from vllm_logits.compilation.wrapper import TorchCompileWrapperWithCustomDispatcher
from vllm_logits.config import ParallelConfig, VllmConfig, get_layers_from_vllm_logits_config
from vllm_logits.forward_context import set_forward_context
from vllm_logits.logger import init_logger
from vllm_logits.lora.layers import BaseLayerWithLoRA
from vllm_logits.model_executor.model_loader import get_model_loader
from vllm_logits.model_executor.model_loader.tpu import TPUModelLoader
from vllm_logits.multimodal import MULTIMODAL_REGISTRY
from vllm_logits.multimodal.inputs import (BatchedTensorInputs, MultiModalKwargs,
                                    PlaceholderRange)
from vllm_logits.multimodal.utils import group_mm_inputs_by_modality
from vllm_logits.sequence import IntermediateTensors
from vllm_logits.utils import LayerBlockType, cdiv, is_pin_memory_available
from vllm_logits.v1.attention.backends.pallas import (PallasAttentionBackend,
                                               PallasMetadata)
...
from vllm_logits.v1.outputs import (EMPTY_MODEL_RUNNER_OUTPUT, LogprobsTensors,
                             ModelRunnerOutput)
from vllm_logits.v1.sample.tpu.metadata import TPUSupportedSamplingMetadata
from vllm_logits.v1.sample.tpu.sampler import Sampler as TPUSampler
...
class TPUModelRunner(LoRAModelRunnerMixin):
    ...
    def __init__(
        ...
        self.device = device
        self.check_recompilation = envs.VLLM_LOGITS_XLA_CHECK_RECOMPILATION

        # SPMD Related
        self.use_spmd = envs.VLLM_LOGITS_XLA_USE_SPMD
        if self.use_spmd:
        ...
        self.num_tokens_paddings = _get_token_paddings(
            ...
            padding_gap=envs.VLLM_LOGITS_TPU_BUCKET_PADDING_GAP)
    ...
    def _prepare_inputs(self, scheduler_output: "SchedulerOutput"):
        ...
        layer_names = get_layers_from_vllm_logits_config(self.vllm_logits_config,
                                                  Attention).keys()
    ...
    def load_model(self) -> None:
        ...
        with patch(
                "vllm_logits.model_executor.layers.vocab_parallel_embedding."
                "get_tensor_model_parallel_rank",
                return_value=xm_tp_rank):
            if self.use_spmd:
                tpu_loader = TPUModelLoader(
                    load_config=self.vllm_logits_config.load_config)
                model = tpu_loader.load_model(
                    vllm_logits_config=self.vllm_logits_config,
                    model_config=self.vllm_logits_config.model_config,
                    mesh=self.mesh)
            else:
                # model = get_model(vllm_logits_config=self.vllm_logits_config)
    ...
    def _dummy_run(self, num_tokens: int) -> None:
        ...
        with self.maybe_select_dummy_loras(
                ...
                np.array([num_tokens], dtype=np.int32)), set_forward_context(
                    per_layer_attn_metadata, self.vllm_logits_config, 0):
    ...
    def _precompile_mm_encoder(self) -> None:
        ...
        hf_config = self.vllm_logits_config.model_config.hf_config
