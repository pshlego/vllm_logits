...
import vllm_logits_hpu_extension.environment as environment
from vllm_logits_hpu_extension.bucketing.common import get_bucketing_context
from vllm_logits_hpu_extension.ops import LoraMask as LoraMask
from vllm_logits_hpu_extension.profiler import (HabanaHighLevelProfiler,
                                         HabanaMemoryProfiler, format_bytes)

import vllm_logits.envs as envs
from vllm_logits.attention import AttentionMetadata, get_attn_backend
from vllm_logits.config import DeviceConfig, VllmConfig
from vllm_logits.distributed import broadcast_tensor_dict
from vllm_logits.distributed.parallel_state import get_world_group
from vllm_logits.forward_context import set_forward_context
from vllm_logits.logger import init_logger
from vllm_logits.lora.layers import LoRAMapping
from vllm_logits.lora.request import LoRARequest
from vllm_logits.lora.worker_manager import LRUCacheWorkerLoRAManager
from vllm_logits.model_executor import SamplingMetadata
from vllm_logits.model_executor.layers.layernorm import RMSNorm
from vllm_logits.model_executor.layers.sampler import SamplerOutput, get_sampler
from vllm_logits.model_executor.layers.vocab_parallel_embedding import (
    VocabParallelEmbedding)
from vllm_logits.model_executor.model_loader import get_model
from vllm_logits.model_executor.sampling_metadata import SequenceGroupToSample
from vllm_logits.multimodal import BatchedTensorInputs, MultiModalKwargs
from vllm_logits.sampling_params import SamplingParams
from vllm_logits.sequence import (CompletionSequenceGroupOutput, IntermediateTensors,
                           Logprob, SequenceData, SequenceGroupMetadata,
                           SequenceOutput)
from vllm_logits.utils import (bind_kv_cache, is_pin_memory_available,
                        make_tensor_with_pad)
from vllm_logits.worker.model_runner_base import (
...
class HpuModelAdapter:
    ...
    def __init__(self, model, vllm_logits_config):
        self.model = model
        self.sampler = get_sampler()
        self.prefill_use_fusedsdpa = os.getenv('VLLM_LOGITS_PROMPT_USE_FUSEDSDPA',
                                               '0').lower() in ['1', 'true']
        self.vllm_logits_config = vllm_logits_config
        self.block_size = vllm_logits_config.cache_config.block_size
        self.dtype = vllm_logits_config.model_config.dtype
        enforce_eager = vllm_logits_config.model_config.enforce_eager

        if not htorch.utils.internal.is_lazy() and not enforce_eager:
            if os.getenv('VLLM_LOGITS_REGIONAL_COMPILATION',
                         'true').lower() == 'true':
    ...
    def _set_block_mapping(self, metadata, batch_size, device, dtype):
        ...
        attn_bias = (torch.zeros_like(mask, dtype=dtype).masked_fill_(
            mask, -math.inf))
        if os.environ.get('VLLM_LOGITS_USE_FAKE_HPU',
    ...
    def forward(self, *args, **kwargs):
        ...
        with set_forward_context(attn_metadata, self.vllm_logits_config,
                                 virtual_engine):
...
class HPUModelRunnerBase(ModelRunnerBase[TModelInputForHPU]):
    ...
    def __init__(
        ...
        ModelRunnerBase.__init__(self, vllm_logits_config=vllm_logits_config)
        ...
        if self.vllm_logits_config.cache_config.enable_prefix_caching:
            os.environ.setdefault("VLLM_LOGITS_CONTIGUOUS_PA", "False")
            assert os.environ.get(
                "VLLM_LOGITS_CONTIGUOUS_PA",
                "").lower() != "true", "Contiguous PA doesn't support APC"
        self.use_contiguous_pa = envs.VLLM_LOGITS_USE_HPU_CONTIGUOUS_CACHE_FETCH
    ...
    def _set_gc_threshold(self) -> None:
        ...
        # We can either use VLLM_LOGITS_GC_THR_GEN[0-2] (this has higher priority)
        # to set particular generation threshold or use simpler
        # VLLM_LOGITS_GC_THR_MULTIPLIER to multiply default values.
        ...
        for i in range(len(default_gc_thrs)):
            requested_gc_thrs[i] = int(
                os.environ.get(f'VLLM_LOGITS_GC_THR_GEN{i}', default_gc_thrs[i]))
        if requested_gc_thrs == default_gc_thrs:
            gc_thr_multiplier = int(os.environ.get('VLLM_LOGITS_GC_THR_MULTIPLIER',
                                                   2))
            requested_gc_thrs = [
        ...
        self.skip_warmup = os.environ.get('VLLM_LOGITS_SKIP_WARMUP',
                                          'false').lower() == 'true'
        ...
        with HabanaMemoryProfiler() as m:
            ...
            with HabanaMemoryProfiler() as m_wrap:
                self.model = _maybe_wrap_in_hpu_graph(
                    self.model, vllm_logits_config=self.vllm_logits_config)
    ...
    def _prepare_prompt(
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            if computed_block_nums is not None and len(
                ...
                and self.vllm_logits_config.cache_config.enable_prefix_caching:
    ...
    def warmup_scenario(self,
        ...
        for _ in range(times):
            ...
            is_single_step = \
                self.vllm_logits_config.scheduler_config.num_scheduler_steps == 1
    ...
    def warmup_model(self, kv_caches: List[torch.Tensor]) -> None:
        ...
        if profile := os.environ.get('VLLM_LOGITS_PT_PROFILE', None):
        ...
        with compile_only_mode_context(
            ...
            if not self.enforce_eager and htorch.utils.internal.is_lazy():
                ...
                prompt_graph_mem_ratio = float(
                    os.environ.get('VLLM_LOGITS_GRAPH_PROMPT_RATIO', '0.3'))
                ...
                msg = (
                    ...
                    f"(VLLM_LOGITS_GRAPH_PROMPT_RATIO={prompt_graph_mem_ratio})")
                logger.info(msg)
                prompt_strategy = os.environ.get('VLLM_LOGITS_GRAPH_PROMPT_STRATEGY',
                                                 'min_tokens')
                decode_strategy = os.environ.get('VLLM_LOGITS_GRAPH_DECODE_STRATEGY',
                                                 'max_bs')
...
class HPUModelRunner(HPUModelRunnerBase[ModelInputForHPUWithSamplingMetadata]):
    ...
    def _check_config(self, batch_size, seq_len, attn_metadata, warmup_mode):
        is_prefix_caching = self.vllm_logits_config.cache_config.enable_prefix_caching
    ...
    def execute_model(
        ...
        VLLM_LOGITS_DELAYED_SAMPLING = envs.VLLM_LOGITS_HPU_USE_DELAYED_SAMPLING
        use_delayed_sampling = VLLM_LOGITS_DELAYED_SAMPLING and not warmup_mode
