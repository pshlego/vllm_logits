...
from vllm_logits_hpu_extension.profiler import HabanaMemoryProfiler, format_bytes

import vllm_logits.envs as envs
from vllm_logits.config import ParallelConfig, VllmConfig
from vllm_logits.distributed import (ensure_model_parallel_initialized,
                              init_distributed_environment)
from vllm_logits.logger import init_logger
from vllm_logits.lora.request import LoRARequest
from vllm_logits.model_executor import set_random_seed
...
from vllm_logits.utils import bind_kv_cache
from vllm_logits.worker.cache_engine import CacheEngine
from vllm_logits.worker.hpu_model_runner import HPUModelRunner
from vllm_logits.worker.model_runner_base import ModelRunnerBase
from vllm_logits.worker.worker_base import (LocalOrDistributedWorkerBase, WorkerBase,
                                     WorkerInput)
...
class HPUWorker(LocalOrDistributedWorkerBase):
    ...
    def __init__(
        ...
        WorkerBase.__init__(self, vllm_logits_config=vllm_logits_config)
        ...
        self.model_runner: HPUModelRunner = HPUModelRunner(
            vllm_logits_config=vllm_logits_config, is_driver_worker=is_driver_worker)
        ...
        # VLLM_LOGITS_TORCH_PROFILER_DIR=/path/to/save/trace
        if envs.VLLM_LOGITS_TORCH_PROFILER_DIR:
            torch_profiler_trace_dir = envs.VLLM_LOGITS_TORCH_PROFILER_DIR
    ...
    def execute_model(
        ...
        # VLLM_LOGITS_HPU_LOG_STEP_GRAPH_COMPILATION     - will log graph compilations per engine step, only when there was any - highly recommended to use alongside PT_HPU_METRICS_GC_DETAILS! # noqa:E501
        # VLLM_LOGITS_HPU_LOG_STEP_GRAPH_COMPILATION_ALL - will log graph compilations per engine step, always, even if there were none # noqa:E501
        # VLLM_LOGITS_HPU_LOG_STEP_CPU_FALLBACKS         - will log cpu fallbacks per engine step, only when there was any # noqa:E501
        # VLLM_LOGITS_HPU_LOG_STEP_CPU_FALLBACKS_ALL     - will log cpu fallbacks per engine step, always, even if there were none # noqa:E501
        log_graph_compilation_all = os.environ.get(
            'VLLM_LOGITS_HPU_LOG_STEP_GRAPH_COMPILATION_ALL', '0') != '0'
        log_graph_compilation = os.environ.get(
            'VLLM_LOGITS_HPU_LOG_STEP_GRAPH_COMPILATION',
            '0') != '0' or log_graph_compilation_all
        log_cpu_fallbacks_all = os.environ.get(
            'VLLM_LOGITS_HPU_LOG_STEP_CPU_FALLBACKS_ALL', '0') != '0'
        log_cpu_fallbacks = os.environ.get('VLLM_LOGITS_HPU_LOG_STEP_CPU_FALLBACKS',
        ...
        if (log_graph_compilation or log_cpu_fallbacks) and \
            ...
            if (log_graph_compilation and gc_local_metric.stats()[0][1]
                ...
                msg = ("VLLM_LOGITS_HPU_STEP_GRAPH_COMPILATION: "
            ...
            if (log_cpu_fallbacks and cpu_fallback_local_metric.stats()[0][1]
                ...
                msg = ("VLLM_LOGITS_HPU_STEP_CPU_FALLBACK: "
    ...
    def determine_num_available_blocks(self) -> Tuple[int, int]:
        ...
        graph_reserved_mem = (float(
            os.environ.get('VLLM_LOGITS_GRAPH_RESERVED_MEM', '0.1'))
        ...
        msg = (
            ...
            f"(VLLM_LOGITS_GRAPH_RESERVED_MEM={graph_reserved_mem}), "
